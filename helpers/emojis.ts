const emojis = [
  {
    hexcode: '1F600',
    shortcodes: ['gleeful'],
    emoji: '😀',
    order: 1,
    group: 0,
  },
  {
    hexcode: '1F603',
    shortcodes: ['glad', 'smile'],
    emoji: '😃',
    order: 2,
    group: 0,
  },
  {
    hexcode: '1F604',
    shortcodes: ['happy'],
    emoji: '😄',
    order: 3,
    group: 0,
  },
  {
    hexcode: '1F601',
    shortcodes: ['blissful', 'grin'],
    emoji: '😁',
    order: 4,
    group: 0,
  },
  {
    hexcode: '1F606',
    shortcodes: ['amused', 'laugh', 'lol'],
    emoji: '😆',
    order: 5,
    group: 0,
  },
  {
    hexcode: '1F605',
    shortcodes: ['embarassed'],
    emoji: '😅',
    order: 6,
    group: 0,
  },
  {
    hexcode: '1F923',
    shortcodes: ['entertained', 'rofl'],
    emoji: '🤣',
    order: 7,
    group: 0,
  },
  {
    hexcode: '1F602',
    shortcodes: ['joyful', 'haha'],
    emoji: '😂',
    order: 8,
    group: 0,
  },
  {
    hexcode: '1F642',
    shortcodes: ['pleased'],
    emoji: '🙂',
    order: 9,
    group: 0,
  },
  {
    hexcode: '1F643',
    shortcodes: ['ecstatic', 'upside_down'],
    emoji: '🙃',
    order: 10,
    group: 0,
  },
  {
    hexcode: '1F609',
    shortcodes: ['coy', 'wink'],
    emoji: '😉',
    order: 11,
    group: 0,
  },
  {
    hexcode: '1F60A',
    shortcodes: ['anxious', 'blush'],
    emoji: '😊',
    order: 12,
    group: 0,
  },
  {
    hexcode: '1F607',
    shortcodes: ['innocent', 'halo'],
    emoji: '😇',
    order: 13,
    group: 0,
  },
  {
    hexcode: '1F970',
    shortcodes: ['love'],
    emoji: '🥰',
    order: 14,
    group: 0,
  },
  {
    hexcode: '1F60D',
    shortcodes: ['lovestruck'],
    emoji: '😍',
    order: 15,
    group: 0,
  },
  {
    hexcode: '1F929',
    shortcodes: ['starstruck'],
    emoji: '🤩',
    order: 16,
    group: 0,
  },
  {
    hexcode: '1F618',
    shortcodes: ['flirty'],
    emoji: '😘',
    order: 17,
    group: 0,
  },
  {
    hexcode: '1F617',
    shortcodes: ['kiss'],
    emoji: '😗',
    order: 18,
    group: 0,
  },
  {
    hexcode: '263A',
    shortcodes: ['relaxed'],
    emoji: '☺️',
    order: 20,
    group: 0,
  },
  {
    hexcode: '1F61A',
    shortcodes: ['loving_kiss'],
    emoji: '😚',
    order: 21,
    group: 0,
  },
  {
    hexcode: '1F619',
    shortcodes: ['happy_kiss'],
    emoji: '😙',
    order: 22,
    group: 0,
  },
  {
    hexcode: '1F60B',
    shortcodes: ['yum', 'savour'],
    emoji: '😋',
    order: 23,
    group: 0,
  },
  {
    hexcode: '1F61B',
    shortcodes: ['playful', 'tongue_out'],
    emoji: '😛',
    order: 24,
    group: 0,
  },
  {
    hexcode: '1F61C',
    shortcodes: ['mischievous'],
    emoji: '😜',
    order: 25,
    group: 0,
  },
  {
    hexcode: '1F92A',
    shortcodes: ['crazy'],
    emoji: '🤪',
    order: 26,
    group: 0,
  },
  {
    hexcode: '1F61D',
    shortcodes: ['facetious', 'lmao'],
    emoji: '😝',
    order: 27,
    group: 0,
  },
  {
    hexcode: '1F911',
    shortcodes: ['pretentious', 'money_mouth'],
    emoji: '🤑',
    order: 28,
    group: 0,
  },
  {
    hexcode: '1F917',
    shortcodes: ['hugging'],
    emoji: '🤗',
    order: 29,
    group: 0,
  },
  {
    hexcode: '1F92D',
    shortcodes: ['gasp'],
    emoji: '🤭',
    order: 30,
    group: 0,
  },
  {
    hexcode: '1F92B',
    shortcodes: ['shushing'],
    emoji: '🤫',
    order: 31,
    group: 0,
  },
  {
    hexcode: '1F914',
    shortcodes: ['curious', 'thinking'],
    emoji: '🤔',
    order: 32,
    group: 0,
  },
  {
    hexcode: '1F910',
    shortcodes: ['silenced', 'zipper_mouth'],
    emoji: '🤐',
    order: 33,
    group: 0,
  },
  {
    hexcode: '1F928',
    shortcodes: ['contempt'],
    emoji: '🤨',
    order: 34,
    group: 0,
  },
  {
    hexcode: '1F610',
    shortcodes: ['indifferent', 'neutral'],
    emoji: '😐️',
    order: 35,
    group: 0,
  },
  {
    hexcode: '1F611',
    shortcodes: ['apathetic', 'expressionless'],
    emoji: '😑',
    order: 36,
    group: 0,
  },
  {
    hexcode: '1F636',
    shortcodes: ['vacant', 'no_mouth'],
    emoji: '😶',
    order: 37,
    group: 0,
  },
  {
    hexcode: '1F60F',
    shortcodes: ['cocky', 'smirk'],
    emoji: '😏',
    order: 38,
    group: 0,
  },
  {
    hexcode: '1F612',
    shortcodes: ['unamused'],
    emoji: '😒',
    order: 39,
    group: 0,
  },
  {
    hexcode: '1F644',
    shortcodes: ['disbelief'],
    emoji: '🙄',
    order: 40,
    group: 0,
  },
  {
    hexcode: '1F62C',
    shortcodes: ['grimaced'],
    emoji: '😬',
    order: 41,
    group: 0,
  },
  {
    hexcode: '1F925',
    shortcodes: ['lying'],
    emoji: '🤥',
    order: 42,
    group: 0,
  },
  {
    hexcode: '1F60C',
    shortcodes: ['relieved'],
    emoji: '😌',
    order: 43,
    group: 0,
  },
  {
    hexcode: '1F614',
    shortcodes: ['pensive'],
    emoji: '😔',
    order: 44,
    group: 0,
  },
  {
    hexcode: '1F62A',
    shortcodes: ['sleepy'],
    emoji: '😪',
    order: 45,
    group: 0,
  },
  {
    hexcode: '1F924',
    shortcodes: ['drooling'],
    emoji: '🤤',
    order: 46,
    group: 0,
  },
  {
    hexcode: '1F634',
    shortcodes: ['exhausted', 'sleeping'],
    emoji: '😴',
    order: 47,
    group: 0,
  },
  {
    hexcode: '1F637',
    shortcodes: ['ill', 'mask'],
    emoji: '😷',
    order: 48,
    group: 0,
  },
  {
    hexcode: '1F912',
    shortcodes: ['sick'],
    emoji: '🤒',
    order: 49,
    group: 0,
  },
  {
    hexcode: '1F915',
    shortcodes: ['injured'],
    emoji: '🤕',
    order: 50,
    group: 0,
  },
  {
    hexcode: '1F922',
    shortcodes: ['nauseated'],
    emoji: '🤢',
    order: 51,
    group: 0,
  },
  {
    hexcode: '1F92E',
    shortcodes: ['vomiting'],
    emoji: '🤮',
    order: 52,
    group: 0,
  },
  {
    hexcode: '1F927',
    shortcodes: ['sneezing'],
    emoji: '🤧',
    order: 53,
    group: 0,
  },
  {
    hexcode: '1F975',
    shortcodes: ['overheating'],
    emoji: '🥵',
    order: 54,
    group: 0,
  },
  {
    hexcode: '1F976',
    shortcodes: ['freezing'],
    emoji: '🥶',
    order: 55,
    group: 0,
  },
  {
    hexcode: '1F974',
    shortcodes: ['woozy'],
    emoji: '🥴',
    order: 56,
    group: 0,
  },
  {
    hexcode: '1F635',
    shortcodes: ['dizzy'],
    emoji: '😵',
    order: 57,
    group: 0,
  },
  {
    hexcode: '1F92F',
    shortcodes: ['shocked', 'exploding_head'],
    emoji: '🤯',
    order: 58,
    group: 0,
  },
  {
    hexcode: '1F920',
    shortcodes: ['cowboy'],
    emoji: '🤠',
    order: 59,
    group: 0,
  },
  {
    hexcode: '1F973',
    shortcodes: ['partying', 'celebrating'],
    emoji: '🥳',
    order: 60,
    group: 0,
  },
  {
    hexcode: '1F60E',
    shortcodes: ['confident'],
    emoji: '😎',
    order: 61,
    group: 0,
  },
  {
    hexcode: '1F913',
    shortcodes: ['nerd'],
    emoji: '🤓',
    order: 62,
    group: 0,
  },
  {
    hexcode: '1F9D0',
    shortcodes: ['monocle'],
    emoji: '🧐',
    order: 63,
    group: 0,
  },
  {
    hexcode: '1F615',
    shortcodes: ['confused'],
    emoji: '😕',
    order: 64,
    group: 0,
  },
  {
    hexcode: '1F61F',
    shortcodes: ['worried'],
    emoji: '😟',
    order: 65,
    group: 0,
  },
  {
    hexcode: '1F641',
    shortcodes: ['cheerless'],
    emoji: '🙁',
    order: 66,
    group: 0,
  },
  {
    hexcode: '2639',
    shortcodes: ['sad', 'frowning'],
    emoji: '☹️',
    order: 68,
    group: 0,
  },
  {
    hexcode: '1F62E',
    shortcodes: ['surprised'],
    emoji: '😮',
    order: 69,
    group: 0,
  },
  {
    hexcode: '1F62F',
    shortcodes: ['hushed'],
    emoji: '😯',
    order: 70,
    group: 0,
  },
  {
    hexcode: '1F632',
    shortcodes: ['astonished'],
    emoji: '😲',
    order: 71,
    group: 0,
  },
  {
    hexcode: '1F633',
    shortcodes: ['flushed'],
    emoji: '😳',
    order: 72,
    group: 0,
  },
  {
    hexcode: '1F97A',
    shortcodes: ['pleading'],
    emoji: '🥺',
    order: 73,
    group: 0,
  },
  {
    hexcode: '1F626',
    shortcodes: ['bored'],
    emoji: '😦',
    order: 74,
    group: 0,
  },
  {
    hexcode: '1F627',
    shortcodes: ['anguished', 'wtf'],
    emoji: '😧',
    order: 75,
    group: 0,
  },
  {
    hexcode: '1F628',
    shortcodes: ['fearful'],
    emoji: '😨',
    order: 76,
    group: 0,
  },
  {
    hexcode: '1F630',
    shortcodes: ['frustrated'],
    emoji: '😰',
    order: 77,
    group: 0,
  },
  {
    hexcode: '1F625',
    shortcodes: ['hopeful'],
    emoji: '😥',
    order: 78,
    group: 0,
  },
  {
    hexcode: '1F622',
    shortcodes: ['upset', 'cry'],
    emoji: '😢',
    order: 79,
    group: 0,
  },
  {
    hexcode: '1F62D',
    shortcodes: ['distressed', 'sob'],
    emoji: '😭',
    order: 80,
    group: 0,
  },
  {
    hexcode: '1F631',
    shortcodes: ['frightened', 'scream'],
    emoji: '😱',
    order: 81,
    group: 0,
  },
  {
    hexcode: '1F616',
    shortcodes: ['confounded'],
    emoji: '😖',
    order: 82,
    group: 0,
  },
  {
    hexcode: '1F623',
    shortcodes: ['persevered'],
    emoji: '😣',
    order: 83,
    group: 0,
  },
  {
    hexcode: '1F61E',
    shortcodes: ['disappointed'],
    emoji: '😞',
    order: 84,
    group: 0,
  },
  {
    hexcode: '1F613',
    shortcodes: ['shamed'],
    emoji: '😓',
    order: 85,
    group: 0,
  },
  {
    hexcode: '1F629',
    shortcodes: ['weary'],
    emoji: '😩',
    order: 86,
    group: 0,
  },
  {
    hexcode: '1F62B',
    shortcodes: ['tired'],
    emoji: '😫',
    order: 87,
    group: 0,
  },
  {
    hexcode: '1F971',
    shortcodes: ['yawn'],
    emoji: '🥱',
    order: 88,
    group: 0,
  },
  {
    hexcode: '1F624',
    shortcodes: ['annoyed', 'hrmph'],
    emoji: '😤',
    order: 89,
    group: 0,
  },
  {
    hexcode: '1F621',
    shortcodes: ['enraged', 'pout'],
    emoji: '😡',
    order: 90,
    group: 0,
  },
  {
    hexcode: '1F620',
    shortcodes: ['angry'],
    emoji: '😠',
    order: 91,
    group: 0,
  },
  {
    hexcode: '1F92C',
    shortcodes: ['censored'],
    emoji: '🤬',
    order: 92,
    group: 0,
  },
  {
    hexcode: '1F608',
    shortcodes: ['imp'],
    emoji: '😈',
    order: 93,
    group: 0,
  },
  {
    hexcode: '1F47F',
    shortcodes: ['angry_imp'],
    emoji: '👿',
    order: 94,
    group: 0,
  },
  {
    hexcode: '1F480',
    shortcodes: ['skull'],
    emoji: '💀',
    order: 95,
    group: 0,
  },
  {
    hexcode: '2620',
    shortcodes: ['crossbones'],
    emoji: '☠️',
    order: 97,
    group: 0,
  },
  {
    hexcode: '1F4A9',
    shortcodes: ['poop'],
    emoji: '💩',
    order: 98,
    group: 0,
  },
  {
    hexcode: '1F921',
    shortcodes: ['clown'],
    emoji: '🤡',
    order: 99,
    group: 0,
  },
  {
    hexcode: '1F479',
    shortcodes: ['ogre'],
    emoji: '👹',
    order: 100,
    group: 0,
  },
  {
    hexcode: '1F47A',
    shortcodes: ['goblin'],
    emoji: '👺',
    order: 101,
    group: 0,
  },
  {
    hexcode: '1F47B',
    shortcodes: ['ghost'],
    emoji: '👻',
    order: 102,
    group: 0,
  },
  {
    hexcode: '1F47D',
    shortcodes: ['alien'],
    emoji: '👽️',
    order: 103,
    group: 0,
  },
  {
    hexcode: '1F47E',
    shortcodes: ['alien_monster', 'space_invader'],
    emoji: '👾',
    order: 104,
    group: 0,
  },
  {
    hexcode: '1F916',
    shortcodes: ['robot'],
    emoji: '🤖',
    order: 105,
    group: 0,
  },
  {
    hexcode: '1F63A',
    shortcodes: ['smiling_cat'],
    emoji: '😺',
    order: 106,
    group: 0,
  },
  {
    hexcode: '1F638',
    shortcodes: ['grinning_cat'],
    emoji: '😸',
    order: 107,
    group: 0,
  },
  {
    hexcode: '1F639',
    shortcodes: ['joyful_cat'],
    emoji: '😹',
    order: 108,
    group: 0,
  },
  {
    hexcode: '1F63B',
    shortcodes: ['lovestruck_cat'],
    emoji: '😻',
    order: 109,
    group: 0,
  },
  {
    hexcode: '1F63C',
    shortcodes: ['smirking_cat'],
    emoji: '😼',
    order: 110,
    group: 0,
  },
  {
    hexcode: '1F63D',
    shortcodes: ['kissing_cat'],
    emoji: '😽',
    order: 111,
    group: 0,
  },
  {
    hexcode: '1F640',
    shortcodes: ['weary_cat'],
    emoji: '🙀',
    order: 112,
    group: 0,
  },
  {
    hexcode: '1F63F',
    shortcodes: ['crying_cat'],
    emoji: '😿',
    order: 113,
    group: 0,
  },
  {
    hexcode: '1F63E',
    shortcodes: ['pouting_cat'],
    emoji: '😾',
    order: 114,
    group: 0,
  },
  {
    hexcode: '1F648',
    shortcodes: ['see_no_evil'],
    emoji: '🙈',
    order: 115,
    group: 0,
  },
  {
    hexcode: '1F649',
    shortcodes: ['hear_no_evil'],
    emoji: '🙉',
    order: 116,
    group: 0,
  },
  {
    hexcode: '1F64A',
    shortcodes: ['speak_no_evil'],
    emoji: '🙊',
    order: 117,
    group: 0,
  },
  {
    hexcode: '1F48B',
    shortcodes: ['kiss_lips'],
    emoji: '💋',
    order: 118,
    group: 0,
  },
  {
    hexcode: '1F48C',
    shortcodes: ['love_letter'],
    emoji: '💌',
    order: 119,
    group: 0,
  },
  {
    hexcode: '1F498',
    shortcodes: ['cupid'],
    emoji: '💘',
    order: 120,
    group: 0,
  },
  {
    hexcode: '1F49D',
    shortcodes: ['heart_ribbon'],
    emoji: '💝',
    order: 121,
    group: 0,
  },
  {
    hexcode: '1F496',
    shortcodes: ['sparkling_heart'],
    emoji: '💖',
    order: 122,
    group: 0,
  },
  {
    hexcode: '1F497',
    shortcodes: ['heartpulse'],
    emoji: '💗',
    order: 123,
    group: 0,
  },
  {
    hexcode: '1F493',
    shortcodes: ['heartbeat'],
    emoji: '💓',
    order: 124,
    group: 0,
  },
  {
    hexcode: '1F49E',
    shortcodes: ['revolving_hearts'],
    emoji: '💞',
    order: 125,
    group: 0,
  },
  {
    hexcode: '1F495',
    shortcodes: ['two_hearts'],
    emoji: '💕',
    order: 126,
    group: 0,
  },
  {
    hexcode: '1F49F',
    shortcodes: ['heart_decoration'],
    emoji: '💟',
    order: 127,
    group: 0,
  },
  {
    hexcode: '2763',
    shortcodes: ['heart_exclamation'],
    emoji: '❣️',
    order: 129,
    group: 0,
  },
  {
    hexcode: '1F494',
    shortcodes: ['broken_heart'],
    emoji: '💔',
    order: 130,
    group: 0,
  },
  {
    hexcode: '2764',
    shortcodes: ['heart'],
    emoji: '❤️',
    order: 132,
    group: 0,
  },
  {
    hexcode: '1F9E1',
    shortcodes: ['orange_heart'],
    emoji: '🧡',
    order: 133,
    group: 0,
  },
  {
    hexcode: '1F49B',
    shortcodes: ['yellow_heart'],
    emoji: '💛',
    order: 134,
    group: 0,
  },
  {
    hexcode: '1F49A',
    shortcodes: ['green_heart'],
    emoji: '💚',
    order: 135,
    group: 0,
  },
  {
    hexcode: '1F499',
    shortcodes: ['blue_heart'],
    emoji: '💙',
    order: 136,
    group: 0,
  },
  {
    hexcode: '1F49C',
    shortcodes: ['purple_heart'],
    emoji: '💜',
    order: 137,
    group: 0,
  },
  {
    hexcode: '1F90E',
    shortcodes: ['brown_heart'],
    emoji: '🤎',
    order: 138,
    group: 0,
  },
  {
    hexcode: '1F5A4',
    shortcodes: ['black_heart'],
    emoji: '🖤',
    order: 139,
    group: 0,
  },
  {
    hexcode: '1F90D',
    shortcodes: ['white_heart'],
    emoji: '🤍',
    order: 140,
    group: 0,
  },
  {
    hexcode: '1F4AF',
    shortcodes: ['100'],
    emoji: '💯',
    order: 141,
    group: 0,
  },
  {
    hexcode: '1F4A2',
    shortcodes: ['anger'],
    emoji: '💢',
    order: 142,
    group: 0,
  },
  {
    hexcode: '1F4A5',
    shortcodes: ['boom', 'collision'],
    emoji: '💥',
    order: 143,
    group: 0,
  },
  {
    hexcode: '1F4AB',
    shortcodes: ['dizzy_star'],
    emoji: '💫',
    order: 144,
    group: 0,
  },
  {
    hexcode: '1F4A6',
    shortcodes: ['sweat_drops'],
    emoji: '💦',
    order: 145,
    group: 0,
  },
  {
    hexcode: '1F4A8',
    shortcodes: ['dash'],
    emoji: '💨',
    order: 146,
    group: 0,
  },
  {
    hexcode: '1F573',
    shortcodes: ['hole'],
    emoji: '🕳️',
    order: 148,
    group: 0,
  },
  {
    hexcode: '1F4A3',
    shortcodes: ['bomb'],
    emoji: '💣️',
    order: 149,
    group: 0,
  },
  {
    hexcode: '1F4AC',
    shortcodes: ['speech'],
    emoji: '💬',
    order: 150,
    group: 0,
  },
  {
    hexcode: '1F441-FE0F-200D-1F5E8-FE0F',
    shortcodes: ['eye_bubble'],
    emoji: '👁️‍🗨️',
    order: 151,
    group: 0,
  },
  {
    hexcode: '1F5E8',
    shortcodes: ['left_speech'],
    emoji: '🗨️',
    order: 156,
    group: 0,
  },
  {
    hexcode: '1F5EF',
    shortcodes: ['right_anger_speech'],
    emoji: '🗯️',
    order: 158,
    group: 0,
  },
  {
    hexcode: '1F4AD',
    shortcodes: ['thought'],
    emoji: '💭',
    order: 159,
    group: 0,
  },
  {
    hexcode: '1F4A4',
    shortcodes: ['zzz'],
    emoji: '💤',
    order: 160,
    group: 0,
  },
  {
    hexcode: '1F44B',
    shortcodes: ['wave'],
    emoji: '👋',
    order: 161,
    group: 1,
    skins: [
      {
        hexcode: '1F44B-1F3FB',
        emoji: '👋🏻',
        tone: 1,
      },
      {
        hexcode: '1F44B-1F3FC',
        emoji: '👋🏼',
        tone: 2,
      },
      {
        hexcode: '1F44B-1F3FD',
        emoji: '👋🏽',
        tone: 3,
      },
      {
        hexcode: '1F44B-1F3FE',
        emoji: '👋🏾',
        tone: 4,
      },
      {
        hexcode: '1F44B-1F3FF',
        emoji: '👋🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91A',
    shortcodes: ['raised_backhand'],
    emoji: '🤚',
    order: 167,
    group: 1,
    skins: [
      {
        hexcode: '1F91A-1F3FB',
        emoji: '🤚🏻',
        tone: 1,
      },
      {
        hexcode: '1F91A-1F3FC',
        emoji: '🤚🏼',
        tone: 2,
      },
      {
        hexcode: '1F91A-1F3FD',
        emoji: '🤚🏽',
        tone: 3,
      },
      {
        hexcode: '1F91A-1F3FE',
        emoji: '🤚🏾',
        tone: 4,
      },
      {
        hexcode: '1F91A-1F3FF',
        emoji: '🤚🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F590',
    shortcodes: ['splayed_hand'],
    emoji: '🖐️',
    order: 174,
    group: 1,
    skins: [
      {
        hexcode: '1F590-1F3FB',
        emoji: '🖐🏻',
        tone: 1,
      },
      {
        hexcode: '1F590-1F3FC',
        emoji: '🖐🏼',
        tone: 2,
      },
      {
        hexcode: '1F590-1F3FD',
        emoji: '🖐🏽',
        tone: 3,
      },
      {
        hexcode: '1F590-1F3FE',
        emoji: '🖐🏾',
        tone: 4,
      },
      {
        hexcode: '1F590-1F3FF',
        emoji: '🖐🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '270B',
    shortcodes: ['raised_hand'],
    emoji: '✋',
    order: 180,
    group: 1,
    skins: [
      {
        hexcode: '270B-1F3FB',
        emoji: '✋🏻',
        tone: 1,
      },
      {
        hexcode: '270B-1F3FC',
        emoji: '✋🏼',
        tone: 2,
      },
      {
        hexcode: '270B-1F3FD',
        emoji: '✋🏽',
        tone: 3,
      },
      {
        hexcode: '270B-1F3FE',
        emoji: '✋🏾',
        tone: 4,
      },
      {
        hexcode: '270B-1F3FF',
        emoji: '✋🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F596',
    shortcodes: ['vulcan'],
    emoji: '🖖',
    order: 186,
    group: 1,
    skins: [
      {
        hexcode: '1F596-1F3FB',
        emoji: '🖖🏻',
        tone: 1,
      },
      {
        hexcode: '1F596-1F3FC',
        emoji: '🖖🏼',
        tone: 2,
      },
      {
        hexcode: '1F596-1F3FD',
        emoji: '🖖🏽',
        tone: 3,
      },
      {
        hexcode: '1F596-1F3FE',
        emoji: '🖖🏾',
        tone: 4,
      },
      {
        hexcode: '1F596-1F3FF',
        emoji: '🖖🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F44C',
    shortcodes: ['ok_hand'],
    emoji: '👌',
    order: 192,
    group: 1,
    skins: [
      {
        hexcode: '1F44C-1F3FB',
        emoji: '👌🏻',
        tone: 1,
      },
      {
        hexcode: '1F44C-1F3FC',
        emoji: '👌🏼',
        tone: 2,
      },
      {
        hexcode: '1F44C-1F3FD',
        emoji: '👌🏽',
        tone: 3,
      },
      {
        hexcode: '1F44C-1F3FE',
        emoji: '👌🏾',
        tone: 4,
      },
      {
        hexcode: '1F44C-1F3FF',
        emoji: '👌🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F90F',
    shortcodes: ['pinch'],
    emoji: '🤏',
    order: 198,
    group: 1,
    skins: [
      {
        hexcode: '1F90F-1F3FB',
        emoji: '🤏🏻',
        tone: 1,
      },
      {
        hexcode: '1F90F-1F3FC',
        emoji: '🤏🏼',
        tone: 2,
      },
      {
        hexcode: '1F90F-1F3FD',
        emoji: '🤏🏽',
        tone: 3,
      },
      {
        hexcode: '1F90F-1F3FE',
        emoji: '🤏🏾',
        tone: 4,
      },
      {
        hexcode: '1F90F-1F3FF',
        emoji: '🤏🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '270C',
    shortcodes: ['victory'],
    emoji: '✌️',
    order: 205,
    group: 1,
    skins: [
      {
        hexcode: '270C-1F3FB',
        emoji: '✌🏻',
        tone: 1,
      },
      {
        hexcode: '270C-1F3FC',
        emoji: '✌🏼',
        tone: 2,
      },
      {
        hexcode: '270C-1F3FD',
        emoji: '✌🏽',
        tone: 3,
      },
      {
        hexcode: '270C-1F3FE',
        emoji: '✌🏾',
        tone: 4,
      },
      {
        hexcode: '270C-1F3FF',
        emoji: '✌🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91E',
    shortcodes: ['fingers_crossed'],
    emoji: '🤞',
    order: 211,
    group: 1,
    skins: [
      {
        hexcode: '1F91E-1F3FB',
        emoji: '🤞🏻',
        tone: 1,
      },
      {
        hexcode: '1F91E-1F3FC',
        emoji: '🤞🏼',
        tone: 2,
      },
      {
        hexcode: '1F91E-1F3FD',
        emoji: '🤞🏽',
        tone: 3,
      },
      {
        hexcode: '1F91E-1F3FE',
        emoji: '🤞🏾',
        tone: 4,
      },
      {
        hexcode: '1F91E-1F3FF',
        emoji: '🤞🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91F',
    shortcodes: ['love_you_gesture'],
    emoji: '🤟',
    order: 217,
    group: 1,
    skins: [
      {
        hexcode: '1F91F-1F3FB',
        emoji: '🤟🏻',
        tone: 1,
      },
      {
        hexcode: '1F91F-1F3FC',
        emoji: '🤟🏼',
        tone: 2,
      },
      {
        hexcode: '1F91F-1F3FD',
        emoji: '🤟🏽',
        tone: 3,
      },
      {
        hexcode: '1F91F-1F3FE',
        emoji: '🤟🏾',
        tone: 4,
      },
      {
        hexcode: '1F91F-1F3FF',
        emoji: '🤟🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F918',
    shortcodes: ['metal'],
    emoji: '🤘',
    order: 223,
    group: 1,
    skins: [
      {
        hexcode: '1F918-1F3FB',
        emoji: '🤘🏻',
        tone: 1,
      },
      {
        hexcode: '1F918-1F3FC',
        emoji: '🤘🏼',
        tone: 2,
      },
      {
        hexcode: '1F918-1F3FD',
        emoji: '🤘🏽',
        tone: 3,
      },
      {
        hexcode: '1F918-1F3FE',
        emoji: '🤘🏾',
        tone: 4,
      },
      {
        hexcode: '1F918-1F3FF',
        emoji: '🤘🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F919',
    shortcodes: ['call_me'],
    emoji: '🤙',
    order: 229,
    group: 1,
    skins: [
      {
        hexcode: '1F919-1F3FB',
        emoji: '🤙🏻',
        tone: 1,
      },
      {
        hexcode: '1F919-1F3FC',
        emoji: '🤙🏼',
        tone: 2,
      },
      {
        hexcode: '1F919-1F3FD',
        emoji: '🤙🏽',
        tone: 3,
      },
      {
        hexcode: '1F919-1F3FE',
        emoji: '🤙🏾',
        tone: 4,
      },
      {
        hexcode: '1F919-1F3FF',
        emoji: '🤙🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F448',
    shortcodes: ['point_left'],
    emoji: '👈️',
    order: 235,
    group: 1,
    skins: [
      {
        hexcode: '1F448-1F3FB',
        emoji: '👈🏻',
        tone: 1,
      },
      {
        hexcode: '1F448-1F3FC',
        emoji: '👈🏼',
        tone: 2,
      },
      {
        hexcode: '1F448-1F3FD',
        emoji: '👈🏽',
        tone: 3,
      },
      {
        hexcode: '1F448-1F3FE',
        emoji: '👈🏾',
        tone: 4,
      },
      {
        hexcode: '1F448-1F3FF',
        emoji: '👈🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F449',
    shortcodes: ['point_right'],
    emoji: '👉️',
    order: 241,
    group: 1,
    skins: [
      {
        hexcode: '1F449-1F3FB',
        emoji: '👉🏻',
        tone: 1,
      },
      {
        hexcode: '1F449-1F3FC',
        emoji: '👉🏼',
        tone: 2,
      },
      {
        hexcode: '1F449-1F3FD',
        emoji: '👉🏽',
        tone: 3,
      },
      {
        hexcode: '1F449-1F3FE',
        emoji: '👉🏾',
        tone: 4,
      },
      {
        hexcode: '1F449-1F3FF',
        emoji: '👉🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F446',
    shortcodes: ['backhand_point_up'],
    emoji: '👆️',
    order: 247,
    group: 1,
    skins: [
      {
        hexcode: '1F446-1F3FB',
        emoji: '👆🏻',
        tone: 1,
      },
      {
        hexcode: '1F446-1F3FC',
        emoji: '👆🏼',
        tone: 2,
      },
      {
        hexcode: '1F446-1F3FD',
        emoji: '👆🏽',
        tone: 3,
      },
      {
        hexcode: '1F446-1F3FE',
        emoji: '👆🏾',
        tone: 4,
      },
      {
        hexcode: '1F446-1F3FF',
        emoji: '👆🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F595',
    shortcodes: ['middle_finger'],
    emoji: '🖕',
    order: 253,
    group: 1,
    skins: [
      {
        hexcode: '1F595-1F3FB',
        emoji: '🖕🏻',
        tone: 1,
      },
      {
        hexcode: '1F595-1F3FC',
        emoji: '🖕🏼',
        tone: 2,
      },
      {
        hexcode: '1F595-1F3FD',
        emoji: '🖕🏽',
        tone: 3,
      },
      {
        hexcode: '1F595-1F3FE',
        emoji: '🖕🏾',
        tone: 4,
      },
      {
        hexcode: '1F595-1F3FF',
        emoji: '🖕🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F447',
    shortcodes: ['point_down'],
    emoji: '👇️',
    order: 259,
    group: 1,
    skins: [
      {
        hexcode: '1F447-1F3FB',
        emoji: '👇🏻',
        tone: 1,
      },
      {
        hexcode: '1F447-1F3FC',
        emoji: '👇🏼',
        tone: 2,
      },
      {
        hexcode: '1F447-1F3FD',
        emoji: '👇🏽',
        tone: 3,
      },
      {
        hexcode: '1F447-1F3FE',
        emoji: '👇🏾',
        tone: 4,
      },
      {
        hexcode: '1F447-1F3FF',
        emoji: '👇🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '261D',
    shortcodes: ['point_up'],
    emoji: '☝️',
    order: 266,
    group: 1,
    skins: [
      {
        hexcode: '261D-1F3FB',
        emoji: '☝🏻',
        tone: 1,
      },
      {
        hexcode: '261D-1F3FC',
        emoji: '☝🏼',
        tone: 2,
      },
      {
        hexcode: '261D-1F3FD',
        emoji: '☝🏽',
        tone: 3,
      },
      {
        hexcode: '261D-1F3FE',
        emoji: '☝🏾',
        tone: 4,
      },
      {
        hexcode: '261D-1F3FF',
        emoji: '☝🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F44D',
    shortcodes: ['thumbsup', '+1', 'yes'],
    emoji: '👍️',
    order: 272,
    group: 1,
    skins: [
      {
        hexcode: '1F44D-1F3FB',
        emoji: '👍🏻',
        tone: 1,
      },
      {
        hexcode: '1F44D-1F3FC',
        emoji: '👍🏼',
        tone: 2,
      },
      {
        hexcode: '1F44D-1F3FD',
        emoji: '👍🏽',
        tone: 3,
      },
      {
        hexcode: '1F44D-1F3FE',
        emoji: '👍🏾',
        tone: 4,
      },
      {
        hexcode: '1F44D-1F3FF',
        emoji: '👍🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F44E',
    shortcodes: ['thumbsdown', '-1', 'no'],
    emoji: '👎️',
    order: 278,
    group: 1,
    skins: [
      {
        hexcode: '1F44E-1F3FB',
        emoji: '👎🏻',
        tone: 1,
      },
      {
        hexcode: '1F44E-1F3FC',
        emoji: '👎🏼',
        tone: 2,
      },
      {
        hexcode: '1F44E-1F3FD',
        emoji: '👎🏽',
        tone: 3,
      },
      {
        hexcode: '1F44E-1F3FE',
        emoji: '👎🏾',
        tone: 4,
      },
      {
        hexcode: '1F44E-1F3FF',
        emoji: '👎🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '270A',
    shortcodes: ['fist'],
    emoji: '✊',
    order: 284,
    group: 1,
    skins: [
      {
        hexcode: '270A-1F3FB',
        emoji: '✊🏻',
        tone: 1,
      },
      {
        hexcode: '270A-1F3FC',
        emoji: '✊🏼',
        tone: 2,
      },
      {
        hexcode: '270A-1F3FD',
        emoji: '✊🏽',
        tone: 3,
      },
      {
        hexcode: '270A-1F3FE',
        emoji: '✊🏾',
        tone: 4,
      },
      {
        hexcode: '270A-1F3FF',
        emoji: '✊🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F44A',
    shortcodes: ['punch'],
    emoji: '👊',
    order: 290,
    group: 1,
    skins: [
      {
        hexcode: '1F44A-1F3FB',
        emoji: '👊🏻',
        tone: 1,
      },
      {
        hexcode: '1F44A-1F3FC',
        emoji: '👊🏼',
        tone: 2,
      },
      {
        hexcode: '1F44A-1F3FD',
        emoji: '👊🏽',
        tone: 3,
      },
      {
        hexcode: '1F44A-1F3FE',
        emoji: '👊🏾',
        tone: 4,
      },
      {
        hexcode: '1F44A-1F3FF',
        emoji: '👊🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91B',
    shortcodes: ['left_facing_fist'],
    emoji: '🤛',
    order: 296,
    group: 1,
    skins: [
      {
        hexcode: '1F91B-1F3FB',
        emoji: '🤛🏻',
        tone: 1,
      },
      {
        hexcode: '1F91B-1F3FC',
        emoji: '🤛🏼',
        tone: 2,
      },
      {
        hexcode: '1F91B-1F3FD',
        emoji: '🤛🏽',
        tone: 3,
      },
      {
        hexcode: '1F91B-1F3FE',
        emoji: '🤛🏾',
        tone: 4,
      },
      {
        hexcode: '1F91B-1F3FF',
        emoji: '🤛🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91C',
    shortcodes: ['right_facing_fist'],
    emoji: '🤜',
    order: 302,
    group: 1,
    skins: [
      {
        hexcode: '1F91C-1F3FB',
        emoji: '🤜🏻',
        tone: 1,
      },
      {
        hexcode: '1F91C-1F3FC',
        emoji: '🤜🏼',
        tone: 2,
      },
      {
        hexcode: '1F91C-1F3FD',
        emoji: '🤜🏽',
        tone: 3,
      },
      {
        hexcode: '1F91C-1F3FE',
        emoji: '🤜🏾',
        tone: 4,
      },
      {
        hexcode: '1F91C-1F3FF',
        emoji: '🤜🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F44F',
    shortcodes: ['clap'],
    emoji: '👏',
    order: 308,
    group: 1,
    skins: [
      {
        hexcode: '1F44F-1F3FB',
        emoji: '👏🏻',
        tone: 1,
      },
      {
        hexcode: '1F44F-1F3FC',
        emoji: '👏🏼',
        tone: 2,
      },
      {
        hexcode: '1F44F-1F3FD',
        emoji: '👏🏽',
        tone: 3,
      },
      {
        hexcode: '1F44F-1F3FE',
        emoji: '👏🏾',
        tone: 4,
      },
      {
        hexcode: '1F44F-1F3FF',
        emoji: '👏🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64C',
    shortcodes: ['raised_hands'],
    emoji: '🙌',
    order: 314,
    group: 1,
    skins: [
      {
        hexcode: '1F64C-1F3FB',
        emoji: '🙌🏻',
        tone: 1,
      },
      {
        hexcode: '1F64C-1F3FC',
        emoji: '🙌🏼',
        tone: 2,
      },
      {
        hexcode: '1F64C-1F3FD',
        emoji: '🙌🏽',
        tone: 3,
      },
      {
        hexcode: '1F64C-1F3FE',
        emoji: '🙌🏾',
        tone: 4,
      },
      {
        hexcode: '1F64C-1F3FF',
        emoji: '🙌🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F450',
    shortcodes: ['open_hands'],
    emoji: '👐',
    order: 320,
    group: 1,
    skins: [
      {
        hexcode: '1F450-1F3FB',
        emoji: '👐🏻',
        tone: 1,
      },
      {
        hexcode: '1F450-1F3FC',
        emoji: '👐🏼',
        tone: 2,
      },
      {
        hexcode: '1F450-1F3FD',
        emoji: '👐🏽',
        tone: 3,
      },
      {
        hexcode: '1F450-1F3FE',
        emoji: '👐🏾',
        tone: 4,
      },
      {
        hexcode: '1F450-1F3FF',
        emoji: '👐🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F932',
    shortcodes: ['palms_up'],
    emoji: '🤲',
    order: 326,
    group: 1,
    skins: [
      {
        hexcode: '1F932-1F3FB',
        emoji: '🤲🏻',
        tone: 1,
      },
      {
        hexcode: '1F932-1F3FC',
        emoji: '🤲🏼',
        tone: 2,
      },
      {
        hexcode: '1F932-1F3FD',
        emoji: '🤲🏽',
        tone: 3,
      },
      {
        hexcode: '1F932-1F3FE',
        emoji: '🤲🏾',
        tone: 4,
      },
      {
        hexcode: '1F932-1F3FF',
        emoji: '🤲🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F91D',
    shortcodes: ['handshake'],
    emoji: '🤝',
    order: 332,
    group: 1,
  },
  {
    hexcode: '1F64F',
    shortcodes: ['pray'],
    emoji: '🙏',
    order: 333,
    group: 1,
    skins: [
      {
        hexcode: '1F64F-1F3FB',
        emoji: '🙏🏻',
        tone: 1,
      },
      {
        hexcode: '1F64F-1F3FC',
        emoji: '🙏🏼',
        tone: 2,
      },
      {
        hexcode: '1F64F-1F3FD',
        emoji: '🙏🏽',
        tone: 3,
      },
      {
        hexcode: '1F64F-1F3FE',
        emoji: '🙏🏾',
        tone: 4,
      },
      {
        hexcode: '1F64F-1F3FF',
        emoji: '🙏🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '270D',
    shortcodes: ['writing_hand'],
    emoji: '✍️',
    order: 340,
    group: 1,
    skins: [
      {
        hexcode: '270D-1F3FB',
        emoji: '✍🏻',
        tone: 1,
      },
      {
        hexcode: '270D-1F3FC',
        emoji: '✍🏼',
        tone: 2,
      },
      {
        hexcode: '270D-1F3FD',
        emoji: '✍🏽',
        tone: 3,
      },
      {
        hexcode: '270D-1F3FE',
        emoji: '✍🏾',
        tone: 4,
      },
      {
        hexcode: '270D-1F3FF',
        emoji: '✍🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F485',
    shortcodes: ['nail_care'],
    emoji: '💅',
    order: 346,
    group: 1,
    skins: [
      {
        hexcode: '1F485-1F3FB',
        emoji: '💅🏻',
        tone: 1,
      },
      {
        hexcode: '1F485-1F3FC',
        emoji: '💅🏼',
        tone: 2,
      },
      {
        hexcode: '1F485-1F3FD',
        emoji: '💅🏽',
        tone: 3,
      },
      {
        hexcode: '1F485-1F3FE',
        emoji: '💅🏾',
        tone: 4,
      },
      {
        hexcode: '1F485-1F3FF',
        emoji: '💅🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F933',
    shortcodes: ['selfie'],
    emoji: '🤳',
    order: 352,
    group: 1,
    skins: [
      {
        hexcode: '1F933-1F3FB',
        emoji: '🤳🏻',
        tone: 1,
      },
      {
        hexcode: '1F933-1F3FC',
        emoji: '🤳🏼',
        tone: 2,
      },
      {
        hexcode: '1F933-1F3FD',
        emoji: '🤳🏽',
        tone: 3,
      },
      {
        hexcode: '1F933-1F3FE',
        emoji: '🤳🏾',
        tone: 4,
      },
      {
        hexcode: '1F933-1F3FF',
        emoji: '🤳🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F4AA',
    shortcodes: ['muscle', 'right_bicep'],
    emoji: '💪',
    order: 358,
    group: 1,
    skins: [
      {
        hexcode: '1F4AA-1F3FB',
        emoji: '💪🏻',
        tone: 1,
      },
      {
        hexcode: '1F4AA-1F3FC',
        emoji: '💪🏼',
        tone: 2,
      },
      {
        hexcode: '1F4AA-1F3FD',
        emoji: '💪🏽',
        tone: 3,
      },
      {
        hexcode: '1F4AA-1F3FE',
        emoji: '💪🏾',
        tone: 4,
      },
      {
        hexcode: '1F4AA-1F3FF',
        emoji: '💪🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9BE',
    shortcodes: ['mech_arm'],
    emoji: '🦾',
    order: 364,
    group: 1,
  },
  {
    hexcode: '1F9BF',
    shortcodes: ['mech_leg'],
    emoji: '🦿',
    order: 365,
    group: 1,
  },
  {
    hexcode: '1F9B5',
    shortcodes: ['leg'],
    emoji: '🦵',
    order: 366,
    group: 1,
    skins: [
      {
        hexcode: '1F9B5-1F3FB',
        emoji: '🦵🏻',
        tone: 1,
      },
      {
        hexcode: '1F9B5-1F3FC',
        emoji: '🦵🏼',
        tone: 2,
      },
      {
        hexcode: '1F9B5-1F3FD',
        emoji: '🦵🏽',
        tone: 3,
      },
      {
        hexcode: '1F9B5-1F3FE',
        emoji: '🦵🏾',
        tone: 4,
      },
      {
        hexcode: '1F9B5-1F3FF',
        emoji: '🦵🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B6',
    shortcodes: ['foot'],
    emoji: '🦶',
    order: 372,
    group: 1,
    skins: [
      {
        hexcode: '1F9B6-1F3FB',
        emoji: '🦶🏻',
        tone: 1,
      },
      {
        hexcode: '1F9B6-1F3FC',
        emoji: '🦶🏼',
        tone: 2,
      },
      {
        hexcode: '1F9B6-1F3FD',
        emoji: '🦶🏽',
        tone: 3,
      },
      {
        hexcode: '1F9B6-1F3FE',
        emoji: '🦶🏾',
        tone: 4,
      },
      {
        hexcode: '1F9B6-1F3FF',
        emoji: '🦶🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F442',
    shortcodes: ['ear'],
    emoji: '👂️',
    order: 378,
    group: 1,
    skins: [
      {
        hexcode: '1F442-1F3FB',
        emoji: '👂🏻',
        tone: 1,
      },
      {
        hexcode: '1F442-1F3FC',
        emoji: '👂🏼',
        tone: 2,
      },
      {
        hexcode: '1F442-1F3FD',
        emoji: '👂🏽',
        tone: 3,
      },
      {
        hexcode: '1F442-1F3FE',
        emoji: '👂🏾',
        tone: 4,
      },
      {
        hexcode: '1F442-1F3FF',
        emoji: '👂🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9BB',
    shortcodes: ['hearing_aid'],
    emoji: '🦻',
    order: 384,
    group: 1,
    skins: [
      {
        hexcode: '1F9BB-1F3FB',
        emoji: '🦻🏻',
        tone: 1,
      },
      {
        hexcode: '1F9BB-1F3FC',
        emoji: '🦻🏼',
        tone: 2,
      },
      {
        hexcode: '1F9BB-1F3FD',
        emoji: '🦻🏽',
        tone: 3,
      },
      {
        hexcode: '1F9BB-1F3FE',
        emoji: '🦻🏾',
        tone: 4,
      },
      {
        hexcode: '1F9BB-1F3FF',
        emoji: '🦻🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F443',
    shortcodes: ['nose'],
    emoji: '👃',
    order: 390,
    group: 1,
    skins: [
      {
        hexcode: '1F443-1F3FB',
        emoji: '👃🏻',
        tone: 1,
      },
      {
        hexcode: '1F443-1F3FC',
        emoji: '👃🏼',
        tone: 2,
      },
      {
        hexcode: '1F443-1F3FD',
        emoji: '👃🏽',
        tone: 3,
      },
      {
        hexcode: '1F443-1F3FE',
        emoji: '👃🏾',
        tone: 4,
      },
      {
        hexcode: '1F443-1F3FF',
        emoji: '👃🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9E0',
    shortcodes: ['brain'],
    emoji: '🧠',
    order: 396,
    group: 1,
  },
  {
    hexcode: '1F9B7',
    shortcodes: ['tooth'],
    emoji: '🦷',
    order: 397,
    group: 1,
  },
  {
    hexcode: '1F9B4',
    shortcodes: ['bone'],
    emoji: '🦴',
    order: 398,
    group: 1,
  },
  {
    hexcode: '1F440',
    shortcodes: ['eyes'],
    emoji: '👀',
    order: 399,
    group: 1,
  },
  {
    hexcode: '1F441',
    shortcodes: ['eye'],
    emoji: '👁️',
    order: 401,
    group: 1,
  },
  {
    hexcode: '1F445',
    shortcodes: ['tongue'],
    emoji: '👅',
    order: 402,
    group: 1,
  },
  {
    hexcode: '1F444',
    shortcodes: ['lips'],
    emoji: '👄',
    order: 403,
    group: 1,
  },
  {
    hexcode: '1F476',
    shortcodes: ['baby'],
    emoji: '👶',
    order: 404,
    group: 1,
    skins: [
      {
        hexcode: '1F476-1F3FB',
        emoji: '👶🏻',
        tone: 1,
      },
      {
        hexcode: '1F476-1F3FC',
        emoji: '👶🏼',
        tone: 2,
      },
      {
        hexcode: '1F476-1F3FD',
        emoji: '👶🏽',
        tone: 3,
      },
      {
        hexcode: '1F476-1F3FE',
        emoji: '👶🏾',
        tone: 4,
      },
      {
        hexcode: '1F476-1F3FF',
        emoji: '👶🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D2',
    shortcodes: ['child'],
    emoji: '🧒',
    order: 410,
    group: 1,
    skins: [
      {
        hexcode: '1F9D2-1F3FB',
        emoji: '🧒🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D2-1F3FC',
        emoji: '🧒🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D2-1F3FD',
        emoji: '🧒🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D2-1F3FE',
        emoji: '🧒🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D2-1F3FF',
        emoji: '🧒🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F466',
    shortcodes: ['boy'],
    emoji: '👦',
    order: 416,
    group: 1,
    skins: [
      {
        hexcode: '1F466-1F3FB',
        emoji: '👦🏻',
        tone: 1,
      },
      {
        hexcode: '1F466-1F3FC',
        emoji: '👦🏼',
        tone: 2,
      },
      {
        hexcode: '1F466-1F3FD',
        emoji: '👦🏽',
        tone: 3,
      },
      {
        hexcode: '1F466-1F3FE',
        emoji: '👦🏾',
        tone: 4,
      },
      {
        hexcode: '1F466-1F3FF',
        emoji: '👦🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F467',
    shortcodes: ['girl'],
    emoji: '👧',
    order: 422,
    group: 1,
    skins: [
      {
        hexcode: '1F467-1F3FB',
        emoji: '👧🏻',
        tone: 1,
      },
      {
        hexcode: '1F467-1F3FC',
        emoji: '👧🏼',
        tone: 2,
      },
      {
        hexcode: '1F467-1F3FD',
        emoji: '👧🏽',
        tone: 3,
      },
      {
        hexcode: '1F467-1F3FE',
        emoji: '👧🏾',
        tone: 4,
      },
      {
        hexcode: '1F467-1F3FF',
        emoji: '👧🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D1',
    shortcodes: ['adult'],
    emoji: '🧑',
    order: 428,
    group: 1,
    skins: [
      {
        hexcode: '1F9D1-1F3FB',
        emoji: '🧑🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D1-1F3FC',
        emoji: '🧑🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D1-1F3FD',
        emoji: '🧑🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D1-1F3FE',
        emoji: '🧑🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D1-1F3FF',
        emoji: '🧑🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F471',
    shortcodes: ['blond_person'],
    emoji: '👱',
    order: 434,
    group: 1,
    skins: [
      {
        hexcode: '1F471-1F3FB',
        emoji: '👱🏻',
        tone: 1,
      },
      {
        hexcode: '1F471-1F3FC',
        emoji: '👱🏼',
        tone: 2,
      },
      {
        hexcode: '1F471-1F3FD',
        emoji: '👱🏽',
        tone: 3,
      },
      {
        hexcode: '1F471-1F3FE',
        emoji: '👱🏾',
        tone: 4,
      },
      {
        hexcode: '1F471-1F3FF',
        emoji: '👱🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468',
    shortcodes: ['man'],
    emoji: '👨',
    order: 440,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB',
        emoji: '👨🏻',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC',
        emoji: '👨🏼',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD',
        emoji: '👨🏽',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE',
        emoji: '👨🏾',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF',
        emoji: '👨🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D4',
    shortcodes: ['bearded_person'],
    emoji: '🧔',
    order: 446,
    group: 1,
    skins: [
      {
        hexcode: '1F9D4-1F3FB',
        emoji: '🧔🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D4-1F3FC',
        emoji: '🧔🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D4-1F3FD',
        emoji: '🧔🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D4-1F3FE',
        emoji: '🧔🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D4-1F3FF',
        emoji: '🧔🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F471-200D-2642-FE0F',
    shortcodes: ['blond_man'],
    emoji: '👱‍♂️',
    order: 452,
    group: 1,
    skins: [
      {
        hexcode: '1F471-1F3FB-200D-2642-FE0F',
        emoji: '👱🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F471-1F3FC-200D-2642-FE0F',
        emoji: '👱🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F471-1F3FD-200D-2642-FE0F',
        emoji: '👱🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F471-1F3FE-200D-2642-FE0F',
        emoji: '👱🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F471-1F3FF-200D-2642-FE0F',
        emoji: '👱🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9B0',
    shortcodes: ['red_haired_man'],
    emoji: '👨‍🦰',
    order: 464,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9B0',
        emoji: '👨🏻‍🦰',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9B0',
        emoji: '👨🏼‍🦰',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9B0',
        emoji: '👨🏽‍🦰',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9B0',
        emoji: '👨🏾‍🦰',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9B0',
        emoji: '👨🏿‍🦰',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9B1',
    shortcodes: ['curly_haired_man'],
    emoji: '👨‍🦱',
    order: 470,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9B1',
        emoji: '👨🏻‍🦱',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9B1',
        emoji: '👨🏼‍🦱',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9B1',
        emoji: '👨🏽‍🦱',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9B1',
        emoji: '👨🏾‍🦱',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9B1',
        emoji: '👨🏿‍🦱',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9B3',
    shortcodes: ['white_haired_man'],
    emoji: '👨‍🦳',
    order: 476,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9B3',
        emoji: '👨🏻‍🦳',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9B3',
        emoji: '👨🏼‍🦳',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9B3',
        emoji: '👨🏽‍🦳',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9B3',
        emoji: '👨🏾‍🦳',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9B3',
        emoji: '👨🏿‍🦳',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9B2',
    shortcodes: ['bald_man'],
    emoji: '👨‍🦲',
    order: 482,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9B2',
        emoji: '👨🏻‍🦲',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9B2',
        emoji: '👨🏼‍🦲',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9B2',
        emoji: '👨🏽‍🦲',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9B2',
        emoji: '👨🏾‍🦲',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9B2',
        emoji: '👨🏿‍🦲',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469',
    shortcodes: ['woman'],
    emoji: '👩',
    order: 488,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB',
        emoji: '👩🏻',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC',
        emoji: '👩🏼',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD',
        emoji: '👩🏽',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE',
        emoji: '👩🏾',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF',
        emoji: '👩🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F471-200D-2640-FE0F',
    shortcodes: ['blond_woman'],
    emoji: '👱‍♀️',
    order: 494,
    group: 1,
    skins: [
      {
        hexcode: '1F471-1F3FB-200D-2640-FE0F',
        emoji: '👱🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F471-1F3FC-200D-2640-FE0F',
        emoji: '👱🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F471-1F3FD-200D-2640-FE0F',
        emoji: '👱🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F471-1F3FE-200D-2640-FE0F',
        emoji: '👱🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F471-1F3FF-200D-2640-FE0F',
        emoji: '👱🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9B0',
    shortcodes: ['red_haired_woman'],
    emoji: '👩‍🦰',
    order: 506,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9B0',
        emoji: '👩🏻‍🦰',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9B0',
        emoji: '👩🏼‍🦰',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9B0',
        emoji: '👩🏽‍🦰',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9B0',
        emoji: '👩🏾‍🦰',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9B0',
        emoji: '👩🏿‍🦰',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9B1',
    shortcodes: ['curly_haired_woman'],
    emoji: '👩‍🦱',
    order: 512,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9B1',
        emoji: '👩🏻‍🦱',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9B1',
        emoji: '👩🏼‍🦱',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9B1',
        emoji: '👩🏽‍🦱',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9B1',
        emoji: '👩🏾‍🦱',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9B1',
        emoji: '👩🏿‍🦱',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9B3',
    shortcodes: ['white_haired_woman'],
    emoji: '👩‍🦳',
    order: 518,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9B3',
        emoji: '👩🏻‍🦳',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9B3',
        emoji: '👩🏼‍🦳',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9B3',
        emoji: '👩🏽‍🦳',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9B3',
        emoji: '👩🏾‍🦳',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9B3',
        emoji: '👩🏿‍🦳',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9B2',
    shortcodes: ['bald_woman'],
    emoji: '👩‍🦲',
    order: 524,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9B2',
        emoji: '👩🏻‍🦲',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9B2',
        emoji: '👩🏼‍🦲',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9B2',
        emoji: '👩🏽‍🦲',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9B2',
        emoji: '👩🏾‍🦲',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9B2',
        emoji: '👩🏿‍🦲',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D3',
    shortcodes: ['older_adult'],
    emoji: '🧓',
    order: 530,
    group: 1,
    skins: [
      {
        hexcode: '1F9D3-1F3FB',
        emoji: '🧓🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D3-1F3FC',
        emoji: '🧓🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D3-1F3FD',
        emoji: '🧓🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D3-1F3FE',
        emoji: '🧓🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D3-1F3FF',
        emoji: '🧓🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F474',
    shortcodes: ['older_man'],
    emoji: '👴',
    order: 536,
    group: 1,
    skins: [
      {
        hexcode: '1F474-1F3FB',
        emoji: '👴🏻',
        tone: 1,
      },
      {
        hexcode: '1F474-1F3FC',
        emoji: '👴🏼',
        tone: 2,
      },
      {
        hexcode: '1F474-1F3FD',
        emoji: '👴🏽',
        tone: 3,
      },
      {
        hexcode: '1F474-1F3FE',
        emoji: '👴🏾',
        tone: 4,
      },
      {
        hexcode: '1F474-1F3FF',
        emoji: '👴🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F475',
    shortcodes: ['older_woman'],
    emoji: '👵',
    order: 542,
    group: 1,
    skins: [
      {
        hexcode: '1F475-1F3FB',
        emoji: '👵🏻',
        tone: 1,
      },
      {
        hexcode: '1F475-1F3FC',
        emoji: '👵🏼',
        tone: 2,
      },
      {
        hexcode: '1F475-1F3FD',
        emoji: '👵🏽',
        tone: 3,
      },
      {
        hexcode: '1F475-1F3FE',
        emoji: '👵🏾',
        tone: 4,
      },
      {
        hexcode: '1F475-1F3FF',
        emoji: '👵🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64D',
    shortcodes: ['person_frowning'],
    emoji: '🙍',
    order: 548,
    group: 1,
    skins: [
      {
        hexcode: '1F64D-1F3FB',
        emoji: '🙍🏻',
        tone: 1,
      },
      {
        hexcode: '1F64D-1F3FC',
        emoji: '🙍🏼',
        tone: 2,
      },
      {
        hexcode: '1F64D-1F3FD',
        emoji: '🙍🏽',
        tone: 3,
      },
      {
        hexcode: '1F64D-1F3FE',
        emoji: '🙍🏾',
        tone: 4,
      },
      {
        hexcode: '1F64D-1F3FF',
        emoji: '🙍🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64D-200D-2642-FE0F',
    shortcodes: ['man_frowning'],
    emoji: '🙍‍♂️',
    order: 554,
    group: 1,
    skins: [
      {
        hexcode: '1F64D-1F3FB-200D-2642-FE0F',
        emoji: '🙍🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F64D-1F3FC-200D-2642-FE0F',
        emoji: '🙍🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F64D-1F3FD-200D-2642-FE0F',
        emoji: '🙍🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F64D-1F3FE-200D-2642-FE0F',
        emoji: '🙍🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F64D-1F3FF-200D-2642-FE0F',
        emoji: '🙍🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64D-200D-2640-FE0F',
    shortcodes: ['woman_frowning'],
    emoji: '🙍‍♀️',
    order: 566,
    group: 1,
    skins: [
      {
        hexcode: '1F64D-1F3FB-200D-2640-FE0F',
        emoji: '🙍🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F64D-1F3FC-200D-2640-FE0F',
        emoji: '🙍🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F64D-1F3FD-200D-2640-FE0F',
        emoji: '🙍🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F64D-1F3FE-200D-2640-FE0F',
        emoji: '🙍🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F64D-1F3FF-200D-2640-FE0F',
        emoji: '🙍🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64E',
    shortcodes: ['person_pouting'],
    emoji: '🙎',
    order: 578,
    group: 1,
    skins: [
      {
        hexcode: '1F64E-1F3FB',
        emoji: '🙎🏻',
        tone: 1,
      },
      {
        hexcode: '1F64E-1F3FC',
        emoji: '🙎🏼',
        tone: 2,
      },
      {
        hexcode: '1F64E-1F3FD',
        emoji: '🙎🏽',
        tone: 3,
      },
      {
        hexcode: '1F64E-1F3FE',
        emoji: '🙎🏾',
        tone: 4,
      },
      {
        hexcode: '1F64E-1F3FF',
        emoji: '🙎🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64E-200D-2642-FE0F',
    shortcodes: ['man_pouting'],
    emoji: '🙎‍♂️',
    order: 584,
    group: 1,
    skins: [
      {
        hexcode: '1F64E-1F3FB-200D-2642-FE0F',
        emoji: '🙎🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F64E-1F3FC-200D-2642-FE0F',
        emoji: '🙎🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F64E-1F3FD-200D-2642-FE0F',
        emoji: '🙎🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F64E-1F3FE-200D-2642-FE0F',
        emoji: '🙎🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F64E-1F3FF-200D-2642-FE0F',
        emoji: '🙎🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64E-200D-2640-FE0F',
    shortcodes: ['woman_pouting'],
    emoji: '🙎‍♀️',
    order: 596,
    group: 1,
    skins: [
      {
        hexcode: '1F64E-1F3FB-200D-2640-FE0F',
        emoji: '🙎🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F64E-1F3FC-200D-2640-FE0F',
        emoji: '🙎🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F64E-1F3FD-200D-2640-FE0F',
        emoji: '🙎🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F64E-1F3FE-200D-2640-FE0F',
        emoji: '🙎🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F64E-1F3FF-200D-2640-FE0F',
        emoji: '🙎🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F645',
    shortcodes: ['person_gesturing_no'],
    emoji: '🙅',
    order: 608,
    group: 1,
    skins: [
      {
        hexcode: '1F645-1F3FB',
        emoji: '🙅🏻',
        tone: 1,
      },
      {
        hexcode: '1F645-1F3FC',
        emoji: '🙅🏼',
        tone: 2,
      },
      {
        hexcode: '1F645-1F3FD',
        emoji: '🙅🏽',
        tone: 3,
      },
      {
        hexcode: '1F645-1F3FE',
        emoji: '🙅🏾',
        tone: 4,
      },
      {
        hexcode: '1F645-1F3FF',
        emoji: '🙅🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F645-200D-2642-FE0F',
    shortcodes: ['man_gesturing_no'],
    emoji: '🙅‍♂️',
    order: 614,
    group: 1,
    skins: [
      {
        hexcode: '1F645-1F3FB-200D-2642-FE0F',
        emoji: '🙅🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F645-1F3FC-200D-2642-FE0F',
        emoji: '🙅🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F645-1F3FD-200D-2642-FE0F',
        emoji: '🙅🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F645-1F3FE-200D-2642-FE0F',
        emoji: '🙅🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F645-1F3FF-200D-2642-FE0F',
        emoji: '🙅🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F645-200D-2640-FE0F',
    shortcodes: ['woman_gesturing_no'],
    emoji: '🙅‍♀️',
    order: 626,
    group: 1,
    skins: [
      {
        hexcode: '1F645-1F3FB-200D-2640-FE0F',
        emoji: '🙅🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F645-1F3FC-200D-2640-FE0F',
        emoji: '🙅🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F645-1F3FD-200D-2640-FE0F',
        emoji: '🙅🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F645-1F3FE-200D-2640-FE0F',
        emoji: '🙅🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F645-1F3FF-200D-2640-FE0F',
        emoji: '🙅🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F646',
    shortcodes: ['person_gesturing_ok'],
    emoji: '🙆',
    order: 638,
    group: 1,
    skins: [
      {
        hexcode: '1F646-1F3FB',
        emoji: '🙆🏻',
        tone: 1,
      },
      {
        hexcode: '1F646-1F3FC',
        emoji: '🙆🏼',
        tone: 2,
      },
      {
        hexcode: '1F646-1F3FD',
        emoji: '🙆🏽',
        tone: 3,
      },
      {
        hexcode: '1F646-1F3FE',
        emoji: '🙆🏾',
        tone: 4,
      },
      {
        hexcode: '1F646-1F3FF',
        emoji: '🙆🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F646-200D-2642-FE0F',
    shortcodes: ['man_gesturing_ok'],
    emoji: '🙆‍♂️',
    order: 644,
    group: 1,
    skins: [
      {
        hexcode: '1F646-1F3FB-200D-2642-FE0F',
        emoji: '🙆🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F646-1F3FC-200D-2642-FE0F',
        emoji: '🙆🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F646-1F3FD-200D-2642-FE0F',
        emoji: '🙆🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F646-1F3FE-200D-2642-FE0F',
        emoji: '🙆🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F646-1F3FF-200D-2642-FE0F',
        emoji: '🙆🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F646-200D-2640-FE0F',
    shortcodes: ['woman_gesturing_ok'],
    emoji: '🙆‍♀️',
    order: 656,
    group: 1,
    skins: [
      {
        hexcode: '1F646-1F3FB-200D-2640-FE0F',
        emoji: '🙆🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F646-1F3FC-200D-2640-FE0F',
        emoji: '🙆🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F646-1F3FD-200D-2640-FE0F',
        emoji: '🙆🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F646-1F3FE-200D-2640-FE0F',
        emoji: '🙆🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F646-1F3FF-200D-2640-FE0F',
        emoji: '🙆🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F481',
    shortcodes: ['person_tipping_hand'],
    emoji: '💁',
    order: 668,
    group: 1,
    skins: [
      {
        hexcode: '1F481-1F3FB',
        emoji: '💁🏻',
        tone: 1,
      },
      {
        hexcode: '1F481-1F3FC',
        emoji: '💁🏼',
        tone: 2,
      },
      {
        hexcode: '1F481-1F3FD',
        emoji: '💁🏽',
        tone: 3,
      },
      {
        hexcode: '1F481-1F3FE',
        emoji: '💁🏾',
        tone: 4,
      },
      {
        hexcode: '1F481-1F3FF',
        emoji: '💁🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F481-200D-2642-FE0F',
    shortcodes: ['man_tipping_hand'],
    emoji: '💁‍♂️',
    order: 674,
    group: 1,
    skins: [
      {
        hexcode: '1F481-1F3FB-200D-2642-FE0F',
        emoji: '💁🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F481-1F3FC-200D-2642-FE0F',
        emoji: '💁🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F481-1F3FD-200D-2642-FE0F',
        emoji: '💁🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F481-1F3FE-200D-2642-FE0F',
        emoji: '💁🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F481-1F3FF-200D-2642-FE0F',
        emoji: '💁🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F481-200D-2640-FE0F',
    shortcodes: ['woman_tipping_hand'],
    emoji: '💁‍♀️',
    order: 686,
    group: 1,
    skins: [
      {
        hexcode: '1F481-1F3FB-200D-2640-FE0F',
        emoji: '💁🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F481-1F3FC-200D-2640-FE0F',
        emoji: '💁🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F481-1F3FD-200D-2640-FE0F',
        emoji: '💁🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F481-1F3FE-200D-2640-FE0F',
        emoji: '💁🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F481-1F3FF-200D-2640-FE0F',
        emoji: '💁🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64B',
    shortcodes: ['person_raising_hand'],
    emoji: '🙋',
    order: 698,
    group: 1,
    skins: [
      {
        hexcode: '1F64B-1F3FB',
        emoji: '🙋🏻',
        tone: 1,
      },
      {
        hexcode: '1F64B-1F3FC',
        emoji: '🙋🏼',
        tone: 2,
      },
      {
        hexcode: '1F64B-1F3FD',
        emoji: '🙋🏽',
        tone: 3,
      },
      {
        hexcode: '1F64B-1F3FE',
        emoji: '🙋🏾',
        tone: 4,
      },
      {
        hexcode: '1F64B-1F3FF',
        emoji: '🙋🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64B-200D-2642-FE0F',
    shortcodes: ['man_raising_hand'],
    emoji: '🙋‍♂️',
    order: 704,
    group: 1,
    skins: [
      {
        hexcode: '1F64B-1F3FB-200D-2642-FE0F',
        emoji: '🙋🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F64B-1F3FC-200D-2642-FE0F',
        emoji: '🙋🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F64B-1F3FD-200D-2642-FE0F',
        emoji: '🙋🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F64B-1F3FE-200D-2642-FE0F',
        emoji: '🙋🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F64B-1F3FF-200D-2642-FE0F',
        emoji: '🙋🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F64B-200D-2640-FE0F',
    shortcodes: ['woman_raising_hand'],
    emoji: '🙋‍♀️',
    order: 716,
    group: 1,
    skins: [
      {
        hexcode: '1F64B-1F3FB-200D-2640-FE0F',
        emoji: '🙋🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F64B-1F3FC-200D-2640-FE0F',
        emoji: '🙋🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F64B-1F3FD-200D-2640-FE0F',
        emoji: '🙋🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F64B-1F3FE-200D-2640-FE0F',
        emoji: '🙋🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F64B-1F3FF-200D-2640-FE0F',
        emoji: '🙋🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CF',
    shortcodes: ['person_deaf'],
    emoji: '🧏',
    order: 728,
    group: 1,
    skins: [
      {
        hexcode: '1F9CF-1F3FB',
        emoji: '🧏🏻',
        tone: 1,
      },
      {
        hexcode: '1F9CF-1F3FC',
        emoji: '🧏🏼',
        tone: 2,
      },
      {
        hexcode: '1F9CF-1F3FD',
        emoji: '🧏🏽',
        tone: 3,
      },
      {
        hexcode: '1F9CF-1F3FE',
        emoji: '🧏🏾',
        tone: 4,
      },
      {
        hexcode: '1F9CF-1F3FF',
        emoji: '🧏🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CF-200D-2642-FE0F',
    shortcodes: ['man_deaf'],
    emoji: '🧏‍♂️',
    order: 734,
    group: 1,
    skins: [
      {
        hexcode: '1F9CF-1F3FB-200D-2642-FE0F',
        emoji: '🧏🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9CF-1F3FC-200D-2642-FE0F',
        emoji: '🧏🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9CF-1F3FD-200D-2642-FE0F',
        emoji: '🧏🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9CF-1F3FE-200D-2642-FE0F',
        emoji: '🧏🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9CF-1F3FF-200D-2642-FE0F',
        emoji: '🧏🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CF-200D-2640-FE0F',
    shortcodes: ['woman_deaf'],
    emoji: '🧏‍♀️',
    order: 746,
    group: 1,
    skins: [
      {
        hexcode: '1F9CF-1F3FB-200D-2640-FE0F',
        emoji: '🧏🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9CF-1F3FC-200D-2640-FE0F',
        emoji: '🧏🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9CF-1F3FD-200D-2640-FE0F',
        emoji: '🧏🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9CF-1F3FE-200D-2640-FE0F',
        emoji: '🧏🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9CF-1F3FF-200D-2640-FE0F',
        emoji: '🧏🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F647',
    shortcodes: ['person_bowing'],
    emoji: '🙇',
    order: 758,
    group: 1,
    skins: [
      {
        hexcode: '1F647-1F3FB',
        emoji: '🙇🏻',
        tone: 1,
      },
      {
        hexcode: '1F647-1F3FC',
        emoji: '🙇🏼',
        tone: 2,
      },
      {
        hexcode: '1F647-1F3FD',
        emoji: '🙇🏽',
        tone: 3,
      },
      {
        hexcode: '1F647-1F3FE',
        emoji: '🙇🏾',
        tone: 4,
      },
      {
        hexcode: '1F647-1F3FF',
        emoji: '🙇🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F647-200D-2642-FE0F',
    shortcodes: ['man_bowing'],
    emoji: '🙇‍♂️',
    order: 764,
    group: 1,
    skins: [
      {
        hexcode: '1F647-1F3FB-200D-2642-FE0F',
        emoji: '🙇🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F647-1F3FC-200D-2642-FE0F',
        emoji: '🙇🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F647-1F3FD-200D-2642-FE0F',
        emoji: '🙇🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F647-1F3FE-200D-2642-FE0F',
        emoji: '🙇🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F647-1F3FF-200D-2642-FE0F',
        emoji: '🙇🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F647-200D-2640-FE0F',
    shortcodes: ['woman_bowing'],
    emoji: '🙇‍♀️',
    order: 776,
    group: 1,
    skins: [
      {
        hexcode: '1F647-1F3FB-200D-2640-FE0F',
        emoji: '🙇🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F647-1F3FC-200D-2640-FE0F',
        emoji: '🙇🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F647-1F3FD-200D-2640-FE0F',
        emoji: '🙇🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F647-1F3FE-200D-2640-FE0F',
        emoji: '🙇🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F647-1F3FF-200D-2640-FE0F',
        emoji: '🙇🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F926',
    shortcodes: ['person_facepalming'],
    emoji: '🤦',
    order: 788,
    group: 1,
    skins: [
      {
        hexcode: '1F926-1F3FB',
        emoji: '🤦🏻',
        tone: 1,
      },
      {
        hexcode: '1F926-1F3FC',
        emoji: '🤦🏼',
        tone: 2,
      },
      {
        hexcode: '1F926-1F3FD',
        emoji: '🤦🏽',
        tone: 3,
      },
      {
        hexcode: '1F926-1F3FE',
        emoji: '🤦🏾',
        tone: 4,
      },
      {
        hexcode: '1F926-1F3FF',
        emoji: '🤦🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F926-200D-2642-FE0F',
    shortcodes: ['man_facepalming'],
    emoji: '🤦‍♂️',
    order: 794,
    group: 1,
    skins: [
      {
        hexcode: '1F926-1F3FB-200D-2642-FE0F',
        emoji: '🤦🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F926-1F3FC-200D-2642-FE0F',
        emoji: '🤦🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F926-1F3FD-200D-2642-FE0F',
        emoji: '🤦🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F926-1F3FE-200D-2642-FE0F',
        emoji: '🤦🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F926-1F3FF-200D-2642-FE0F',
        emoji: '🤦🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F926-200D-2640-FE0F',
    shortcodes: ['woman_facepalming'],
    emoji: '🤦‍♀️',
    order: 806,
    group: 1,
    skins: [
      {
        hexcode: '1F926-1F3FB-200D-2640-FE0F',
        emoji: '🤦🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F926-1F3FC-200D-2640-FE0F',
        emoji: '🤦🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F926-1F3FD-200D-2640-FE0F',
        emoji: '🤦🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F926-1F3FE-200D-2640-FE0F',
        emoji: '🤦🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F926-1F3FF-200D-2640-FE0F',
        emoji: '🤦🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F937',
    shortcodes: ['person_shrugging'],
    emoji: '🤷',
    order: 818,
    group: 1,
    skins: [
      {
        hexcode: '1F937-1F3FB',
        emoji: '🤷🏻',
        tone: 1,
      },
      {
        hexcode: '1F937-1F3FC',
        emoji: '🤷🏼',
        tone: 2,
      },
      {
        hexcode: '1F937-1F3FD',
        emoji: '🤷🏽',
        tone: 3,
      },
      {
        hexcode: '1F937-1F3FE',
        emoji: '🤷🏾',
        tone: 4,
      },
      {
        hexcode: '1F937-1F3FF',
        emoji: '🤷🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F937-200D-2642-FE0F',
    shortcodes: ['man_shrugging'],
    emoji: '🤷‍♂️',
    order: 824,
    group: 1,
    skins: [
      {
        hexcode: '1F937-1F3FB-200D-2642-FE0F',
        emoji: '🤷🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F937-1F3FC-200D-2642-FE0F',
        emoji: '🤷🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F937-1F3FD-200D-2642-FE0F',
        emoji: '🤷🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F937-1F3FE-200D-2642-FE0F',
        emoji: '🤷🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F937-1F3FF-200D-2642-FE0F',
        emoji: '🤷🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F937-200D-2640-FE0F',
    shortcodes: ['woman_shrugging'],
    emoji: '🤷‍♀️',
    order: 836,
    group: 1,
    skins: [
      {
        hexcode: '1F937-1F3FB-200D-2640-FE0F',
        emoji: '🤷🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F937-1F3FC-200D-2640-FE0F',
        emoji: '🤷🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F937-1F3FD-200D-2640-FE0F',
        emoji: '🤷🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F937-1F3FE-200D-2640-FE0F',
        emoji: '🤷🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F937-1F3FF-200D-2640-FE0F',
        emoji: '🤷🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-2695-FE0F',
    shortcodes: ['man_health_worker'],
    emoji: '👨‍⚕️',
    order: 848,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-2695-FE0F',
        emoji: '👨🏻‍⚕️',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-2695-FE0F',
        emoji: '👨🏼‍⚕️',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-2695-FE0F',
        emoji: '👨🏽‍⚕️',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-2695-FE0F',
        emoji: '👨🏾‍⚕️',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-2695-FE0F',
        emoji: '👨🏿‍⚕️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-2695-FE0F',
    shortcodes: ['woman_health_worker'],
    emoji: '👩‍⚕️',
    order: 860,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-2695-FE0F',
        emoji: '👩🏻‍⚕️',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-2695-FE0F',
        emoji: '👩🏼‍⚕️',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-2695-FE0F',
        emoji: '👩🏽‍⚕️',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-2695-FE0F',
        emoji: '👩🏾‍⚕️',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-2695-FE0F',
        emoji: '👩🏿‍⚕️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F393',
    shortcodes: ['man_student'],
    emoji: '👨‍🎓',
    order: 872,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F393',
        emoji: '👨🏻‍🎓',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F393',
        emoji: '👨🏼‍🎓',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F393',
        emoji: '👨🏽‍🎓',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F393',
        emoji: '👨🏾‍🎓',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F393',
        emoji: '👨🏿‍🎓',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F393',
    shortcodes: ['woman_student'],
    emoji: '👩‍🎓',
    order: 878,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F393',
        emoji: '👩🏻‍🎓',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F393',
        emoji: '👩🏼‍🎓',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F393',
        emoji: '👩🏽‍🎓',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F393',
        emoji: '👩🏾‍🎓',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F393',
        emoji: '👩🏿‍🎓',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F3EB',
    shortcodes: ['man_teacher'],
    emoji: '👨‍🏫',
    order: 884,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F3EB',
        emoji: '👨🏻‍🏫',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F3EB',
        emoji: '👨🏼‍🏫',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F3EB',
        emoji: '👨🏽‍🏫',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F3EB',
        emoji: '👨🏾‍🏫',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F3EB',
        emoji: '👨🏿‍🏫',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F3EB',
    shortcodes: ['woman_teacher'],
    emoji: '👩‍🏫',
    order: 890,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F3EB',
        emoji: '👩🏻‍🏫',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F3EB',
        emoji: '👩🏼‍🏫',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F3EB',
        emoji: '👩🏽‍🏫',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F3EB',
        emoji: '👩🏾‍🏫',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F3EB',
        emoji: '👩🏿‍🏫',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-2696-FE0F',
    shortcodes: ['man_judge'],
    emoji: '👨‍⚖️',
    order: 896,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-2696-FE0F',
        emoji: '👨🏻‍⚖️',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-2696-FE0F',
        emoji: '👨🏼‍⚖️',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-2696-FE0F',
        emoji: '👨🏽‍⚖️',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-2696-FE0F',
        emoji: '👨🏾‍⚖️',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-2696-FE0F',
        emoji: '👨🏿‍⚖️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-2696-FE0F',
    shortcodes: ['woman_judge'],
    emoji: '👩‍⚖️',
    order: 908,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-2696-FE0F',
        emoji: '👩🏻‍⚖️',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-2696-FE0F',
        emoji: '👩🏼‍⚖️',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-2696-FE0F',
        emoji: '👩🏽‍⚖️',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-2696-FE0F',
        emoji: '👩🏾‍⚖️',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-2696-FE0F',
        emoji: '👩🏿‍⚖️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F33E',
    shortcodes: ['man_farmer'],
    emoji: '👨‍🌾',
    order: 920,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F33E',
        emoji: '👨🏻‍🌾',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F33E',
        emoji: '👨🏼‍🌾',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F33E',
        emoji: '👨🏽‍🌾',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F33E',
        emoji: '👨🏾‍🌾',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F33E',
        emoji: '👨🏿‍🌾',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F33E',
    shortcodes: ['woman_farmer'],
    emoji: '👩‍🌾',
    order: 926,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F33E',
        emoji: '👩🏻‍🌾',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F33E',
        emoji: '👩🏼‍🌾',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F33E',
        emoji: '👩🏽‍🌾',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F33E',
        emoji: '👩🏾‍🌾',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F33E',
        emoji: '👩🏿‍🌾',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F373',
    shortcodes: ['man_cook'],
    emoji: '👨‍🍳',
    order: 932,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F373',
        emoji: '👨🏻‍🍳',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F373',
        emoji: '👨🏼‍🍳',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F373',
        emoji: '👨🏽‍🍳',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F373',
        emoji: '👨🏾‍🍳',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F373',
        emoji: '👨🏿‍🍳',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F373',
    shortcodes: ['woman_cook'],
    emoji: '👩‍🍳',
    order: 938,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F373',
        emoji: '👩🏻‍🍳',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F373',
        emoji: '👩🏼‍🍳',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F373',
        emoji: '👩🏽‍🍳',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F373',
        emoji: '👩🏾‍🍳',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F373',
        emoji: '👩🏿‍🍳',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F527',
    shortcodes: ['man_mechanic'],
    emoji: '👨‍🔧',
    order: 944,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F527',
        emoji: '👨🏻‍🔧',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F527',
        emoji: '👨🏼‍🔧',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F527',
        emoji: '👨🏽‍🔧',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F527',
        emoji: '👨🏾‍🔧',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F527',
        emoji: '👨🏿‍🔧',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F527',
    shortcodes: ['woman_mechanic'],
    emoji: '👩‍🔧',
    order: 950,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F527',
        emoji: '👩🏻‍🔧',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F527',
        emoji: '👩🏼‍🔧',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F527',
        emoji: '👩🏽‍🔧',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F527',
        emoji: '👩🏾‍🔧',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F527',
        emoji: '👩🏿‍🔧',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F3ED',
    shortcodes: ['man_factory_worker'],
    emoji: '👨‍🏭',
    order: 956,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F3ED',
        emoji: '👨🏻‍🏭',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F3ED',
        emoji: '👨🏼‍🏭',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F3ED',
        emoji: '👨🏽‍🏭',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F3ED',
        emoji: '👨🏾‍🏭',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F3ED',
        emoji: '👨🏿‍🏭',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F3ED',
    shortcodes: ['woman_factory_worker'],
    emoji: '👩‍🏭',
    order: 962,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F3ED',
        emoji: '👩🏻‍🏭',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F3ED',
        emoji: '👩🏼‍🏭',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F3ED',
        emoji: '👩🏽‍🏭',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F3ED',
        emoji: '👩🏾‍🏭',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F3ED',
        emoji: '👩🏿‍🏭',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F4BC',
    shortcodes: ['man_office_worker'],
    emoji: '👨‍💼',
    order: 968,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F4BC',
        emoji: '👨🏻‍💼',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F4BC',
        emoji: '👨🏼‍💼',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F4BC',
        emoji: '👨🏽‍💼',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F4BC',
        emoji: '👨🏾‍💼',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F4BC',
        emoji: '👨🏿‍💼',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F4BC',
    shortcodes: ['woman_office_worker'],
    emoji: '👩‍💼',
    order: 974,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F4BC',
        emoji: '👩🏻‍💼',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F4BC',
        emoji: '👩🏼‍💼',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F4BC',
        emoji: '👩🏽‍💼',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F4BC',
        emoji: '👩🏾‍💼',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F4BC',
        emoji: '👩🏿‍💼',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F52C',
    shortcodes: ['man_scientist'],
    emoji: '👨‍🔬',
    order: 980,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F52C',
        emoji: '👨🏻‍🔬',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F52C',
        emoji: '👨🏼‍🔬',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F52C',
        emoji: '👨🏽‍🔬',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F52C',
        emoji: '👨🏾‍🔬',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F52C',
        emoji: '👨🏿‍🔬',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F52C',
    shortcodes: ['woman_scientist'],
    emoji: '👩‍🔬',
    order: 986,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F52C',
        emoji: '👩🏻‍🔬',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F52C',
        emoji: '👩🏼‍🔬',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F52C',
        emoji: '👩🏽‍🔬',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F52C',
        emoji: '👩🏾‍🔬',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F52C',
        emoji: '👩🏿‍🔬',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F4BB',
    shortcodes: ['man_technologist'],
    emoji: '👨‍💻',
    order: 992,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F4BB',
        emoji: '👨🏻‍💻',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F4BB',
        emoji: '👨🏼‍💻',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F4BB',
        emoji: '👨🏽‍💻',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F4BB',
        emoji: '👨🏾‍💻',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F4BB',
        emoji: '👨🏿‍💻',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F4BB',
    shortcodes: ['woman_technologist'],
    emoji: '👩‍💻',
    order: 998,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F4BB',
        emoji: '👩🏻‍💻',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F4BB',
        emoji: '👩🏼‍💻',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F4BB',
        emoji: '👩🏽‍💻',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F4BB',
        emoji: '👩🏾‍💻',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F4BB',
        emoji: '👩🏿‍💻',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F3A4',
    shortcodes: ['man_singer'],
    emoji: '👨‍🎤',
    order: 1004,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F3A4',
        emoji: '👨🏻‍🎤',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F3A4',
        emoji: '👨🏼‍🎤',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F3A4',
        emoji: '👨🏽‍🎤',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F3A4',
        emoji: '👨🏾‍🎤',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F3A4',
        emoji: '👨🏿‍🎤',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F3A4',
    shortcodes: ['woman_singer'],
    emoji: '👩‍🎤',
    order: 1010,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F3A4',
        emoji: '👩🏻‍🎤',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F3A4',
        emoji: '👩🏼‍🎤',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F3A4',
        emoji: '👩🏽‍🎤',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F3A4',
        emoji: '👩🏾‍🎤',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F3A4',
        emoji: '👩🏿‍🎤',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F3A8',
    shortcodes: ['man_artist'],
    emoji: '👨‍🎨',
    order: 1016,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F3A8',
        emoji: '👨🏻‍🎨',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F3A8',
        emoji: '👨🏼‍🎨',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F3A8',
        emoji: '👨🏽‍🎨',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F3A8',
        emoji: '👨🏾‍🎨',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F3A8',
        emoji: '👨🏿‍🎨',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F3A8',
    shortcodes: ['woman_artist'],
    emoji: '👩‍🎨',
    order: 1022,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F3A8',
        emoji: '👩🏻‍🎨',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F3A8',
        emoji: '👩🏼‍🎨',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F3A8',
        emoji: '👩🏽‍🎨',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F3A8',
        emoji: '👩🏾‍🎨',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F3A8',
        emoji: '👩🏿‍🎨',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-2708-FE0F',
    shortcodes: ['man_pilot'],
    emoji: '👨‍✈️',
    order: 1028,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-2708-FE0F',
        emoji: '👨🏻‍✈️',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-2708-FE0F',
        emoji: '👨🏼‍✈️',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-2708-FE0F',
        emoji: '👨🏽‍✈️',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-2708-FE0F',
        emoji: '👨🏾‍✈️',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-2708-FE0F',
        emoji: '👨🏿‍✈️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-2708-FE0F',
    shortcodes: ['woman_pilot'],
    emoji: '👩‍✈️',
    order: 1040,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-2708-FE0F',
        emoji: '👩🏻‍✈️',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-2708-FE0F',
        emoji: '👩🏼‍✈️',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-2708-FE0F',
        emoji: '👩🏽‍✈️',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-2708-FE0F',
        emoji: '👩🏾‍✈️',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-2708-FE0F',
        emoji: '👩🏿‍✈️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F680',
    shortcodes: ['man_astronaut'],
    emoji: '👨‍🚀',
    order: 1052,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F680',
        emoji: '👨🏻‍🚀',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F680',
        emoji: '👨🏼‍🚀',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F680',
        emoji: '👨🏽‍🚀',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F680',
        emoji: '👨🏾‍🚀',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F680',
        emoji: '👨🏿‍🚀',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F680',
    shortcodes: ['woman_astronaut'],
    emoji: '👩‍🚀',
    order: 1058,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F680',
        emoji: '👩🏻‍🚀',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F680',
        emoji: '👩🏼‍🚀',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F680',
        emoji: '👩🏽‍🚀',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F680',
        emoji: '👩🏾‍🚀',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F680',
        emoji: '👩🏿‍🚀',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F692',
    shortcodes: ['man_firefighter'],
    emoji: '👨‍🚒',
    order: 1064,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F692',
        emoji: '👨🏻‍🚒',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F692',
        emoji: '👨🏼‍🚒',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F692',
        emoji: '👨🏽‍🚒',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F692',
        emoji: '👨🏾‍🚒',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F692',
        emoji: '👨🏿‍🚒',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F692',
    shortcodes: ['woman_firefighter'],
    emoji: '👩‍🚒',
    order: 1070,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F692',
        emoji: '👩🏻‍🚒',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F692',
        emoji: '👩🏼‍🚒',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F692',
        emoji: '👩🏽‍🚒',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F692',
        emoji: '👩🏾‍🚒',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F692',
        emoji: '👩🏿‍🚒',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F46E',
    shortcodes: ['police_officer'],
    emoji: '👮',
    order: 1076,
    group: 1,
    skins: [
      {
        hexcode: '1F46E-1F3FB',
        emoji: '👮🏻',
        tone: 1,
      },
      {
        hexcode: '1F46E-1F3FC',
        emoji: '👮🏼',
        tone: 2,
      },
      {
        hexcode: '1F46E-1F3FD',
        emoji: '👮🏽',
        tone: 3,
      },
      {
        hexcode: '1F46E-1F3FE',
        emoji: '👮🏾',
        tone: 4,
      },
      {
        hexcode: '1F46E-1F3FF',
        emoji: '👮🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F46E-200D-2642-FE0F',
    shortcodes: ['man_police_officer'],
    emoji: '👮‍♂️',
    order: 1082,
    group: 1,
    skins: [
      {
        hexcode: '1F46E-1F3FB-200D-2642-FE0F',
        emoji: '👮🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F46E-1F3FC-200D-2642-FE0F',
        emoji: '👮🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F46E-1F3FD-200D-2642-FE0F',
        emoji: '👮🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F46E-1F3FE-200D-2642-FE0F',
        emoji: '👮🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F46E-1F3FF-200D-2642-FE0F',
        emoji: '👮🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F46E-200D-2640-FE0F',
    shortcodes: ['woman_police_officer'],
    emoji: '👮‍♀️',
    order: 1094,
    group: 1,
    skins: [
      {
        hexcode: '1F46E-1F3FB-200D-2640-FE0F',
        emoji: '👮🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F46E-1F3FC-200D-2640-FE0F',
        emoji: '👮🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F46E-1F3FD-200D-2640-FE0F',
        emoji: '👮🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F46E-1F3FE-200D-2640-FE0F',
        emoji: '👮🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F46E-1F3FF-200D-2640-FE0F',
        emoji: '👮🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F575',
    shortcodes: ['detective'],
    emoji: '🕵️',
    order: 1107,
    group: 1,
    skins: [
      {
        hexcode: '1F575-1F3FB',
        emoji: '🕵🏻',
        tone: 1,
      },
      {
        hexcode: '1F575-1F3FC',
        emoji: '🕵🏼',
        tone: 2,
      },
      {
        hexcode: '1F575-1F3FD',
        emoji: '🕵🏽',
        tone: 3,
      },
      {
        hexcode: '1F575-1F3FE',
        emoji: '🕵🏾',
        tone: 4,
      },
      {
        hexcode: '1F575-1F3FF',
        emoji: '🕵🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F575-FE0F-200D-2642-FE0F',
    shortcodes: ['man_detective'],
    emoji: '🕵️‍♂️',
    order: 1113,
    group: 1,
    skins: [
      {
        hexcode: '1F575-1F3FB-200D-2642-FE0F',
        emoji: '🕵🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F575-1F3FC-200D-2642-FE0F',
        emoji: '🕵🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F575-1F3FD-200D-2642-FE0F',
        emoji: '🕵🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F575-1F3FE-200D-2642-FE0F',
        emoji: '🕵🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F575-1F3FF-200D-2642-FE0F',
        emoji: '🕵🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F575-FE0F-200D-2640-FE0F',
    shortcodes: ['woman_detective'],
    emoji: '🕵️‍♀️',
    order: 1127,
    group: 1,
    skins: [
      {
        hexcode: '1F575-1F3FB-200D-2640-FE0F',
        emoji: '🕵🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F575-1F3FC-200D-2640-FE0F',
        emoji: '🕵🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F575-1F3FD-200D-2640-FE0F',
        emoji: '🕵🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F575-1F3FE-200D-2640-FE0F',
        emoji: '🕵🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F575-1F3FF-200D-2640-FE0F',
        emoji: '🕵🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F482',
    shortcodes: ['guard'],
    emoji: '💂',
    order: 1141,
    group: 1,
    skins: [
      {
        hexcode: '1F482-1F3FB',
        emoji: '💂🏻',
        tone: 1,
      },
      {
        hexcode: '1F482-1F3FC',
        emoji: '💂🏼',
        tone: 2,
      },
      {
        hexcode: '1F482-1F3FD',
        emoji: '💂🏽',
        tone: 3,
      },
      {
        hexcode: '1F482-1F3FE',
        emoji: '💂🏾',
        tone: 4,
      },
      {
        hexcode: '1F482-1F3FF',
        emoji: '💂🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F482-200D-2642-FE0F',
    shortcodes: ['man_guard'],
    emoji: '💂‍♂️',
    order: 1147,
    group: 1,
    skins: [
      {
        hexcode: '1F482-1F3FB-200D-2642-FE0F',
        emoji: '💂🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F482-1F3FC-200D-2642-FE0F',
        emoji: '💂🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F482-1F3FD-200D-2642-FE0F',
        emoji: '💂🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F482-1F3FE-200D-2642-FE0F',
        emoji: '💂🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F482-1F3FF-200D-2642-FE0F',
        emoji: '💂🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F482-200D-2640-FE0F',
    shortcodes: ['woman_guard'],
    emoji: '💂‍♀️',
    order: 1159,
    group: 1,
    skins: [
      {
        hexcode: '1F482-1F3FB-200D-2640-FE0F',
        emoji: '💂🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F482-1F3FC-200D-2640-FE0F',
        emoji: '💂🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F482-1F3FD-200D-2640-FE0F',
        emoji: '💂🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F482-1F3FE-200D-2640-FE0F',
        emoji: '💂🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F482-1F3FF-200D-2640-FE0F',
        emoji: '💂🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F477',
    shortcodes: ['construction_worker'],
    emoji: '👷',
    order: 1171,
    group: 1,
    skins: [
      {
        hexcode: '1F477-1F3FB',
        emoji: '👷🏻',
        tone: 1,
      },
      {
        hexcode: '1F477-1F3FC',
        emoji: '👷🏼',
        tone: 2,
      },
      {
        hexcode: '1F477-1F3FD',
        emoji: '👷🏽',
        tone: 3,
      },
      {
        hexcode: '1F477-1F3FE',
        emoji: '👷🏾',
        tone: 4,
      },
      {
        hexcode: '1F477-1F3FF',
        emoji: '👷🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F477-200D-2642-FE0F',
    shortcodes: ['man_construction_worker'],
    emoji: '👷‍♂️',
    order: 1177,
    group: 1,
    skins: [
      {
        hexcode: '1F477-1F3FB-200D-2642-FE0F',
        emoji: '👷🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F477-1F3FC-200D-2642-FE0F',
        emoji: '👷🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F477-1F3FD-200D-2642-FE0F',
        emoji: '👷🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F477-1F3FE-200D-2642-FE0F',
        emoji: '👷🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F477-1F3FF-200D-2642-FE0F',
        emoji: '👷🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F477-200D-2640-FE0F',
    shortcodes: ['woman_construction_worker'],
    emoji: '👷‍♀️',
    order: 1189,
    group: 1,
    skins: [
      {
        hexcode: '1F477-1F3FB-200D-2640-FE0F',
        emoji: '👷🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F477-1F3FC-200D-2640-FE0F',
        emoji: '👷🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F477-1F3FD-200D-2640-FE0F',
        emoji: '👷🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F477-1F3FE-200D-2640-FE0F',
        emoji: '👷🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F477-1F3FF-200D-2640-FE0F',
        emoji: '👷🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F934',
    shortcodes: ['prince'],
    emoji: '🤴',
    order: 1201,
    group: 1,
    skins: [
      {
        hexcode: '1F934-1F3FB',
        emoji: '🤴🏻',
        tone: 1,
      },
      {
        hexcode: '1F934-1F3FC',
        emoji: '🤴🏼',
        tone: 2,
      },
      {
        hexcode: '1F934-1F3FD',
        emoji: '🤴🏽',
        tone: 3,
      },
      {
        hexcode: '1F934-1F3FE',
        emoji: '🤴🏾',
        tone: 4,
      },
      {
        hexcode: '1F934-1F3FF',
        emoji: '🤴🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F478',
    shortcodes: ['princess'],
    emoji: '👸',
    order: 1207,
    group: 1,
    skins: [
      {
        hexcode: '1F478-1F3FB',
        emoji: '👸🏻',
        tone: 1,
      },
      {
        hexcode: '1F478-1F3FC',
        emoji: '👸🏼',
        tone: 2,
      },
      {
        hexcode: '1F478-1F3FD',
        emoji: '👸🏽',
        tone: 3,
      },
      {
        hexcode: '1F478-1F3FE',
        emoji: '👸🏾',
        tone: 4,
      },
      {
        hexcode: '1F478-1F3FF',
        emoji: '👸🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F473',
    shortcodes: ['person_turban'],
    emoji: '👳',
    order: 1213,
    group: 1,
    skins: [
      {
        hexcode: '1F473-1F3FB',
        emoji: '👳🏻',
        tone: 1,
      },
      {
        hexcode: '1F473-1F3FC',
        emoji: '👳🏼',
        tone: 2,
      },
      {
        hexcode: '1F473-1F3FD',
        emoji: '👳🏽',
        tone: 3,
      },
      {
        hexcode: '1F473-1F3FE',
        emoji: '👳🏾',
        tone: 4,
      },
      {
        hexcode: '1F473-1F3FF',
        emoji: '👳🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F473-200D-2642-FE0F',
    shortcodes: ['man_turban'],
    emoji: '👳‍♂️',
    order: 1219,
    group: 1,
    skins: [
      {
        hexcode: '1F473-1F3FB-200D-2642-FE0F',
        emoji: '👳🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F473-1F3FC-200D-2642-FE0F',
        emoji: '👳🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F473-1F3FD-200D-2642-FE0F',
        emoji: '👳🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F473-1F3FE-200D-2642-FE0F',
        emoji: '👳🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F473-1F3FF-200D-2642-FE0F',
        emoji: '👳🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F473-200D-2640-FE0F',
    shortcodes: ['woman_turban'],
    emoji: '👳‍♀️',
    order: 1231,
    group: 1,
    skins: [
      {
        hexcode: '1F473-1F3FB-200D-2640-FE0F',
        emoji: '👳🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F473-1F3FC-200D-2640-FE0F',
        emoji: '👳🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F473-1F3FD-200D-2640-FE0F',
        emoji: '👳🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F473-1F3FE-200D-2640-FE0F',
        emoji: '👳🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F473-1F3FF-200D-2640-FE0F',
        emoji: '👳🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F472',
    shortcodes: ['man_chinese_cap'],
    emoji: '👲',
    order: 1243,
    group: 1,
    skins: [
      {
        hexcode: '1F472-1F3FB',
        emoji: '👲🏻',
        tone: 1,
      },
      {
        hexcode: '1F472-1F3FC',
        emoji: '👲🏼',
        tone: 2,
      },
      {
        hexcode: '1F472-1F3FD',
        emoji: '👲🏽',
        tone: 3,
      },
      {
        hexcode: '1F472-1F3FE',
        emoji: '👲🏾',
        tone: 4,
      },
      {
        hexcode: '1F472-1F3FF',
        emoji: '👲🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D5',
    shortcodes: ['woman_headscarf'],
    emoji: '🧕',
    order: 1249,
    group: 1,
    skins: [
      {
        hexcode: '1F9D5-1F3FB',
        emoji: '🧕🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D5-1F3FC',
        emoji: '🧕🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D5-1F3FD',
        emoji: '🧕🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D5-1F3FE',
        emoji: '🧕🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D5-1F3FF',
        emoji: '🧕🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F935',
    shortcodes: ['man_tuxedo'],
    emoji: '🤵',
    order: 1255,
    group: 1,
    skins: [
      {
        hexcode: '1F935-1F3FB',
        emoji: '🤵🏻',
        tone: 1,
      },
      {
        hexcode: '1F935-1F3FC',
        emoji: '🤵🏼',
        tone: 2,
      },
      {
        hexcode: '1F935-1F3FD',
        emoji: '🤵🏽',
        tone: 3,
      },
      {
        hexcode: '1F935-1F3FE',
        emoji: '🤵🏾',
        tone: 4,
      },
      {
        hexcode: '1F935-1F3FF',
        emoji: '🤵🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F470',
    shortcodes: ['bride_veil'],
    emoji: '👰',
    order: 1261,
    group: 1,
    skins: [
      {
        hexcode: '1F470-1F3FB',
        emoji: '👰🏻',
        tone: 1,
      },
      {
        hexcode: '1F470-1F3FC',
        emoji: '👰🏼',
        tone: 2,
      },
      {
        hexcode: '1F470-1F3FD',
        emoji: '👰🏽',
        tone: 3,
      },
      {
        hexcode: '1F470-1F3FE',
        emoji: '👰🏾',
        tone: 4,
      },
      {
        hexcode: '1F470-1F3FF',
        emoji: '👰🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F930',
    shortcodes: ['pregnant_woman'],
    emoji: '🤰',
    order: 1267,
    group: 1,
    skins: [
      {
        hexcode: '1F930-1F3FB',
        emoji: '🤰🏻',
        tone: 1,
      },
      {
        hexcode: '1F930-1F3FC',
        emoji: '🤰🏼',
        tone: 2,
      },
      {
        hexcode: '1F930-1F3FD',
        emoji: '🤰🏽',
        tone: 3,
      },
      {
        hexcode: '1F930-1F3FE',
        emoji: '🤰🏾',
        tone: 4,
      },
      {
        hexcode: '1F930-1F3FF',
        emoji: '🤰🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F931',
    shortcodes: ['breast_feeding'],
    emoji: '🤱',
    order: 1273,
    group: 1,
    skins: [
      {
        hexcode: '1F931-1F3FB',
        emoji: '🤱🏻',
        tone: 1,
      },
      {
        hexcode: '1F931-1F3FC',
        emoji: '🤱🏼',
        tone: 2,
      },
      {
        hexcode: '1F931-1F3FD',
        emoji: '🤱🏽',
        tone: 3,
      },
      {
        hexcode: '1F931-1F3FE',
        emoji: '🤱🏾',
        tone: 4,
      },
      {
        hexcode: '1F931-1F3FF',
        emoji: '🤱🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F47C',
    shortcodes: ['baby_angel'],
    emoji: '👼',
    order: 1279,
    group: 1,
    skins: [
      {
        hexcode: '1F47C-1F3FB',
        emoji: '👼🏻',
        tone: 1,
      },
      {
        hexcode: '1F47C-1F3FC',
        emoji: '👼🏼',
        tone: 2,
      },
      {
        hexcode: '1F47C-1F3FD',
        emoji: '👼🏽',
        tone: 3,
      },
      {
        hexcode: '1F47C-1F3FE',
        emoji: '👼🏾',
        tone: 4,
      },
      {
        hexcode: '1F47C-1F3FF',
        emoji: '👼🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F385',
    shortcodes: ['santa'],
    emoji: '🎅',
    order: 1285,
    group: 1,
    skins: [
      {
        hexcode: '1F385-1F3FB',
        emoji: '🎅🏻',
        tone: 1,
      },
      {
        hexcode: '1F385-1F3FC',
        emoji: '🎅🏼',
        tone: 2,
      },
      {
        hexcode: '1F385-1F3FD',
        emoji: '🎅🏽',
        tone: 3,
      },
      {
        hexcode: '1F385-1F3FE',
        emoji: '🎅🏾',
        tone: 4,
      },
      {
        hexcode: '1F385-1F3FF',
        emoji: '🎅🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F936',
    shortcodes: ['mrs_claus'],
    emoji: '🤶',
    order: 1291,
    group: 1,
    skins: [
      {
        hexcode: '1F936-1F3FB',
        emoji: '🤶🏻',
        tone: 1,
      },
      {
        hexcode: '1F936-1F3FC',
        emoji: '🤶🏼',
        tone: 2,
      },
      {
        hexcode: '1F936-1F3FD',
        emoji: '🤶🏽',
        tone: 3,
      },
      {
        hexcode: '1F936-1F3FE',
        emoji: '🤶🏾',
        tone: 4,
      },
      {
        hexcode: '1F936-1F3FF',
        emoji: '🤶🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B8',
    shortcodes: ['hero', 'superhero'],
    emoji: '🦸',
    order: 1297,
    group: 1,
    skins: [
      {
        hexcode: '1F9B8-1F3FB',
        emoji: '🦸🏻',
        tone: 1,
      },
      {
        hexcode: '1F9B8-1F3FC',
        emoji: '🦸🏼',
        tone: 2,
      },
      {
        hexcode: '1F9B8-1F3FD',
        emoji: '🦸🏽',
        tone: 3,
      },
      {
        hexcode: '1F9B8-1F3FE',
        emoji: '🦸🏾',
        tone: 4,
      },
      {
        hexcode: '1F9B8-1F3FF',
        emoji: '🦸🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B8-200D-2642-FE0F',
    shortcodes: ['man_hero'],
    emoji: '🦸‍♂️',
    order: 1303,
    group: 1,
    skins: [
      {
        hexcode: '1F9B8-1F3FB-200D-2642-FE0F',
        emoji: '🦸🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9B8-1F3FC-200D-2642-FE0F',
        emoji: '🦸🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9B8-1F3FD-200D-2642-FE0F',
        emoji: '🦸🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9B8-1F3FE-200D-2642-FE0F',
        emoji: '🦸🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9B8-1F3FF-200D-2642-FE0F',
        emoji: '🦸🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B8-200D-2640-FE0F',
    shortcodes: ['woman_hero'],
    emoji: '🦸‍♀️',
    order: 1315,
    group: 1,
    skins: [
      {
        hexcode: '1F9B8-1F3FB-200D-2640-FE0F',
        emoji: '🦸🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9B8-1F3FC-200D-2640-FE0F',
        emoji: '🦸🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9B8-1F3FD-200D-2640-FE0F',
        emoji: '🦸🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9B8-1F3FE-200D-2640-FE0F',
        emoji: '🦸🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9B8-1F3FF-200D-2640-FE0F',
        emoji: '🦸🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B9',
    shortcodes: ['villain', 'supervillain'],
    emoji: '🦹',
    order: 1327,
    group: 1,
    skins: [
      {
        hexcode: '1F9B9-1F3FB',
        emoji: '🦹🏻',
        tone: 1,
      },
      {
        hexcode: '1F9B9-1F3FC',
        emoji: '🦹🏼',
        tone: 2,
      },
      {
        hexcode: '1F9B9-1F3FD',
        emoji: '🦹🏽',
        tone: 3,
      },
      {
        hexcode: '1F9B9-1F3FE',
        emoji: '🦹🏾',
        tone: 4,
      },
      {
        hexcode: '1F9B9-1F3FF',
        emoji: '🦹🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B9-200D-2642-FE0F',
    shortcodes: ['man_villain'],
    emoji: '🦹‍♂️',
    order: 1333,
    group: 1,
    skins: [
      {
        hexcode: '1F9B9-1F3FB-200D-2642-FE0F',
        emoji: '🦹🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9B9-1F3FC-200D-2642-FE0F',
        emoji: '🦹🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9B9-1F3FD-200D-2642-FE0F',
        emoji: '🦹🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9B9-1F3FE-200D-2642-FE0F',
        emoji: '🦹🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9B9-1F3FF-200D-2642-FE0F',
        emoji: '🦹🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9B9-200D-2640-FE0F',
    shortcodes: ['woman_villain'],
    emoji: '🦹‍♀️',
    order: 1345,
    group: 1,
    skins: [
      {
        hexcode: '1F9B9-1F3FB-200D-2640-FE0F',
        emoji: '🦹🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9B9-1F3FC-200D-2640-FE0F',
        emoji: '🦹🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9B9-1F3FD-200D-2640-FE0F',
        emoji: '🦹🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9B9-1F3FE-200D-2640-FE0F',
        emoji: '🦹🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9B9-1F3FF-200D-2640-FE0F',
        emoji: '🦹🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D9',
    shortcodes: ['mage'],
    emoji: '🧙',
    order: 1357,
    group: 1,
    skins: [
      {
        hexcode: '1F9D9-1F3FB',
        emoji: '🧙🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D9-1F3FC',
        emoji: '🧙🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D9-1F3FD',
        emoji: '🧙🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D9-1F3FE',
        emoji: '🧙🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D9-1F3FF',
        emoji: '🧙🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D9-200D-2642-FE0F',
    shortcodes: ['man_mage'],
    emoji: '🧙‍♂️',
    order: 1363,
    group: 1,
    skins: [
      {
        hexcode: '1F9D9-1F3FB-200D-2642-FE0F',
        emoji: '🧙🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9D9-1F3FC-200D-2642-FE0F',
        emoji: '🧙🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9D9-1F3FD-200D-2642-FE0F',
        emoji: '🧙🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9D9-1F3FE-200D-2642-FE0F',
        emoji: '🧙🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9D9-1F3FF-200D-2642-FE0F',
        emoji: '🧙🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D9-200D-2640-FE0F',
    shortcodes: ['woman_mage'],
    emoji: '🧙‍♀️',
    order: 1375,
    group: 1,
    skins: [
      {
        hexcode: '1F9D9-1F3FB-200D-2640-FE0F',
        emoji: '🧙🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9D9-1F3FC-200D-2640-FE0F',
        emoji: '🧙🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9D9-1F3FD-200D-2640-FE0F',
        emoji: '🧙🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9D9-1F3FE-200D-2640-FE0F',
        emoji: '🧙🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9D9-1F3FF-200D-2640-FE0F',
        emoji: '🧙🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DA',
    shortcodes: ['fairy'],
    emoji: '🧚',
    order: 1387,
    group: 1,
    skins: [
      {
        hexcode: '1F9DA-1F3FB',
        emoji: '🧚🏻',
        tone: 1,
      },
      {
        hexcode: '1F9DA-1F3FC',
        emoji: '🧚🏼',
        tone: 2,
      },
      {
        hexcode: '1F9DA-1F3FD',
        emoji: '🧚🏽',
        tone: 3,
      },
      {
        hexcode: '1F9DA-1F3FE',
        emoji: '🧚🏾',
        tone: 4,
      },
      {
        hexcode: '1F9DA-1F3FF',
        emoji: '🧚🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DA-200D-2642-FE0F',
    shortcodes: ['man_fairy'],
    emoji: '🧚‍♂️',
    order: 1393,
    group: 1,
    skins: [
      {
        hexcode: '1F9DA-1F3FB-200D-2642-FE0F',
        emoji: '🧚🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9DA-1F3FC-200D-2642-FE0F',
        emoji: '🧚🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9DA-1F3FD-200D-2642-FE0F',
        emoji: '🧚🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9DA-1F3FE-200D-2642-FE0F',
        emoji: '🧚🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9DA-1F3FF-200D-2642-FE0F',
        emoji: '🧚🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DA-200D-2640-FE0F',
    shortcodes: ['woman_fairy'],
    emoji: '🧚‍♀️',
    order: 1405,
    group: 1,
    skins: [
      {
        hexcode: '1F9DA-1F3FB-200D-2640-FE0F',
        emoji: '🧚🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9DA-1F3FC-200D-2640-FE0F',
        emoji: '🧚🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9DA-1F3FD-200D-2640-FE0F',
        emoji: '🧚🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9DA-1F3FE-200D-2640-FE0F',
        emoji: '🧚🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9DA-1F3FF-200D-2640-FE0F',
        emoji: '🧚🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DB',
    shortcodes: ['vampire'],
    emoji: '🧛',
    order: 1417,
    group: 1,
    skins: [
      {
        hexcode: '1F9DB-1F3FB',
        emoji: '🧛🏻',
        tone: 1,
      },
      {
        hexcode: '1F9DB-1F3FC',
        emoji: '🧛🏼',
        tone: 2,
      },
      {
        hexcode: '1F9DB-1F3FD',
        emoji: '🧛🏽',
        tone: 3,
      },
      {
        hexcode: '1F9DB-1F3FE',
        emoji: '🧛🏾',
        tone: 4,
      },
      {
        hexcode: '1F9DB-1F3FF',
        emoji: '🧛🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DB-200D-2642-FE0F',
    shortcodes: ['man_vampire'],
    emoji: '🧛‍♂️',
    order: 1423,
    group: 1,
    skins: [
      {
        hexcode: '1F9DB-1F3FB-200D-2642-FE0F',
        emoji: '🧛🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9DB-1F3FC-200D-2642-FE0F',
        emoji: '🧛🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9DB-1F3FD-200D-2642-FE0F',
        emoji: '🧛🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9DB-1F3FE-200D-2642-FE0F',
        emoji: '🧛🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9DB-1F3FF-200D-2642-FE0F',
        emoji: '🧛🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DB-200D-2640-FE0F',
    shortcodes: ['woman_vampire'],
    emoji: '🧛‍♀️',
    order: 1435,
    group: 1,
    skins: [
      {
        hexcode: '1F9DB-1F3FB-200D-2640-FE0F',
        emoji: '🧛🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9DB-1F3FC-200D-2640-FE0F',
        emoji: '🧛🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9DB-1F3FD-200D-2640-FE0F',
        emoji: '🧛🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9DB-1F3FE-200D-2640-FE0F',
        emoji: '🧛🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9DB-1F3FF-200D-2640-FE0F',
        emoji: '🧛🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DC',
    shortcodes: ['merperson'],
    emoji: '🧜',
    order: 1447,
    group: 1,
    skins: [
      {
        hexcode: '1F9DC-1F3FB',
        emoji: '🧜🏻',
        tone: 1,
      },
      {
        hexcode: '1F9DC-1F3FC',
        emoji: '🧜🏼',
        tone: 2,
      },
      {
        hexcode: '1F9DC-1F3FD',
        emoji: '🧜🏽',
        tone: 3,
      },
      {
        hexcode: '1F9DC-1F3FE',
        emoji: '🧜🏾',
        tone: 4,
      },
      {
        hexcode: '1F9DC-1F3FF',
        emoji: '🧜🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DC-200D-2642-FE0F',
    shortcodes: ['merman'],
    emoji: '🧜‍♂️',
    order: 1453,
    group: 1,
    skins: [
      {
        hexcode: '1F9DC-1F3FB-200D-2642-FE0F',
        emoji: '🧜🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9DC-1F3FC-200D-2642-FE0F',
        emoji: '🧜🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9DC-1F3FD-200D-2642-FE0F',
        emoji: '🧜🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9DC-1F3FE-200D-2642-FE0F',
        emoji: '🧜🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9DC-1F3FF-200D-2642-FE0F',
        emoji: '🧜🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DC-200D-2640-FE0F',
    shortcodes: ['mermaid'],
    emoji: '🧜‍♀️',
    order: 1465,
    group: 1,
    skins: [
      {
        hexcode: '1F9DC-1F3FB-200D-2640-FE0F',
        emoji: '🧜🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9DC-1F3FC-200D-2640-FE0F',
        emoji: '🧜🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9DC-1F3FD-200D-2640-FE0F',
        emoji: '🧜🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9DC-1F3FE-200D-2640-FE0F',
        emoji: '🧜🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9DC-1F3FF-200D-2640-FE0F',
        emoji: '🧜🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DD',
    shortcodes: ['elf'],
    emoji: '🧝',
    order: 1477,
    group: 1,
    skins: [
      {
        hexcode: '1F9DD-1F3FB',
        emoji: '🧝🏻',
        tone: 1,
      },
      {
        hexcode: '1F9DD-1F3FC',
        emoji: '🧝🏼',
        tone: 2,
      },
      {
        hexcode: '1F9DD-1F3FD',
        emoji: '🧝🏽',
        tone: 3,
      },
      {
        hexcode: '1F9DD-1F3FE',
        emoji: '🧝🏾',
        tone: 4,
      },
      {
        hexcode: '1F9DD-1F3FF',
        emoji: '🧝🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DD-200D-2642-FE0F',
    shortcodes: ['man_elf'],
    emoji: '🧝‍♂️',
    order: 1483,
    group: 1,
    skins: [
      {
        hexcode: '1F9DD-1F3FB-200D-2642-FE0F',
        emoji: '🧝🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9DD-1F3FC-200D-2642-FE0F',
        emoji: '🧝🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9DD-1F3FD-200D-2642-FE0F',
        emoji: '🧝🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9DD-1F3FE-200D-2642-FE0F',
        emoji: '🧝🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9DD-1F3FF-200D-2642-FE0F',
        emoji: '🧝🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DD-200D-2640-FE0F',
    shortcodes: ['woman_elf'],
    emoji: '🧝‍♀️',
    order: 1495,
    group: 1,
    skins: [
      {
        hexcode: '1F9DD-1F3FB-200D-2640-FE0F',
        emoji: '🧝🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9DD-1F3FC-200D-2640-FE0F',
        emoji: '🧝🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9DD-1F3FD-200D-2640-FE0F',
        emoji: '🧝🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9DD-1F3FE-200D-2640-FE0F',
        emoji: '🧝🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9DD-1F3FF-200D-2640-FE0F',
        emoji: '🧝🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9DE',
    shortcodes: ['genie'],
    emoji: '🧞',
    order: 1507,
    group: 1,
  },
  {
    hexcode: '1F9DE-200D-2642-FE0F',
    shortcodes: ['man_genie'],
    emoji: '🧞‍♂️',
    order: 1508,
    group: 1,
  },
  {
    hexcode: '1F9DE-200D-2640-FE0F',
    shortcodes: ['woman_genie'],
    emoji: '🧞‍♀️',
    order: 1510,
    group: 1,
  },
  {
    hexcode: '1F9DF',
    shortcodes: ['zombie'],
    emoji: '🧟',
    order: 1512,
    group: 1,
  },
  {
    hexcode: '1F9DF-200D-2642-FE0F',
    shortcodes: ['man_zombie'],
    emoji: '🧟‍♂️',
    order: 1513,
    group: 1,
  },
  {
    hexcode: '1F9DF-200D-2640-FE0F',
    shortcodes: ['woman_zombie'],
    emoji: '🧟‍♀️',
    order: 1515,
    group: 1,
  },
  {
    hexcode: '1F486',
    shortcodes: ['person_getting_massage'],
    emoji: '💆',
    order: 1517,
    group: 1,
    skins: [
      {
        hexcode: '1F486-1F3FB',
        emoji: '💆🏻',
        tone: 1,
      },
      {
        hexcode: '1F486-1F3FC',
        emoji: '💆🏼',
        tone: 2,
      },
      {
        hexcode: '1F486-1F3FD',
        emoji: '💆🏽',
        tone: 3,
      },
      {
        hexcode: '1F486-1F3FE',
        emoji: '💆🏾',
        tone: 4,
      },
      {
        hexcode: '1F486-1F3FF',
        emoji: '💆🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F486-200D-2642-FE0F',
    shortcodes: ['man_getting_face_massage'],
    emoji: '💆‍♂️',
    order: 1523,
    group: 1,
    skins: [
      {
        hexcode: '1F486-1F3FB-200D-2642-FE0F',
        emoji: '💆🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F486-1F3FC-200D-2642-FE0F',
        emoji: '💆🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F486-1F3FD-200D-2642-FE0F',
        emoji: '💆🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F486-1F3FE-200D-2642-FE0F',
        emoji: '💆🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F486-1F3FF-200D-2642-FE0F',
        emoji: '💆🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F486-200D-2640-FE0F',
    shortcodes: ['woman_getting_face_massage'],
    emoji: '💆‍♀️',
    order: 1535,
    group: 1,
    skins: [
      {
        hexcode: '1F486-1F3FB-200D-2640-FE0F',
        emoji: '💆🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F486-1F3FC-200D-2640-FE0F',
        emoji: '💆🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F486-1F3FD-200D-2640-FE0F',
        emoji: '💆🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F486-1F3FE-200D-2640-FE0F',
        emoji: '💆🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F486-1F3FF-200D-2640-FE0F',
        emoji: '💆🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F487',
    shortcodes: ['person_getting_haircut'],
    emoji: '💇',
    order: 1547,
    group: 1,
    skins: [
      {
        hexcode: '1F487-1F3FB',
        emoji: '💇🏻',
        tone: 1,
      },
      {
        hexcode: '1F487-1F3FC',
        emoji: '💇🏼',
        tone: 2,
      },
      {
        hexcode: '1F487-1F3FD',
        emoji: '💇🏽',
        tone: 3,
      },
      {
        hexcode: '1F487-1F3FE',
        emoji: '💇🏾',
        tone: 4,
      },
      {
        hexcode: '1F487-1F3FF',
        emoji: '💇🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F487-200D-2642-FE0F',
    shortcodes: ['man_getting_haircut'],
    emoji: '💇‍♂️',
    order: 1553,
    group: 1,
    skins: [
      {
        hexcode: '1F487-1F3FB-200D-2642-FE0F',
        emoji: '💇🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F487-1F3FC-200D-2642-FE0F',
        emoji: '💇🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F487-1F3FD-200D-2642-FE0F',
        emoji: '💇🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F487-1F3FE-200D-2642-FE0F',
        emoji: '💇🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F487-1F3FF-200D-2642-FE0F',
        emoji: '💇🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F487-200D-2640-FE0F',
    shortcodes: ['woman_getting_haircut'],
    emoji: '💇‍♀️',
    order: 1565,
    group: 1,
    skins: [
      {
        hexcode: '1F487-1F3FB-200D-2640-FE0F',
        emoji: '💇🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F487-1F3FC-200D-2640-FE0F',
        emoji: '💇🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F487-1F3FD-200D-2640-FE0F',
        emoji: '💇🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F487-1F3FE-200D-2640-FE0F',
        emoji: '💇🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F487-1F3FF-200D-2640-FE0F',
        emoji: '💇🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B6',
    shortcodes: ['person_walking'],
    emoji: '🚶',
    order: 1577,
    group: 1,
    skins: [
      {
        hexcode: '1F6B6-1F3FB',
        emoji: '🚶🏻',
        tone: 1,
      },
      {
        hexcode: '1F6B6-1F3FC',
        emoji: '🚶🏼',
        tone: 2,
      },
      {
        hexcode: '1F6B6-1F3FD',
        emoji: '🚶🏽',
        tone: 3,
      },
      {
        hexcode: '1F6B6-1F3FE',
        emoji: '🚶🏾',
        tone: 4,
      },
      {
        hexcode: '1F6B6-1F3FF',
        emoji: '🚶🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B6-200D-2642-FE0F',
    shortcodes: ['man_walking'],
    emoji: '🚶‍♂️',
    order: 1583,
    group: 1,
    skins: [
      {
        hexcode: '1F6B6-1F3FB-200D-2642-FE0F',
        emoji: '🚶🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F6B6-1F3FC-200D-2642-FE0F',
        emoji: '🚶🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F6B6-1F3FD-200D-2642-FE0F',
        emoji: '🚶🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F6B6-1F3FE-200D-2642-FE0F',
        emoji: '🚶🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F6B6-1F3FF-200D-2642-FE0F',
        emoji: '🚶🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B6-200D-2640-FE0F',
    shortcodes: ['woman_walking'],
    emoji: '🚶‍♀️',
    order: 1595,
    group: 1,
    skins: [
      {
        hexcode: '1F6B6-1F3FB-200D-2640-FE0F',
        emoji: '🚶🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F6B6-1F3FC-200D-2640-FE0F',
        emoji: '🚶🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F6B6-1F3FD-200D-2640-FE0F',
        emoji: '🚶🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F6B6-1F3FE-200D-2640-FE0F',
        emoji: '🚶🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F6B6-1F3FF-200D-2640-FE0F',
        emoji: '🚶🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CD',
    shortcodes: ['person_standing'],
    emoji: '🧍',
    order: 1607,
    group: 1,
    skins: [
      {
        hexcode: '1F9CD-1F3FB',
        emoji: '🧍🏻',
        tone: 1,
      },
      {
        hexcode: '1F9CD-1F3FC',
        emoji: '🧍🏼',
        tone: 2,
      },
      {
        hexcode: '1F9CD-1F3FD',
        emoji: '🧍🏽',
        tone: 3,
      },
      {
        hexcode: '1F9CD-1F3FE',
        emoji: '🧍🏾',
        tone: 4,
      },
      {
        hexcode: '1F9CD-1F3FF',
        emoji: '🧍🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CD-200D-2642-FE0F',
    shortcodes: ['man_standing'],
    emoji: '🧍‍♂️',
    order: 1613,
    group: 1,
    skins: [
      {
        hexcode: '1F9CD-1F3FB-200D-2642-FE0F',
        emoji: '🧍🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9CD-1F3FC-200D-2642-FE0F',
        emoji: '🧍🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9CD-1F3FD-200D-2642-FE0F',
        emoji: '🧍🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9CD-1F3FE-200D-2642-FE0F',
        emoji: '🧍🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9CD-1F3FF-200D-2642-FE0F',
        emoji: '🧍🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CD-200D-2640-FE0F',
    shortcodes: ['woman_standing'],
    emoji: '🧍‍♀️',
    order: 1625,
    group: 1,
    skins: [
      {
        hexcode: '1F9CD-1F3FB-200D-2640-FE0F',
        emoji: '🧍🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9CD-1F3FC-200D-2640-FE0F',
        emoji: '🧍🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9CD-1F3FD-200D-2640-FE0F',
        emoji: '🧍🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9CD-1F3FE-200D-2640-FE0F',
        emoji: '🧍🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9CD-1F3FF-200D-2640-FE0F',
        emoji: '🧍🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CE',
    shortcodes: ['person_kneeling'],
    emoji: '🧎',
    order: 1637,
    group: 1,
    skins: [
      {
        hexcode: '1F9CE-1F3FB',
        emoji: '🧎🏻',
        tone: 1,
      },
      {
        hexcode: '1F9CE-1F3FC',
        emoji: '🧎🏼',
        tone: 2,
      },
      {
        hexcode: '1F9CE-1F3FD',
        emoji: '🧎🏽',
        tone: 3,
      },
      {
        hexcode: '1F9CE-1F3FE',
        emoji: '🧎🏾',
        tone: 4,
      },
      {
        hexcode: '1F9CE-1F3FF',
        emoji: '🧎🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CE-200D-2642-FE0F',
    shortcodes: ['man_kneeling'],
    emoji: '🧎‍♂️',
    order: 1643,
    group: 1,
    skins: [
      {
        hexcode: '1F9CE-1F3FB-200D-2642-FE0F',
        emoji: '🧎🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9CE-1F3FC-200D-2642-FE0F',
        emoji: '🧎🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9CE-1F3FD-200D-2642-FE0F',
        emoji: '🧎🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9CE-1F3FE-200D-2642-FE0F',
        emoji: '🧎🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9CE-1F3FF-200D-2642-FE0F',
        emoji: '🧎🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9CE-200D-2640-FE0F',
    shortcodes: ['woman_kneeling'],
    emoji: '🧎‍♀️',
    order: 1655,
    group: 1,
    skins: [
      {
        hexcode: '1F9CE-1F3FB-200D-2640-FE0F',
        emoji: '🧎🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9CE-1F3FC-200D-2640-FE0F',
        emoji: '🧎🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9CE-1F3FD-200D-2640-FE0F',
        emoji: '🧎🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9CE-1F3FE-200D-2640-FE0F',
        emoji: '🧎🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9CE-1F3FF-200D-2640-FE0F',
        emoji: '🧎🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9AF',
    shortcodes: ['man_probing_cane'],
    emoji: '👨‍🦯',
    order: 1667,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9AF',
        emoji: '👨🏻‍🦯',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9AF',
        emoji: '👨🏼‍🦯',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9AF',
        emoji: '👨🏽‍🦯',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9AF',
        emoji: '👨🏾‍🦯',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9AF',
        emoji: '👨🏿‍🦯',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9AF',
    shortcodes: ['woman_probing_cane'],
    emoji: '👩‍🦯',
    order: 1673,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9AF',
        emoji: '👩🏻‍🦯',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9AF',
        emoji: '👩🏼‍🦯',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9AF',
        emoji: '👩🏽‍🦯',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9AF',
        emoji: '👩🏾‍🦯',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9AF',
        emoji: '👩🏿‍🦯',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9BC',
    shortcodes: ['man_motor_wheelchair'],
    emoji: '👨‍🦼',
    order: 1679,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9BC',
        emoji: '👨🏻‍🦼',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9BC',
        emoji: '👨🏼‍🦼',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9BC',
        emoji: '👨🏽‍🦼',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9BC',
        emoji: '👨🏾‍🦼',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9BC',
        emoji: '👨🏿‍🦼',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9BC',
    shortcodes: ['woman_motor_wheelchair'],
    emoji: '👩‍🦼',
    order: 1685,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9BC',
        emoji: '👩🏻‍🦼',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9BC',
        emoji: '👩🏼‍🦼',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9BC',
        emoji: '👩🏽‍🦼',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9BC',
        emoji: '👩🏾‍🦼',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9BC',
        emoji: '👩🏿‍🦼',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F468-200D-1F9BD',
    shortcodes: ['man_wheelchair'],
    emoji: '👨‍🦽',
    order: 1691,
    group: 1,
    skins: [
      {
        hexcode: '1F468-1F3FB-200D-1F9BD',
        emoji: '👨🏻‍🦽',
        tone: 1,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F9BD',
        emoji: '👨🏼‍🦽',
        tone: 2,
      },
      {
        hexcode: '1F468-1F3FD-200D-1F9BD',
        emoji: '👨🏽‍🦽',
        tone: 3,
      },
      {
        hexcode: '1F468-1F3FE-200D-1F9BD',
        emoji: '👨🏾‍🦽',
        tone: 4,
      },
      {
        hexcode: '1F468-1F3FF-200D-1F9BD',
        emoji: '👨🏿‍🦽',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F469-200D-1F9BD',
    shortcodes: ['woman_wheelchair'],
    emoji: '👩‍🦽',
    order: 1697,
    group: 1,
    skins: [
      {
        hexcode: '1F469-1F3FB-200D-1F9BD',
        emoji: '👩🏻‍🦽',
        tone: 1,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F9BD',
        emoji: '👩🏼‍🦽',
        tone: 2,
      },
      {
        hexcode: '1F469-1F3FD-200D-1F9BD',
        emoji: '👩🏽‍🦽',
        tone: 3,
      },
      {
        hexcode: '1F469-1F3FE-200D-1F9BD',
        emoji: '👩🏾‍🦽',
        tone: 4,
      },
      {
        hexcode: '1F469-1F3FF-200D-1F9BD',
        emoji: '👩🏿‍🦽',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C3',
    shortcodes: ['person_running'],
    emoji: '🏃',
    order: 1703,
    group: 1,
    skins: [
      {
        hexcode: '1F3C3-1F3FB',
        emoji: '🏃🏻',
        tone: 1,
      },
      {
        hexcode: '1F3C3-1F3FC',
        emoji: '🏃🏼',
        tone: 2,
      },
      {
        hexcode: '1F3C3-1F3FD',
        emoji: '🏃🏽',
        tone: 3,
      },
      {
        hexcode: '1F3C3-1F3FE',
        emoji: '🏃🏾',
        tone: 4,
      },
      {
        hexcode: '1F3C3-1F3FF',
        emoji: '🏃🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C3-200D-2642-FE0F',
    shortcodes: ['man_running'],
    emoji: '🏃‍♂️',
    order: 1709,
    group: 1,
    skins: [
      {
        hexcode: '1F3C3-1F3FB-200D-2642-FE0F',
        emoji: '🏃🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F3C3-1F3FC-200D-2642-FE0F',
        emoji: '🏃🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F3C3-1F3FD-200D-2642-FE0F',
        emoji: '🏃🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F3C3-1F3FE-200D-2642-FE0F',
        emoji: '🏃🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F3C3-1F3FF-200D-2642-FE0F',
        emoji: '🏃🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C3-200D-2640-FE0F',
    shortcodes: ['woman_running'],
    emoji: '🏃‍♀️',
    order: 1721,
    group: 1,
    skins: [
      {
        hexcode: '1F3C3-1F3FB-200D-2640-FE0F',
        emoji: '🏃🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F3C3-1F3FC-200D-2640-FE0F',
        emoji: '🏃🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F3C3-1F3FD-200D-2640-FE0F',
        emoji: '🏃🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F3C3-1F3FE-200D-2640-FE0F',
        emoji: '🏃🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F3C3-1F3FF-200D-2640-FE0F',
        emoji: '🏃🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F483',
    shortcodes: ['dancer', 'woman_dancing'],
    emoji: '💃',
    order: 1733,
    group: 1,
    skins: [
      {
        hexcode: '1F483-1F3FB',
        emoji: '💃🏻',
        tone: 1,
      },
      {
        hexcode: '1F483-1F3FC',
        emoji: '💃🏼',
        tone: 2,
      },
      {
        hexcode: '1F483-1F3FD',
        emoji: '💃🏽',
        tone: 3,
      },
      {
        hexcode: '1F483-1F3FE',
        emoji: '💃🏾',
        tone: 4,
      },
      {
        hexcode: '1F483-1F3FF',
        emoji: '💃🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F57A',
    shortcodes: ['man_dancing'],
    emoji: '🕺',
    order: 1739,
    group: 1,
    skins: [
      {
        hexcode: '1F57A-1F3FB',
        emoji: '🕺🏻',
        tone: 1,
      },
      {
        hexcode: '1F57A-1F3FC',
        emoji: '🕺🏼',
        tone: 2,
      },
      {
        hexcode: '1F57A-1F3FD',
        emoji: '🕺🏽',
        tone: 3,
      },
      {
        hexcode: '1F57A-1F3FE',
        emoji: '🕺🏾',
        tone: 4,
      },
      {
        hexcode: '1F57A-1F3FF',
        emoji: '🕺🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F574',
    shortcodes: ['levitate'],
    emoji: '🕴️',
    order: 1746,
    group: 1,
    skins: [
      {
        hexcode: '1F574-1F3FB',
        emoji: '🕴🏻',
        tone: 1,
      },
      {
        hexcode: '1F574-1F3FC',
        emoji: '🕴🏼',
        tone: 2,
      },
      {
        hexcode: '1F574-1F3FD',
        emoji: '🕴🏽',
        tone: 3,
      },
      {
        hexcode: '1F574-1F3FE',
        emoji: '🕴🏾',
        tone: 4,
      },
      {
        hexcode: '1F574-1F3FF',
        emoji: '🕴🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F46F',
    shortcodes: ['people_bunny_ears_partying'],
    emoji: '👯',
    order: 1752,
    group: 1,
  },
  {
    hexcode: '1F46F-200D-2642-FE0F',
    shortcodes: ['men_bunny_ears_partying'],
    emoji: '👯‍♂️',
    order: 1753,
    group: 1,
  },
  {
    hexcode: '1F46F-200D-2640-FE0F',
    shortcodes: ['women_bunny_ears_partying'],
    emoji: '👯‍♀️',
    order: 1755,
    group: 1,
  },
  {
    hexcode: '1F9D6',
    shortcodes: ['person_steamy_room'],
    emoji: '🧖',
    order: 1757,
    group: 1,
    skins: [
      {
        hexcode: '1F9D6-1F3FB',
        emoji: '🧖🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D6-1F3FC',
        emoji: '🧖🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D6-1F3FD',
        emoji: '🧖🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D6-1F3FE',
        emoji: '🧖🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D6-1F3FF',
        emoji: '🧖🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D6-200D-2642-FE0F',
    shortcodes: ['man_steamy_room'],
    emoji: '🧖‍♂️',
    order: 1763,
    group: 1,
    skins: [
      {
        hexcode: '1F9D6-1F3FB-200D-2642-FE0F',
        emoji: '🧖🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9D6-1F3FC-200D-2642-FE0F',
        emoji: '🧖🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9D6-1F3FD-200D-2642-FE0F',
        emoji: '🧖🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9D6-1F3FE-200D-2642-FE0F',
        emoji: '🧖🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9D6-1F3FF-200D-2642-FE0F',
        emoji: '🧖🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D6-200D-2640-FE0F',
    shortcodes: ['woman_steamy_room'],
    emoji: '🧖‍♀️',
    order: 1775,
    group: 1,
    skins: [
      {
        hexcode: '1F9D6-1F3FB-200D-2640-FE0F',
        emoji: '🧖🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9D6-1F3FC-200D-2640-FE0F',
        emoji: '🧖🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9D6-1F3FD-200D-2640-FE0F',
        emoji: '🧖🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9D6-1F3FE-200D-2640-FE0F',
        emoji: '🧖🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9D6-1F3FF-200D-2640-FE0F',
        emoji: '🧖🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D7',
    shortcodes: ['person_climbing'],
    emoji: '🧗',
    order: 1787,
    group: 1,
    skins: [
      {
        hexcode: '1F9D7-1F3FB',
        emoji: '🧗🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D7-1F3FC',
        emoji: '🧗🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D7-1F3FD',
        emoji: '🧗🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D7-1F3FE',
        emoji: '🧗🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D7-1F3FF',
        emoji: '🧗🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D7-200D-2642-FE0F',
    shortcodes: ['man_climbing'],
    emoji: '🧗‍♂️',
    order: 1793,
    group: 1,
    skins: [
      {
        hexcode: '1F9D7-1F3FB-200D-2642-FE0F',
        emoji: '🧗🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9D7-1F3FC-200D-2642-FE0F',
        emoji: '🧗🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9D7-1F3FD-200D-2642-FE0F',
        emoji: '🧗🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9D7-1F3FE-200D-2642-FE0F',
        emoji: '🧗🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9D7-1F3FF-200D-2642-FE0F',
        emoji: '🧗🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D7-200D-2640-FE0F',
    shortcodes: ['woman_climbing'],
    emoji: '🧗‍♀️',
    order: 1805,
    group: 1,
    skins: [
      {
        hexcode: '1F9D7-1F3FB-200D-2640-FE0F',
        emoji: '🧗🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9D7-1F3FC-200D-2640-FE0F',
        emoji: '🧗🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9D7-1F3FD-200D-2640-FE0F',
        emoji: '🧗🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9D7-1F3FE-200D-2640-FE0F',
        emoji: '🧗🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9D7-1F3FF-200D-2640-FE0F',
        emoji: '🧗🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93A',
    shortcodes: ['person_fencing'],
    emoji: '🤺',
    order: 1817,
    group: 1,
  },
  {
    hexcode: '1F3C7',
    shortcodes: ['horse_racing'],
    emoji: '🏇',
    order: 1818,
    group: 1,
    skins: [
      {
        hexcode: '1F3C7-1F3FB',
        emoji: '🏇🏻',
        tone: 1,
      },
      {
        hexcode: '1F3C7-1F3FC',
        emoji: '🏇🏼',
        tone: 2,
      },
      {
        hexcode: '1F3C7-1F3FD',
        emoji: '🏇🏽',
        tone: 3,
      },
      {
        hexcode: '1F3C7-1F3FE',
        emoji: '🏇🏾',
        tone: 4,
      },
      {
        hexcode: '1F3C7-1F3FF',
        emoji: '🏇🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '26F7',
    shortcodes: ['skier'],
    emoji: '⛷️',
    order: 1825,
    group: 1,
  },
  {
    hexcode: '1F3C2',
    shortcodes: ['snowboarder'],
    emoji: '🏂️',
    order: 1826,
    group: 1,
    skins: [
      {
        hexcode: '1F3C2-1F3FB',
        emoji: '🏂🏻',
        tone: 1,
      },
      {
        hexcode: '1F3C2-1F3FC',
        emoji: '🏂🏼',
        tone: 2,
      },
      {
        hexcode: '1F3C2-1F3FD',
        emoji: '🏂🏽',
        tone: 3,
      },
      {
        hexcode: '1F3C2-1F3FE',
        emoji: '🏂🏾',
        tone: 4,
      },
      {
        hexcode: '1F3C2-1F3FF',
        emoji: '🏂🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CC',
    shortcodes: ['person_golfing'],
    emoji: '🏌️',
    order: 1833,
    group: 1,
    skins: [
      {
        hexcode: '1F3CC-1F3FB',
        emoji: '🏌🏻',
        tone: 1,
      },
      {
        hexcode: '1F3CC-1F3FC',
        emoji: '🏌🏼',
        tone: 2,
      },
      {
        hexcode: '1F3CC-1F3FD',
        emoji: '🏌🏽',
        tone: 3,
      },
      {
        hexcode: '1F3CC-1F3FE',
        emoji: '🏌🏾',
        tone: 4,
      },
      {
        hexcode: '1F3CC-1F3FF',
        emoji: '🏌🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CC-FE0F-200D-2642-FE0F',
    shortcodes: ['man_golfing'],
    emoji: '🏌️‍♂️',
    order: 1839,
    group: 1,
    skins: [
      {
        hexcode: '1F3CC-1F3FB-200D-2642-FE0F',
        emoji: '🏌🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F3CC-1F3FC-200D-2642-FE0F',
        emoji: '🏌🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F3CC-1F3FD-200D-2642-FE0F',
        emoji: '🏌🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F3CC-1F3FE-200D-2642-FE0F',
        emoji: '🏌🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F3CC-1F3FF-200D-2642-FE0F',
        emoji: '🏌🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CC-FE0F-200D-2640-FE0F',
    shortcodes: ['woman_golfing'],
    emoji: '🏌️‍♀️',
    order: 1853,
    group: 1,
    skins: [
      {
        hexcode: '1F3CC-1F3FB-200D-2640-FE0F',
        emoji: '🏌🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F3CC-1F3FC-200D-2640-FE0F',
        emoji: '🏌🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F3CC-1F3FD-200D-2640-FE0F',
        emoji: '🏌🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F3CC-1F3FE-200D-2640-FE0F',
        emoji: '🏌🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F3CC-1F3FF-200D-2640-FE0F',
        emoji: '🏌🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C4',
    shortcodes: ['person_surfing'],
    emoji: '🏄️',
    order: 1867,
    group: 1,
    skins: [
      {
        hexcode: '1F3C4-1F3FB',
        emoji: '🏄🏻',
        tone: 1,
      },
      {
        hexcode: '1F3C4-1F3FC',
        emoji: '🏄🏼',
        tone: 2,
      },
      {
        hexcode: '1F3C4-1F3FD',
        emoji: '🏄🏽',
        tone: 3,
      },
      {
        hexcode: '1F3C4-1F3FE',
        emoji: '🏄🏾',
        tone: 4,
      },
      {
        hexcode: '1F3C4-1F3FF',
        emoji: '🏄🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C4-200D-2642-FE0F',
    shortcodes: ['man_surfing'],
    emoji: '🏄‍♂️',
    order: 1873,
    group: 1,
    skins: [
      {
        hexcode: '1F3C4-1F3FB-200D-2642-FE0F',
        emoji: '🏄🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F3C4-1F3FC-200D-2642-FE0F',
        emoji: '🏄🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F3C4-1F3FD-200D-2642-FE0F',
        emoji: '🏄🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F3C4-1F3FE-200D-2642-FE0F',
        emoji: '🏄🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F3C4-1F3FF-200D-2642-FE0F',
        emoji: '🏄🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3C4-200D-2640-FE0F',
    shortcodes: ['woman_surfing'],
    emoji: '🏄‍♀️',
    order: 1885,
    group: 1,
    skins: [
      {
        hexcode: '1F3C4-1F3FB-200D-2640-FE0F',
        emoji: '🏄🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F3C4-1F3FC-200D-2640-FE0F',
        emoji: '🏄🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F3C4-1F3FD-200D-2640-FE0F',
        emoji: '🏄🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F3C4-1F3FE-200D-2640-FE0F',
        emoji: '🏄🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F3C4-1F3FF-200D-2640-FE0F',
        emoji: '🏄🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6A3',
    shortcodes: ['person_rowing_boat'],
    emoji: '🚣',
    order: 1897,
    group: 1,
    skins: [
      {
        hexcode: '1F6A3-1F3FB',
        emoji: '🚣🏻',
        tone: 1,
      },
      {
        hexcode: '1F6A3-1F3FC',
        emoji: '🚣🏼',
        tone: 2,
      },
      {
        hexcode: '1F6A3-1F3FD',
        emoji: '🚣🏽',
        tone: 3,
      },
      {
        hexcode: '1F6A3-1F3FE',
        emoji: '🚣🏾',
        tone: 4,
      },
      {
        hexcode: '1F6A3-1F3FF',
        emoji: '🚣🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6A3-200D-2642-FE0F',
    shortcodes: ['man_rowing_boat'],
    emoji: '🚣‍♂️',
    order: 1903,
    group: 1,
    skins: [
      {
        hexcode: '1F6A3-1F3FB-200D-2642-FE0F',
        emoji: '🚣🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F6A3-1F3FC-200D-2642-FE0F',
        emoji: '🚣🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F6A3-1F3FD-200D-2642-FE0F',
        emoji: '🚣🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F6A3-1F3FE-200D-2642-FE0F',
        emoji: '🚣🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F6A3-1F3FF-200D-2642-FE0F',
        emoji: '🚣🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6A3-200D-2640-FE0F',
    shortcodes: ['woman_rowing_boat'],
    emoji: '🚣‍♀️',
    order: 1915,
    group: 1,
    skins: [
      {
        hexcode: '1F6A3-1F3FB-200D-2640-FE0F',
        emoji: '🚣🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F6A3-1F3FC-200D-2640-FE0F',
        emoji: '🚣🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F6A3-1F3FD-200D-2640-FE0F',
        emoji: '🚣🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F6A3-1F3FE-200D-2640-FE0F',
        emoji: '🚣🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F6A3-1F3FF-200D-2640-FE0F',
        emoji: '🚣🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CA',
    shortcodes: ['person_swimming'],
    emoji: '🏊️',
    order: 1927,
    group: 1,
    skins: [
      {
        hexcode: '1F3CA-1F3FB',
        emoji: '🏊🏻',
        tone: 1,
      },
      {
        hexcode: '1F3CA-1F3FC',
        emoji: '🏊🏼',
        tone: 2,
      },
      {
        hexcode: '1F3CA-1F3FD',
        emoji: '🏊🏽',
        tone: 3,
      },
      {
        hexcode: '1F3CA-1F3FE',
        emoji: '🏊🏾',
        tone: 4,
      },
      {
        hexcode: '1F3CA-1F3FF',
        emoji: '🏊🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CA-200D-2642-FE0F',
    shortcodes: ['man_swimming'],
    emoji: '🏊‍♂️',
    order: 1933,
    group: 1,
    skins: [
      {
        hexcode: '1F3CA-1F3FB-200D-2642-FE0F',
        emoji: '🏊🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F3CA-1F3FC-200D-2642-FE0F',
        emoji: '🏊🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F3CA-1F3FD-200D-2642-FE0F',
        emoji: '🏊🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F3CA-1F3FE-200D-2642-FE0F',
        emoji: '🏊🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F3CA-1F3FF-200D-2642-FE0F',
        emoji: '🏊🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CA-200D-2640-FE0F',
    shortcodes: ['woman_swimming'],
    emoji: '🏊‍♀️',
    order: 1945,
    group: 1,
    skins: [
      {
        hexcode: '1F3CA-1F3FB-200D-2640-FE0F',
        emoji: '🏊🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F3CA-1F3FC-200D-2640-FE0F',
        emoji: '🏊🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F3CA-1F3FD-200D-2640-FE0F',
        emoji: '🏊🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F3CA-1F3FE-200D-2640-FE0F',
        emoji: '🏊🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F3CA-1F3FF-200D-2640-FE0F',
        emoji: '🏊🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '26F9',
    shortcodes: ['person_bouncing_ball'],
    emoji: '⛹️',
    order: 1958,
    group: 1,
    skins: [
      {
        hexcode: '26F9-1F3FB',
        emoji: '⛹🏻',
        tone: 1,
      },
      {
        hexcode: '26F9-1F3FC',
        emoji: '⛹🏼',
        tone: 2,
      },
      {
        hexcode: '26F9-1F3FD',
        emoji: '⛹🏽',
        tone: 3,
      },
      {
        hexcode: '26F9-1F3FE',
        emoji: '⛹🏾',
        tone: 4,
      },
      {
        hexcode: '26F9-1F3FF',
        emoji: '⛹🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '26F9-FE0F-200D-2642-FE0F',
    shortcodes: ['man_bouncing_ball'],
    emoji: '⛹️‍♂️',
    order: 1964,
    group: 1,
    skins: [
      {
        hexcode: '26F9-1F3FB-200D-2642-FE0F',
        emoji: '⛹🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '26F9-1F3FC-200D-2642-FE0F',
        emoji: '⛹🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '26F9-1F3FD-200D-2642-FE0F',
        emoji: '⛹🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '26F9-1F3FE-200D-2642-FE0F',
        emoji: '⛹🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '26F9-1F3FF-200D-2642-FE0F',
        emoji: '⛹🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '26F9-FE0F-200D-2640-FE0F',
    shortcodes: ['woman_bouncing_ball'],
    emoji: '⛹️‍♀️',
    order: 1978,
    group: 1,
    skins: [
      {
        hexcode: '26F9-1F3FB-200D-2640-FE0F',
        emoji: '⛹🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '26F9-1F3FC-200D-2640-FE0F',
        emoji: '⛹🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '26F9-1F3FD-200D-2640-FE0F',
        emoji: '⛹🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '26F9-1F3FE-200D-2640-FE0F',
        emoji: '⛹🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '26F9-1F3FF-200D-2640-FE0F',
        emoji: '⛹🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CB',
    shortcodes: ['person_lifting_weights'],
    emoji: '🏋️',
    order: 1993,
    group: 1,
    skins: [
      {
        hexcode: '1F3CB-1F3FB',
        emoji: '🏋🏻',
        tone: 1,
      },
      {
        hexcode: '1F3CB-1F3FC',
        emoji: '🏋🏼',
        tone: 2,
      },
      {
        hexcode: '1F3CB-1F3FD',
        emoji: '🏋🏽',
        tone: 3,
      },
      {
        hexcode: '1F3CB-1F3FE',
        emoji: '🏋🏾',
        tone: 4,
      },
      {
        hexcode: '1F3CB-1F3FF',
        emoji: '🏋🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CB-FE0F-200D-2642-FE0F',
    shortcodes: ['man_lifting_weights'],
    emoji: '🏋️‍♂️',
    order: 1999,
    group: 1,
    skins: [
      {
        hexcode: '1F3CB-1F3FB-200D-2642-FE0F',
        emoji: '🏋🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F3CB-1F3FC-200D-2642-FE0F',
        emoji: '🏋🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F3CB-1F3FD-200D-2642-FE0F',
        emoji: '🏋🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F3CB-1F3FE-200D-2642-FE0F',
        emoji: '🏋🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F3CB-1F3FF-200D-2642-FE0F',
        emoji: '🏋🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F3CB-FE0F-200D-2640-FE0F',
    shortcodes: ['woman_lifting_weights'],
    emoji: '🏋️‍♀️',
    order: 2013,
    group: 1,
    skins: [
      {
        hexcode: '1F3CB-1F3FB-200D-2640-FE0F',
        emoji: '🏋🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F3CB-1F3FC-200D-2640-FE0F',
        emoji: '🏋🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F3CB-1F3FD-200D-2640-FE0F',
        emoji: '🏋🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F3CB-1F3FE-200D-2640-FE0F',
        emoji: '🏋🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F3CB-1F3FF-200D-2640-FE0F',
        emoji: '🏋🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B4',
    shortcodes: ['person_biking'],
    emoji: '🚴',
    order: 2027,
    group: 1,
    skins: [
      {
        hexcode: '1F6B4-1F3FB',
        emoji: '🚴🏻',
        tone: 1,
      },
      {
        hexcode: '1F6B4-1F3FC',
        emoji: '🚴🏼',
        tone: 2,
      },
      {
        hexcode: '1F6B4-1F3FD',
        emoji: '🚴🏽',
        tone: 3,
      },
      {
        hexcode: '1F6B4-1F3FE',
        emoji: '🚴🏾',
        tone: 4,
      },
      {
        hexcode: '1F6B4-1F3FF',
        emoji: '🚴🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B4-200D-2642-FE0F',
    shortcodes: ['man_biking'],
    emoji: '🚴‍♂️',
    order: 2033,
    group: 1,
    skins: [
      {
        hexcode: '1F6B4-1F3FB-200D-2642-FE0F',
        emoji: '🚴🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F6B4-1F3FC-200D-2642-FE0F',
        emoji: '🚴🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F6B4-1F3FD-200D-2642-FE0F',
        emoji: '🚴🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F6B4-1F3FE-200D-2642-FE0F',
        emoji: '🚴🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F6B4-1F3FF-200D-2642-FE0F',
        emoji: '🚴🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B4-200D-2640-FE0F',
    shortcodes: ['woman_biking'],
    emoji: '🚴‍♀️',
    order: 2045,
    group: 1,
    skins: [
      {
        hexcode: '1F6B4-1F3FB-200D-2640-FE0F',
        emoji: '🚴🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F6B4-1F3FC-200D-2640-FE0F',
        emoji: '🚴🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F6B4-1F3FD-200D-2640-FE0F',
        emoji: '🚴🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F6B4-1F3FE-200D-2640-FE0F',
        emoji: '🚴🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F6B4-1F3FF-200D-2640-FE0F',
        emoji: '🚴🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B5',
    shortcodes: ['person_mountain_biking'],
    emoji: '🚵',
    order: 2057,
    group: 1,
    skins: [
      {
        hexcode: '1F6B5-1F3FB',
        emoji: '🚵🏻',
        tone: 1,
      },
      {
        hexcode: '1F6B5-1F3FC',
        emoji: '🚵🏼',
        tone: 2,
      },
      {
        hexcode: '1F6B5-1F3FD',
        emoji: '🚵🏽',
        tone: 3,
      },
      {
        hexcode: '1F6B5-1F3FE',
        emoji: '🚵🏾',
        tone: 4,
      },
      {
        hexcode: '1F6B5-1F3FF',
        emoji: '🚵🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B5-200D-2642-FE0F',
    shortcodes: ['man_mountain_biking'],
    emoji: '🚵‍♂️',
    order: 2063,
    group: 1,
    skins: [
      {
        hexcode: '1F6B5-1F3FB-200D-2642-FE0F',
        emoji: '🚵🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F6B5-1F3FC-200D-2642-FE0F',
        emoji: '🚵🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F6B5-1F3FD-200D-2642-FE0F',
        emoji: '🚵🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F6B5-1F3FE-200D-2642-FE0F',
        emoji: '🚵🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F6B5-1F3FF-200D-2642-FE0F',
        emoji: '🚵🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6B5-200D-2640-FE0F',
    shortcodes: ['woman_mountain_biking'],
    emoji: '🚵‍♀️',
    order: 2075,
    group: 1,
    skins: [
      {
        hexcode: '1F6B5-1F3FB-200D-2640-FE0F',
        emoji: '🚵🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F6B5-1F3FC-200D-2640-FE0F',
        emoji: '🚵🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F6B5-1F3FD-200D-2640-FE0F',
        emoji: '🚵🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F6B5-1F3FE-200D-2640-FE0F',
        emoji: '🚵🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F6B5-1F3FF-200D-2640-FE0F',
        emoji: '🚵🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F938',
    shortcodes: ['person_cartwheel'],
    emoji: '🤸',
    order: 2087,
    group: 1,
    skins: [
      {
        hexcode: '1F938-1F3FB',
        emoji: '🤸🏻',
        tone: 1,
      },
      {
        hexcode: '1F938-1F3FC',
        emoji: '🤸🏼',
        tone: 2,
      },
      {
        hexcode: '1F938-1F3FD',
        emoji: '🤸🏽',
        tone: 3,
      },
      {
        hexcode: '1F938-1F3FE',
        emoji: '🤸🏾',
        tone: 4,
      },
      {
        hexcode: '1F938-1F3FF',
        emoji: '🤸🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F938-200D-2642-FE0F',
    shortcodes: ['man_cartwheeling'],
    emoji: '🤸‍♂️',
    order: 2093,
    group: 1,
    skins: [
      {
        hexcode: '1F938-1F3FB-200D-2642-FE0F',
        emoji: '🤸🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F938-1F3FC-200D-2642-FE0F',
        emoji: '🤸🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F938-1F3FD-200D-2642-FE0F',
        emoji: '🤸🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F938-1F3FE-200D-2642-FE0F',
        emoji: '🤸🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F938-1F3FF-200D-2642-FE0F',
        emoji: '🤸🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F938-200D-2640-FE0F',
    shortcodes: ['woman_cartwheeling'],
    emoji: '🤸‍♀️',
    order: 2105,
    group: 1,
    skins: [
      {
        hexcode: '1F938-1F3FB-200D-2640-FE0F',
        emoji: '🤸🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F938-1F3FC-200D-2640-FE0F',
        emoji: '🤸🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F938-1F3FD-200D-2640-FE0F',
        emoji: '🤸🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F938-1F3FE-200D-2640-FE0F',
        emoji: '🤸🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F938-1F3FF-200D-2640-FE0F',
        emoji: '🤸🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93C',
    shortcodes: ['people_wrestling'],
    emoji: '🤼',
    order: 2117,
    group: 1,
  },
  {
    hexcode: '1F93C-200D-2642-FE0F',
    shortcodes: ['men_wrestling'],
    emoji: '🤼‍♂️',
    order: 2118,
    group: 1,
  },
  {
    hexcode: '1F93C-200D-2640-FE0F',
    shortcodes: ['women_wrestling'],
    emoji: '🤼‍♀️',
    order: 2120,
    group: 1,
  },
  {
    hexcode: '1F93D',
    shortcodes: ['person_water_polo'],
    emoji: '🤽',
    order: 2122,
    group: 1,
    skins: [
      {
        hexcode: '1F93D-1F3FB',
        emoji: '🤽🏻',
        tone: 1,
      },
      {
        hexcode: '1F93D-1F3FC',
        emoji: '🤽🏼',
        tone: 2,
      },
      {
        hexcode: '1F93D-1F3FD',
        emoji: '🤽🏽',
        tone: 3,
      },
      {
        hexcode: '1F93D-1F3FE',
        emoji: '🤽🏾',
        tone: 4,
      },
      {
        hexcode: '1F93D-1F3FF',
        emoji: '🤽🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93D-200D-2642-FE0F',
    shortcodes: ['man_water_polo'],
    emoji: '🤽‍♂️',
    order: 2128,
    group: 1,
    skins: [
      {
        hexcode: '1F93D-1F3FB-200D-2642-FE0F',
        emoji: '🤽🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F93D-1F3FC-200D-2642-FE0F',
        emoji: '🤽🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F93D-1F3FD-200D-2642-FE0F',
        emoji: '🤽🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F93D-1F3FE-200D-2642-FE0F',
        emoji: '🤽🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F93D-1F3FF-200D-2642-FE0F',
        emoji: '🤽🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93D-200D-2640-FE0F',
    shortcodes: ['woman_water_polo'],
    emoji: '🤽‍♀️',
    order: 2140,
    group: 1,
    skins: [
      {
        hexcode: '1F93D-1F3FB-200D-2640-FE0F',
        emoji: '🤽🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F93D-1F3FC-200D-2640-FE0F',
        emoji: '🤽🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F93D-1F3FD-200D-2640-FE0F',
        emoji: '🤽🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F93D-1F3FE-200D-2640-FE0F',
        emoji: '🤽🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F93D-1F3FF-200D-2640-FE0F',
        emoji: '🤽🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93E',
    shortcodes: ['person_handball'],
    emoji: '🤾',
    order: 2152,
    group: 1,
    skins: [
      {
        hexcode: '1F93E-1F3FB',
        emoji: '🤾🏻',
        tone: 1,
      },
      {
        hexcode: '1F93E-1F3FC',
        emoji: '🤾🏼',
        tone: 2,
      },
      {
        hexcode: '1F93E-1F3FD',
        emoji: '🤾🏽',
        tone: 3,
      },
      {
        hexcode: '1F93E-1F3FE',
        emoji: '🤾🏾',
        tone: 4,
      },
      {
        hexcode: '1F93E-1F3FF',
        emoji: '🤾🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93E-200D-2642-FE0F',
    shortcodes: ['man_handball'],
    emoji: '🤾‍♂️',
    order: 2158,
    group: 1,
    skins: [
      {
        hexcode: '1F93E-1F3FB-200D-2642-FE0F',
        emoji: '🤾🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F93E-1F3FC-200D-2642-FE0F',
        emoji: '🤾🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F93E-1F3FD-200D-2642-FE0F',
        emoji: '🤾🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F93E-1F3FE-200D-2642-FE0F',
        emoji: '🤾🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F93E-1F3FF-200D-2642-FE0F',
        emoji: '🤾🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F93E-200D-2640-FE0F',
    shortcodes: ['woman_handball'],
    emoji: '🤾‍♀️',
    order: 2170,
    group: 1,
    skins: [
      {
        hexcode: '1F93E-1F3FB-200D-2640-FE0F',
        emoji: '🤾🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F93E-1F3FC-200D-2640-FE0F',
        emoji: '🤾🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F93E-1F3FD-200D-2640-FE0F',
        emoji: '🤾🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F93E-1F3FE-200D-2640-FE0F',
        emoji: '🤾🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F93E-1F3FF-200D-2640-FE0F',
        emoji: '🤾🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F939',
    shortcodes: ['person_juggling'],
    emoji: '🤹',
    order: 2182,
    group: 1,
    skins: [
      {
        hexcode: '1F939-1F3FB',
        emoji: '🤹🏻',
        tone: 1,
      },
      {
        hexcode: '1F939-1F3FC',
        emoji: '🤹🏼',
        tone: 2,
      },
      {
        hexcode: '1F939-1F3FD',
        emoji: '🤹🏽',
        tone: 3,
      },
      {
        hexcode: '1F939-1F3FE',
        emoji: '🤹🏾',
        tone: 4,
      },
      {
        hexcode: '1F939-1F3FF',
        emoji: '🤹🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F939-200D-2642-FE0F',
    shortcodes: ['man_juggling'],
    emoji: '🤹‍♂️',
    order: 2188,
    group: 1,
    skins: [
      {
        hexcode: '1F939-1F3FB-200D-2642-FE0F',
        emoji: '🤹🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F939-1F3FC-200D-2642-FE0F',
        emoji: '🤹🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F939-1F3FD-200D-2642-FE0F',
        emoji: '🤹🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F939-1F3FE-200D-2642-FE0F',
        emoji: '🤹🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F939-1F3FF-200D-2642-FE0F',
        emoji: '🤹🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F939-200D-2640-FE0F',
    shortcodes: ['woman_juggling'],
    emoji: '🤹‍♀️',
    order: 2200,
    group: 1,
    skins: [
      {
        hexcode: '1F939-1F3FB-200D-2640-FE0F',
        emoji: '🤹🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F939-1F3FC-200D-2640-FE0F',
        emoji: '🤹🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F939-1F3FD-200D-2640-FE0F',
        emoji: '🤹🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F939-1F3FE-200D-2640-FE0F',
        emoji: '🤹🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F939-1F3FF-200D-2640-FE0F',
        emoji: '🤹🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D8',
    shortcodes: ['person_lotus_position'],
    emoji: '🧘',
    order: 2212,
    group: 1,
    skins: [
      {
        hexcode: '1F9D8-1F3FB',
        emoji: '🧘🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D8-1F3FC',
        emoji: '🧘🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D8-1F3FD',
        emoji: '🧘🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D8-1F3FE',
        emoji: '🧘🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D8-1F3FF',
        emoji: '🧘🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D8-200D-2642-FE0F',
    shortcodes: ['man_lotus_position'],
    emoji: '🧘‍♂️',
    order: 2218,
    group: 1,
    skins: [
      {
        hexcode: '1F9D8-1F3FB-200D-2642-FE0F',
        emoji: '🧘🏻‍♂️',
        tone: 1,
      },
      {
        hexcode: '1F9D8-1F3FC-200D-2642-FE0F',
        emoji: '🧘🏼‍♂️',
        tone: 2,
      },
      {
        hexcode: '1F9D8-1F3FD-200D-2642-FE0F',
        emoji: '🧘🏽‍♂️',
        tone: 3,
      },
      {
        hexcode: '1F9D8-1F3FE-200D-2642-FE0F',
        emoji: '🧘🏾‍♂️',
        tone: 4,
      },
      {
        hexcode: '1F9D8-1F3FF-200D-2642-FE0F',
        emoji: '🧘🏿‍♂️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D8-200D-2640-FE0F',
    shortcodes: ['woman_lotus_position'],
    emoji: '🧘‍♀️',
    order: 2230,
    group: 1,
    skins: [
      {
        hexcode: '1F9D8-1F3FB-200D-2640-FE0F',
        emoji: '🧘🏻‍♀️',
        tone: 1,
      },
      {
        hexcode: '1F9D8-1F3FC-200D-2640-FE0F',
        emoji: '🧘🏼‍♀️',
        tone: 2,
      },
      {
        hexcode: '1F9D8-1F3FD-200D-2640-FE0F',
        emoji: '🧘🏽‍♀️',
        tone: 3,
      },
      {
        hexcode: '1F9D8-1F3FE-200D-2640-FE0F',
        emoji: '🧘🏾‍♀️',
        tone: 4,
      },
      {
        hexcode: '1F9D8-1F3FF-200D-2640-FE0F',
        emoji: '🧘🏿‍♀️',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6C0',
    shortcodes: ['bath'],
    emoji: '🛀',
    order: 2242,
    group: 1,
    skins: [
      {
        hexcode: '1F6C0-1F3FB',
        emoji: '🛀🏻',
        tone: 1,
      },
      {
        hexcode: '1F6C0-1F3FC',
        emoji: '🛀🏼',
        tone: 2,
      },
      {
        hexcode: '1F6C0-1F3FD',
        emoji: '🛀🏽',
        tone: 3,
      },
      {
        hexcode: '1F6C0-1F3FE',
        emoji: '🛀🏾',
        tone: 4,
      },
      {
        hexcode: '1F6C0-1F3FF',
        emoji: '🛀🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F6CC',
    shortcodes: ['in_bed'],
    emoji: '🛌',
    order: 2248,
    group: 1,
    skins: [
      {
        hexcode: '1F6CC-1F3FB',
        emoji: '🛌🏻',
        tone: 1,
      },
      {
        hexcode: '1F6CC-1F3FC',
        emoji: '🛌🏼',
        tone: 2,
      },
      {
        hexcode: '1F6CC-1F3FD',
        emoji: '🛌🏽',
        tone: 3,
      },
      {
        hexcode: '1F6CC-1F3FE',
        emoji: '🛌🏾',
        tone: 4,
      },
      {
        hexcode: '1F6CC-1F3FF',
        emoji: '🛌🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F9D1-200D-1F91D-200D-1F9D1',
    shortcodes: ['holding_hands_people'],
    emoji: '🧑‍🤝‍🧑',
    order: 2254,
    group: 1,
    skins: [
      {
        hexcode: '1F9D1-1F3FB-200D-1F91D-200D-1F9D1-1F3FB',
        emoji: '🧑🏻‍🤝‍🧑🏻',
        tone: 1,
      },
      {
        hexcode: '1F9D1-1F3FC-200D-1F91D-200D-1F9D1-1F3FB',
        emoji: '🧑🏼‍🤝‍🧑🏻',
        tone: [2, 1],
      },
      {
        hexcode: '1F9D1-1F3FC-200D-1F91D-200D-1F9D1-1F3FC',
        emoji: '🧑🏼‍🤝‍🧑🏼',
        tone: 2,
      },
      {
        hexcode: '1F9D1-1F3FD-200D-1F91D-200D-1F9D1-1F3FB',
        emoji: '🧑🏽‍🤝‍🧑🏻',
        tone: [3, 1],
      },
      {
        hexcode: '1F9D1-1F3FD-200D-1F91D-200D-1F9D1-1F3FC',
        emoji: '🧑🏽‍🤝‍🧑🏼',
        tone: [3, 2],
      },
      {
        hexcode: '1F9D1-1F3FD-200D-1F91D-200D-1F9D1-1F3FD',
        emoji: '🧑🏽‍🤝‍🧑🏽',
        tone: 3,
      },
      {
        hexcode: '1F9D1-1F3FE-200D-1F91D-200D-1F9D1-1F3FB',
        emoji: '🧑🏾‍🤝‍🧑🏻',
        tone: [4, 1],
      },
      {
        hexcode: '1F9D1-1F3FE-200D-1F91D-200D-1F9D1-1F3FC',
        emoji: '🧑🏾‍🤝‍🧑🏼',
        tone: [4, 2],
      },
      {
        hexcode: '1F9D1-1F3FE-200D-1F91D-200D-1F9D1-1F3FD',
        emoji: '🧑🏾‍🤝‍🧑🏽',
        tone: [4, 3],
      },
      {
        hexcode: '1F9D1-1F3FE-200D-1F91D-200D-1F9D1-1F3FE',
        emoji: '🧑🏾‍🤝‍🧑🏾',
        tone: 4,
      },
      {
        hexcode: '1F9D1-1F3FF-200D-1F91D-200D-1F9D1-1F3FB',
        emoji: '🧑🏿‍🤝‍🧑🏻',
        tone: [5, 1],
      },
      {
        hexcode: '1F9D1-1F3FF-200D-1F91D-200D-1F9D1-1F3FC',
        emoji: '🧑🏿‍🤝‍🧑🏼',
        tone: [5, 2],
      },
      {
        hexcode: '1F9D1-1F3FF-200D-1F91D-200D-1F9D1-1F3FD',
        emoji: '🧑🏿‍🤝‍🧑🏽',
        tone: [5, 3],
      },
      {
        hexcode: '1F9D1-1F3FF-200D-1F91D-200D-1F9D1-1F3FE',
        emoji: '🧑🏿‍🤝‍🧑🏾',
        tone: [5, 4],
      },
      {
        hexcode: '1F9D1-1F3FF-200D-1F91D-200D-1F9D1-1F3FF',
        emoji: '🧑🏿‍🤝‍🧑🏿',
        tone: 5,
      },
    ],
  },
  {
    hexcode: '1F46D',
    shortcodes: ['holding_hands_ww'],
    emoji: '👭',
    order: 2270,
    group: 1,
    skins: [
      {
        hexcode: '1F46D-1F3FB',
        emoji: '👭🏻',
        tone: 1,
      },
      {
        hexcode: '1F46D-1F3FC',
        emoji: '👭🏼',
        tone: 2,
      },
      {
        hexcode: '1F46D-1F3FD',
        emoji: '👭🏽',
        tone: 3,
      },
      {
        hexcode: '1F46D-1F3FE',
        emoji: '👭🏾',
        tone: 4,
      },
      {
        hexcode: '1F46D-1F3FF',
        emoji: '👭🏿',
        tone: 5,
      },
      {
        hexcode: '1F469-1F3FC-200D-1F91D-200D-1F469-1F3FB',
        emoji: '👩🏼‍🤝‍👩🏻',
        tone: [2, 1],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F469-1F3FB',
        emoji: '👩🏽‍🤝‍👩🏻',
        tone: [3, 1],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F469-1F3FC',
        emoji: '👩🏽‍🤝‍👩🏼',
        tone: [3, 2],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F469-1F3FB',
        emoji: '👩🏾‍🤝‍👩🏻',
        tone: [4, 1],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F469-1F3FC',
        emoji: '👩🏾‍🤝‍👩🏼',
        tone: [4, 2],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F469-1F3FD',
        emoji: '👩🏾‍🤝‍👩🏽',
        tone: [4, 3],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F469-1F3FB',
        emoji: '👩🏿‍🤝‍👩🏻',
        tone: [5, 1],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F469-1F3FC',
        emoji: '👩🏿‍🤝‍👩🏼',
        tone: [5, 2],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F469-1F3FD',
        emoji: '👩🏿‍🤝‍👩🏽',
        tone: [5, 3],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F469-1F3FE',
        emoji: '👩🏿‍🤝‍👩🏾',
        tone: [5, 4],
      },
    ],
  },
  {
    hexcode: '1F46B',
    shortcodes: ['holding_hands_mw', 'holding_hands_wm'],
    emoji: '👫',
    order: 2286,
    group: 1,
    skins: [
      {
        hexcode: '1F46B-1F3FB',
        emoji: '👫🏻',
        tone: 1,
      },
      {
        hexcode: '1F46B-1F3FC',
        emoji: '👫🏼',
        tone: 2,
      },
      {
        hexcode: '1F46B-1F3FD',
        emoji: '👫🏽',
        tone: 3,
      },
      {
        hexcode: '1F46B-1F3FE',
        emoji: '👫🏾',
        tone: 4,
      },
      {
        hexcode: '1F46B-1F3FF',
        emoji: '👫🏿',
        tone: 5,
      },
      {
        hexcode: '1F469-1F3FB-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👩🏻‍🤝‍👨🏼',
        tone: [1, 2],
      },
      {
        hexcode: '1F469-1F3FB-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👩🏻‍🤝‍👨🏽',
        tone: [1, 3],
      },
      {
        hexcode: '1F469-1F3FB-200D-1F91D-200D-1F468-1F3FE',
        emoji: '👩🏻‍🤝‍👨🏾',
        tone: [1, 4],
      },
      {
        hexcode: '1F469-1F3FB-200D-1F91D-200D-1F468-1F3FF',
        emoji: '👩🏻‍🤝‍👨🏿',
        tone: [1, 5],
      },
      {
        hexcode: '1F469-1F3FC-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👩🏼‍🤝‍👨🏻',
        tone: [2, 1],
      },
      {
        hexcode: '1F469-1F3FC-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👩🏼‍🤝‍👨🏽',
        tone: [2, 3],
      },
      {
        hexcode: '1F469-1F3FC-200D-1F91D-200D-1F468-1F3FE',
        emoji: '👩🏼‍🤝‍👨🏾',
        tone: [2, 4],
      },
      {
        hexcode: '1F469-1F3FC-200D-1F91D-200D-1F468-1F3FF',
        emoji: '👩🏼‍🤝‍👨🏿',
        tone: [2, 5],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👩🏽‍🤝‍👨🏻',
        tone: [3, 1],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👩🏽‍🤝‍👨🏼',
        tone: [3, 2],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F468-1F3FE',
        emoji: '👩🏽‍🤝‍👨🏾',
        tone: [3, 4],
      },
      {
        hexcode: '1F469-1F3FD-200D-1F91D-200D-1F468-1F3FF',
        emoji: '👩🏽‍🤝‍👨🏿',
        tone: [3, 5],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👩🏾‍🤝‍👨🏻',
        tone: [4, 1],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👩🏾‍🤝‍👨🏼',
        tone: [4, 2],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👩🏾‍🤝‍👨🏽',
        tone: [4, 3],
      },
      {
        hexcode: '1F469-1F3FE-200D-1F91D-200D-1F468-1F3FF',
        emoji: '👩🏾‍🤝‍👨🏿',
        tone: [4, 5],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👩🏿‍🤝‍👨🏻',
        tone: [5, 1],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👩🏿‍🤝‍👨🏼',
        tone: [5, 2],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👩🏿‍🤝‍👨🏽',
        tone: [5, 3],
      },
      {
        hexcode: '1F469-1F3FF-200D-1F91D-200D-1F468-1F3FE',
        emoji: '👩🏿‍🤝‍👨🏾',
        tone: [5, 4],
      },
    ],
  },
  {
    hexcode: '1F46C',
    shortcodes: ['holding_hands_mm'],
    emoji: '👬',
    order: 2312,
    group: 1,
    skins: [
      {
        hexcode: '1F46C-1F3FB',
        emoji: '👬🏻',
        tone: 1,
      },
      {
        hexcode: '1F46C-1F3FC',
        emoji: '👬🏼',
        tone: 2,
      },
      {
        hexcode: '1F46C-1F3FD',
        emoji: '👬🏽',
        tone: 3,
      },
      {
        hexcode: '1F46C-1F3FE',
        emoji: '👬🏾',
        tone: 4,
      },
      {
        hexcode: '1F46C-1F3FF',
        emoji: '👬🏿',
        tone: 5,
      },
      {
        hexcode: '1F468-1F3FC-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👨🏼‍🤝‍👨🏻',
        tone: [2, 1],
      },
      {
        hexcode: '1F468-1F3FD-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👨🏽‍🤝‍👨🏻',
        tone: [3, 1],
      },
      {
        hexcode: '1F468-1F3FD-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👨🏽‍🤝‍👨🏼',
        tone: [3, 2],
      },
      {
        hexcode: '1F468-1F3FE-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👨🏾‍🤝‍👨🏻',
        tone: [4, 1],
      },
      {
        hexcode: '1F468-1F3FE-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👨🏾‍🤝‍👨🏼',
        tone: [4, 2],
      },
      {
        hexcode: '1F468-1F3FE-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👨🏾‍🤝‍👨🏽',
        tone: [4, 3],
      },
      {
        hexcode: '1F468-1F3FF-200D-1F91D-200D-1F468-1F3FB',
        emoji: '👨🏿‍🤝‍👨🏻',
        tone: [5, 1],
      },
      {
        hexcode: '1F468-1F3FF-200D-1F91D-200D-1F468-1F3FC',
        emoji: '👨🏿‍🤝‍👨🏼',
        tone: [5, 2],
      },
      {
        hexcode: '1F468-1F3FF-200D-1F91D-200D-1F468-1F3FD',
        emoji: '👨🏿‍🤝‍👨🏽',
        tone: [5, 3],
      },
      {
        hexcode: '1F468-1F3FF-200D-1F91D-200D-1F468-1F3FE',
        emoji: '👨🏿‍🤝‍👨🏾',
        tone: [5, 4],
      },
    ],
  },
  {
    hexcode: '1F48F',
    shortcodes: ['couple'],
    emoji: '💏',
    order: 2328,
    group: 1,
  },
  {
    hexcode: '1F469-200D-2764-FE0F-200D-1F48B-200D-1F468',
    shortcodes: ['kiss_mw', 'kiss_wm'],
    emoji: '👩‍❤️‍💋‍👨',
    order: 2329,
    group: 1,
  },
  {
    hexcode: '1F468-200D-2764-FE0F-200D-1F48B-200D-1F468',
    shortcodes: ['kiss_mm'],
    emoji: '👨‍❤️‍💋‍👨',
    order: 2331,
    group: 1,
  },
  {
    hexcode: '1F469-200D-2764-FE0F-200D-1F48B-200D-1F469',
    shortcodes: ['kiss_ww'],
    emoji: '👩‍❤️‍💋‍👩',
    order: 2333,
    group: 1,
  },
  {
    hexcode: '1F491',
    shortcodes: ['couple_heart'],
    emoji: '💑',
    order: 2335,
    group: 1,
  },
  {
    hexcode: '1F469-200D-2764-FE0F-200D-1F468',
    shortcodes: ['couple_mw', 'couple_wm'],
    emoji: '👩‍❤️‍👨',
    order: 2336,
    group: 1,
  },
  {
    hexcode: '1F468-200D-2764-FE0F-200D-1F468',
    shortcodes: ['couple_mm'],
    emoji: '👨‍❤️‍👨',
    order: 2338,
    group: 1,
  },
  {
    hexcode: '1F469-200D-2764-FE0F-200D-1F469',
    shortcodes: ['couple_ww'],
    emoji: '👩‍❤️‍👩',
    order: 2340,
    group: 1,
  },
  {
    hexcode: '1F46A',
    shortcodes: ['family'],
    emoji: '👪️',
    order: 2342,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F469-200D-1F466',
    shortcodes: ['family_mwb'],
    emoji: '👨‍👩‍👦',
    order: 2343,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F469-200D-1F467',
    shortcodes: ['family_mwg'],
    emoji: '👨‍👩‍👧',
    order: 2344,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F469-200D-1F467-200D-1F466',
    shortcodes: ['family_mwgb'],
    emoji: '👨‍👩‍👧‍👦',
    order: 2345,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F469-200D-1F466-200D-1F466',
    shortcodes: ['family_mwbb'],
    emoji: '👨‍👩‍👦‍👦',
    order: 2346,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F469-200D-1F467-200D-1F467',
    shortcodes: ['family_mwgg'],
    emoji: '👨‍👩‍👧‍👧',
    order: 2347,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F468-200D-1F466',
    shortcodes: ['family_mmb'],
    emoji: '👨‍👨‍👦',
    order: 2348,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F468-200D-1F467',
    shortcodes: ['family_mmg'],
    emoji: '👨‍👨‍👧',
    order: 2349,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F468-200D-1F467-200D-1F466',
    shortcodes: ['family_mmgb'],
    emoji: '👨‍👨‍👧‍👦',
    order: 2350,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F468-200D-1F466-200D-1F466',
    shortcodes: ['family_mmbb'],
    emoji: '👨‍👨‍👦‍👦',
    order: 2351,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F468-200D-1F467-200D-1F467',
    shortcodes: ['family_mmgg'],
    emoji: '👨‍👨‍👧‍👧',
    order: 2352,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F469-200D-1F466',
    shortcodes: ['family_wwb'],
    emoji: '👩‍👩‍👦',
    order: 2353,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F469-200D-1F467',
    shortcodes: ['family_wwg'],
    emoji: '👩‍👩‍👧',
    order: 2354,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F469-200D-1F467-200D-1F466',
    shortcodes: ['family_wwgb'],
    emoji: '👩‍👩‍👧‍👦',
    order: 2355,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F469-200D-1F466-200D-1F466',
    shortcodes: ['family_wwbb'],
    emoji: '👩‍👩‍👦‍👦',
    order: 2356,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F469-200D-1F467-200D-1F467',
    shortcodes: ['family_wwgg'],
    emoji: '👩‍👩‍👧‍👧',
    order: 2357,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F466',
    shortcodes: ['family_mb'],
    emoji: '👨‍👦',
    order: 2358,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F466-200D-1F466',
    shortcodes: ['family_mbb'],
    emoji: '👨‍👦‍👦',
    order: 2359,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F467',
    shortcodes: ['family_mg'],
    emoji: '👨‍👧',
    order: 2360,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F467-200D-1F466',
    shortcodes: ['family_mgb'],
    emoji: '👨‍👧‍👦',
    order: 2361,
    group: 1,
  },
  {
    hexcode: '1F468-200D-1F467-200D-1F467',
    shortcodes: ['family_mgg'],
    emoji: '👨‍👧‍👧',
    order: 2362,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F466',
    shortcodes: ['family_wb'],
    emoji: '👩‍👦',
    order: 2363,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F466-200D-1F466',
    shortcodes: ['family_wbb'],
    emoji: '👩‍👦‍👦',
    order: 2364,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F467',
    shortcodes: ['family_wg'],
    emoji: '👩‍👧',
    order: 2365,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F467-200D-1F466',
    shortcodes: ['family_wgb'],
    emoji: '👩‍👧‍👦',
    order: 2366,
    group: 1,
  },
  {
    hexcode: '1F469-200D-1F467-200D-1F467',
    shortcodes: ['family_wgg'],
    emoji: '👩‍👧‍👧',
    order: 2367,
    group: 1,
  },
  {
    hexcode: '1F5E3',
    shortcodes: ['speaking_head'],
    emoji: '🗣️',
    order: 2369,
    group: 1,
  },
  {
    hexcode: '1F464',
    shortcodes: ['bust_silhouette'],
    emoji: '👤',
    order: 2370,
    group: 1,
  },
  {
    hexcode: '1F465',
    shortcodes: ['busts_silhouette'],
    emoji: '👥',
    order: 2371,
    group: 1,
  },
  {
    hexcode: '1F463',
    shortcodes: ['footprints'],
    emoji: '👣',
    order: 2372,
    group: 1,
  },
  {
    hexcode: '1F3FB',
    shortcodes: ['tone_light', 'tone1'],
    emoji: '🏻',
    order: 2373,
    group: 2,
  },
  {
    hexcode: '1F3FC',
    shortcodes: ['tone_medium_light', 'tone2'],
    emoji: '🏼',
    order: 2374,
    group: 2,
  },
  {
    hexcode: '1F3FD',
    shortcodes: ['tone_medium', 'tone3'],
    emoji: '🏽',
    order: 2375,
    group: 2,
  },
  {
    hexcode: '1F3FE',
    shortcodes: ['tone_medium_dark', 'tone4'],
    emoji: '🏾',
    order: 2376,
    group: 2,
  },
  {
    hexcode: '1F3FF',
    shortcodes: ['tone_dark', 'tone5'],
    emoji: '🏿',
    order: 2377,
    group: 2,
  },
  {
    hexcode: '1F9B0',
    shortcodes: ['red_hair'],
    emoji: '🦰',
    order: 2378,
    group: 2,
  },
  {
    hexcode: '1F9B1',
    shortcodes: ['curly_hair'],
    emoji: '🦱',
    order: 2379,
    group: 2,
  },
  {
    hexcode: '1F9B3',
    shortcodes: ['white_hair'],
    emoji: '🦳',
    order: 2380,
    group: 2,
  },
  {
    hexcode: '1F9B2',
    shortcodes: ['bald'],
    emoji: '🦲',
    order: 2381,
    group: 2,
  },
  {
    hexcode: '1F435',
    shortcodes: ['monkey_face'],
    emoji: '🐵',
    order: 2382,
    group: 3,
  },
  {
    hexcode: '1F412',
    shortcodes: ['monkey'],
    emoji: '🐒',
    order: 2383,
    group: 3,
  },
  {
    hexcode: '1F98D',
    shortcodes: ['gorilla'],
    emoji: '🦍',
    order: 2384,
    group: 3,
  },
  {
    hexcode: '1F9A7',
    shortcodes: ['orangutan'],
    emoji: '🦧',
    order: 2385,
    group: 3,
  },
  {
    hexcode: '1F436',
    shortcodes: ['dog_face'],
    emoji: '🐶',
    order: 2386,
    group: 3,
  },
  {
    hexcode: '1F415',
    shortcodes: ['dog'],
    emoji: '🐕️',
    order: 2387,
    group: 3,
  },
  {
    hexcode: '1F9AE',
    shortcodes: ['guide_dog'],
    emoji: '🦮',
    order: 2388,
    group: 3,
  },
  {
    hexcode: '1F415-200D-1F9BA',
    shortcodes: ['service_dog'],
    emoji: '🐕‍🦺',
    order: 2389,
    group: 3,
  },
  {
    hexcode: '1F429',
    shortcodes: ['poodle'],
    emoji: '🐩',
    order: 2390,
    group: 3,
  },
  {
    hexcode: '1F43A',
    shortcodes: ['wolf_face'],
    emoji: '🐺',
    order: 2391,
    group: 3,
  },
  {
    hexcode: '1F98A',
    shortcodes: ['fox_face'],
    emoji: '🦊',
    order: 2392,
    group: 3,
  },
  {
    hexcode: '1F99D',
    shortcodes: ['raccoon'],
    emoji: '🦝',
    order: 2393,
    group: 3,
  },
  {
    hexcode: '1F431',
    shortcodes: ['cat_face'],
    emoji: '🐱',
    order: 2394,
    group: 3,
  },
  {
    hexcode: '1F408',
    shortcodes: ['cat'],
    emoji: '🐈️',
    order: 2395,
    group: 3,
  },
  {
    hexcode: '1F981',
    shortcodes: ['lion_face'],
    emoji: '🦁',
    order: 2396,
    group: 3,
  },
  {
    hexcode: '1F42F',
    shortcodes: ['tiger_face'],
    emoji: '🐯',
    order: 2397,
    group: 3,
  },
  {
    hexcode: '1F405',
    shortcodes: ['tiger'],
    emoji: '🐅',
    order: 2398,
    group: 3,
  },
  {
    hexcode: '1F406',
    shortcodes: ['leopard'],
    emoji: '🐆',
    order: 2399,
    group: 3,
  },
  {
    hexcode: '1F434',
    shortcodes: ['horse_face'],
    emoji: '🐴',
    order: 2400,
    group: 3,
  },
  {
    hexcode: '1F40E',
    shortcodes: ['horse'],
    emoji: '🐎',
    order: 2401,
    group: 3,
  },
  {
    hexcode: '1F984',
    shortcodes: ['unicorn_face'],
    emoji: '🦄',
    order: 2402,
    group: 3,
  },
  {
    hexcode: '1F993',
    shortcodes: ['zebra'],
    emoji: '🦓',
    order: 2403,
    group: 3,
  },
  {
    hexcode: '1F98C',
    shortcodes: ['deer'],
    emoji: '🦌',
    order: 2404,
    group: 3,
  },
  {
    hexcode: '1F42E',
    shortcodes: ['cow_face'],
    emoji: '🐮',
    order: 2405,
    group: 3,
  },
  {
    hexcode: '1F402',
    shortcodes: ['ox'],
    emoji: '🐂',
    order: 2406,
    group: 3,
  },
  {
    hexcode: '1F403',
    shortcodes: ['water_buffalo'],
    emoji: '🐃',
    order: 2407,
    group: 3,
  },
  {
    hexcode: '1F404',
    shortcodes: ['cow'],
    emoji: '🐄',
    order: 2408,
    group: 3,
  },
  {
    hexcode: '1F437',
    shortcodes: ['pig_face'],
    emoji: '🐷',
    order: 2409,
    group: 3,
  },
  {
    hexcode: '1F416',
    shortcodes: ['pig'],
    emoji: '🐖',
    order: 2410,
    group: 3,
  },
  {
    hexcode: '1F417',
    shortcodes: ['boar'],
    emoji: '🐗',
    order: 2411,
    group: 3,
  },
  {
    hexcode: '1F43D',
    shortcodes: ['pig_nose'],
    emoji: '🐽',
    order: 2412,
    group: 3,
  },
  {
    hexcode: '1F40F',
    shortcodes: ['ram'],
    emoji: '🐏',
    order: 2413,
    group: 3,
  },
  {
    hexcode: '1F411',
    shortcodes: ['sheep'],
    emoji: '🐑',
    order: 2414,
    group: 3,
  },
  {
    hexcode: '1F410',
    shortcodes: ['goat'],
    emoji: '🐐',
    order: 2415,
    group: 3,
  },
  {
    hexcode: '1F42A',
    shortcodes: ['camel'],
    emoji: '🐪',
    order: 2416,
    group: 3,
  },
  {
    hexcode: '1F42B',
    shortcodes: ['two_hump_camel'],
    emoji: '🐫',
    order: 2417,
    group: 3,
  },
  {
    hexcode: '1F999',
    shortcodes: ['llama'],
    emoji: '🦙',
    order: 2418,
    group: 3,
  },
  {
    hexcode: '1F992',
    shortcodes: ['giraffe'],
    emoji: '🦒',
    order: 2419,
    group: 3,
  },
  {
    hexcode: '1F418',
    shortcodes: ['elephant'],
    emoji: '🐘',
    order: 2420,
    group: 3,
  },
  {
    hexcode: '1F98F',
    shortcodes: ['rhino'],
    emoji: '🦏',
    order: 2421,
    group: 3,
  },
  {
    hexcode: '1F99B',
    shortcodes: ['hippo'],
    emoji: '🦛',
    order: 2422,
    group: 3,
  },
  {
    hexcode: '1F42D',
    shortcodes: ['mouse_face'],
    emoji: '🐭',
    order: 2423,
    group: 3,
  },
  {
    hexcode: '1F401',
    shortcodes: ['mouse'],
    emoji: '🐁',
    order: 2424,
    group: 3,
  },
  {
    hexcode: '1F400',
    shortcodes: ['rat'],
    emoji: '🐀',
    order: 2425,
    group: 3,
  },
  {
    hexcode: '1F439',
    shortcodes: ['hamster_face'],
    emoji: '🐹',
    order: 2426,
    group: 3,
  },
  {
    hexcode: '1F430',
    shortcodes: ['rabbit_face'],
    emoji: '🐰',
    order: 2427,
    group: 3,
  },
  {
    hexcode: '1F407',
    shortcodes: ['rabbit'],
    emoji: '🐇',
    order: 2428,
    group: 3,
  },
  {
    hexcode: '1F43F',
    shortcodes: ['chipmunk'],
    emoji: '🐿️',
    order: 2430,
    group: 3,
  },
  {
    hexcode: '1F994',
    shortcodes: ['hedgehog'],
    emoji: '🦔',
    order: 2431,
    group: 3,
  },
  {
    hexcode: '1F987',
    shortcodes: ['bat'],
    emoji: '🦇',
    order: 2432,
    group: 3,
  },
  {
    hexcode: '1F43B',
    shortcodes: ['bear_face'],
    emoji: '🐻',
    order: 2433,
    group: 3,
  },
  {
    hexcode: '1F428',
    shortcodes: ['koala_face'],
    emoji: '🐨',
    order: 2434,
    group: 3,
  },
  {
    hexcode: '1F43C',
    shortcodes: ['panda_face'],
    emoji: '🐼',
    order: 2435,
    group: 3,
  },
  {
    hexcode: '1F9A5',
    shortcodes: ['sloth'],
    emoji: '🦥',
    order: 2436,
    group: 3,
  },
  {
    hexcode: '1F9A6',
    shortcodes: ['otter'],
    emoji: '🦦',
    order: 2437,
    group: 3,
  },
  {
    hexcode: '1F9A8',
    shortcodes: ['skunk'],
    emoji: '🦨',
    order: 2438,
    group: 3,
  },
  {
    hexcode: '1F998',
    shortcodes: ['kangaroo'],
    emoji: '🦘',
    order: 2439,
    group: 3,
  },
  {
    hexcode: '1F9A1',
    shortcodes: ['badger'],
    emoji: '🦡',
    order: 2440,
    group: 3,
  },
  {
    hexcode: '1F43E',
    shortcodes: ['feet'],
    emoji: '🐾',
    order: 2441,
    group: 3,
  },
  {
    hexcode: '1F983',
    shortcodes: ['turkey'],
    emoji: '🦃',
    order: 2442,
    group: 3,
  },
  {
    hexcode: '1F414',
    shortcodes: ['chicken'],
    emoji: '🐔',
    order: 2443,
    group: 3,
  },
  {
    hexcode: '1F413',
    shortcodes: ['rooster'],
    emoji: '🐓',
    order: 2444,
    group: 3,
  },
  {
    hexcode: '1F423',
    shortcodes: ['hatching_chick'],
    emoji: '🐣',
    order: 2445,
    group: 3,
  },
  {
    hexcode: '1F424',
    shortcodes: ['baby_chick'],
    emoji: '🐤',
    order: 2446,
    group: 3,
  },
  {
    hexcode: '1F425',
    shortcodes: ['hatched_chick'],
    emoji: '🐥',
    order: 2447,
    group: 3,
  },
  {
    hexcode: '1F426',
    shortcodes: ['bird'],
    emoji: '🐦️',
    order: 2448,
    group: 3,
  },
  {
    hexcode: '1F427',
    shortcodes: ['penguin'],
    emoji: '🐧',
    order: 2449,
    group: 3,
  },
  {
    hexcode: '1F54A',
    shortcodes: ['dove'],
    emoji: '🕊️',
    order: 2451,
    group: 3,
  },
  {
    hexcode: '1F985',
    shortcodes: ['eagle'],
    emoji: '🦅',
    order: 2452,
    group: 3,
  },
  {
    hexcode: '1F986',
    shortcodes: ['duck'],
    emoji: '🦆',
    order: 2453,
    group: 3,
  },
  {
    hexcode: '1F9A2',
    shortcodes: ['swan'],
    emoji: '🦢',
    order: 2454,
    group: 3,
  },
  {
    hexcode: '1F989',
    shortcodes: ['owl'],
    emoji: '🦉',
    order: 2455,
    group: 3,
  },
  {
    hexcode: '1F9A9',
    shortcodes: ['flamingo'],
    emoji: '🦩',
    order: 2456,
    group: 3,
  },
  {
    hexcode: '1F99A',
    shortcodes: ['peacock'],
    emoji: '🦚',
    order: 2457,
    group: 3,
  },
  {
    hexcode: '1F99C',
    shortcodes: ['parrot'],
    emoji: '🦜',
    order: 2458,
    group: 3,
  },
  {
    hexcode: '1F438',
    shortcodes: ['frog_face'],
    emoji: '🐸',
    order: 2459,
    group: 3,
  },
  {
    hexcode: '1F40A',
    shortcodes: ['crocodile'],
    emoji: '🐊',
    order: 2460,
    group: 3,
  },
  {
    hexcode: '1F422',
    shortcodes: ['turtle'],
    emoji: '🐢',
    order: 2461,
    group: 3,
  },
  {
    hexcode: '1F98E',
    shortcodes: ['lizard'],
    emoji: '🦎',
    order: 2462,
    group: 3,
  },
  {
    hexcode: '1F40D',
    shortcodes: ['snake'],
    emoji: '🐍',
    order: 2463,
    group: 3,
  },
  {
    hexcode: '1F432',
    shortcodes: ['dragon_face'],
    emoji: '🐲',
    order: 2464,
    group: 3,
  },
  {
    hexcode: '1F409',
    shortcodes: ['dragon'],
    emoji: '🐉',
    order: 2465,
    group: 3,
  },
  {
    hexcode: '1F995',
    shortcodes: ['sauropod'],
    emoji: '🦕',
    order: 2466,
    group: 3,
  },
  {
    hexcode: '1F996',
    shortcodes: ['trex'],
    emoji: '🦖',
    order: 2467,
    group: 3,
  },
  {
    hexcode: '1F433',
    shortcodes: ['spouting_whale'],
    emoji: '🐳',
    order: 2468,
    group: 3,
  },
  {
    hexcode: '1F40B',
    shortcodes: ['whale'],
    emoji: '🐋',
    order: 2469,
    group: 3,
  },
  {
    hexcode: '1F42C',
    shortcodes: ['dolphin'],
    emoji: '🐬',
    order: 2470,
    group: 3,
  },
  {
    hexcode: '1F41F',
    shortcodes: ['fish'],
    emoji: '🐟️',
    order: 2471,
    group: 3,
  },
  {
    hexcode: '1F420',
    shortcodes: ['tropical_fish'],
    emoji: '🐠',
    order: 2472,
    group: 3,
  },
  {
    hexcode: '1F421',
    shortcodes: ['blowfish'],
    emoji: '🐡',
    order: 2473,
    group: 3,
  },
  {
    hexcode: '1F988',
    shortcodes: ['shark'],
    emoji: '🦈',
    order: 2474,
    group: 3,
  },
  {
    hexcode: '1F419',
    shortcodes: ['octopus'],
    emoji: '🐙',
    order: 2475,
    group: 3,
  },
  {
    hexcode: '1F41A',
    shortcodes: ['shell'],
    emoji: '🐚',
    order: 2476,
    group: 3,
  },
  {
    hexcode: '1F40C',
    shortcodes: ['snail'],
    emoji: '🐌',
    order: 2477,
    group: 3,
  },
  {
    hexcode: '1F98B',
    shortcodes: ['butterfly'],
    emoji: '🦋',
    order: 2478,
    group: 3,
  },
  {
    hexcode: '1F41B',
    shortcodes: ['bug'],
    emoji: '🐛',
    order: 2479,
    group: 3,
  },
  {
    hexcode: '1F41C',
    shortcodes: ['ant'],
    emoji: '🐜',
    order: 2480,
    group: 3,
  },
  {
    hexcode: '1F41D',
    shortcodes: ['bee'],
    emoji: '🐝',
    order: 2481,
    group: 3,
  },
  {
    hexcode: '1F41E',
    shortcodes: ['beetle'],
    emoji: '🐞',
    order: 2482,
    group: 3,
  },
  {
    hexcode: '1F997',
    shortcodes: ['cricket'],
    emoji: '🦗',
    order: 2483,
    group: 3,
  },
  {
    hexcode: '1F577',
    shortcodes: ['spider'],
    emoji: '🕷️',
    order: 2485,
    group: 3,
  },
  {
    hexcode: '1F578',
    shortcodes: ['spider_web'],
    emoji: '🕸️',
    order: 2487,
    group: 3,
  },
  {
    hexcode: '1F982',
    shortcodes: ['scorpion'],
    emoji: '🦂',
    order: 2488,
    group: 3,
  },
  {
    hexcode: '1F99F',
    shortcodes: ['mosquito'],
    emoji: '🦟',
    order: 2489,
    group: 3,
  },
  {
    hexcode: '1F9A0',
    shortcodes: ['microbe', 'germ'],
    emoji: '🦠',
    order: 2490,
    group: 3,
  },
  {
    hexcode: '1F490',
    shortcodes: ['bouquet'],
    emoji: '💐',
    order: 2491,
    group: 3,
  },
  {
    hexcode: '1F338',
    shortcodes: ['cherry_blossom'],
    emoji: '🌸',
    order: 2492,
    group: 3,
  },
  {
    hexcode: '1F4AE',
    shortcodes: ['white_flower'],
    emoji: '💮',
    order: 2493,
    group: 3,
  },
  {
    hexcode: '1F3F5',
    shortcodes: ['rosette'],
    emoji: '🏵️',
    order: 2495,
    group: 3,
  },
  {
    hexcode: '1F339',
    shortcodes: ['rose'],
    emoji: '🌹',
    order: 2496,
    group: 3,
  },
  {
    hexcode: '1F940',
    shortcodes: ['wilted_rose'],
    emoji: '🥀',
    order: 2497,
    group: 3,
  },
  {
    hexcode: '1F33A',
    shortcodes: ['hibiscus'],
    emoji: '🌺',
    order: 2498,
    group: 3,
  },
  {
    hexcode: '1F33B',
    shortcodes: ['sunflower'],
    emoji: '🌻',
    order: 2499,
    group: 3,
  },
  {
    hexcode: '1F33C',
    shortcodes: ['blossom'],
    emoji: '🌼',
    order: 2500,
    group: 3,
  },
  {
    hexcode: '1F337',
    shortcodes: ['tulip'],
    emoji: '🌷',
    order: 2501,
    group: 3,
  },
  {
    hexcode: '1F331',
    shortcodes: ['seedling'],
    emoji: '🌱',
    order: 2502,
    group: 3,
  },
  {
    hexcode: '1F332',
    shortcodes: ['evergreen_tree'],
    emoji: '🌲',
    order: 2503,
    group: 3,
  },
  {
    hexcode: '1F333',
    shortcodes: ['deciduous_tree'],
    emoji: '🌳',
    order: 2504,
    group: 3,
  },
  {
    hexcode: '1F334',
    shortcodes: ['palm_tree'],
    emoji: '🌴',
    order: 2505,
    group: 3,
  },
  {
    hexcode: '1F335',
    shortcodes: ['cactus'],
    emoji: '🌵',
    order: 2506,
    group: 3,
  },
  {
    hexcode: '1F33E',
    shortcodes: ['ear_of_rice'],
    emoji: '🌾',
    order: 2507,
    group: 3,
  },
  {
    hexcode: '1F33F',
    shortcodes: ['herb'],
    emoji: '🌿',
    order: 2508,
    group: 3,
  },
  {
    hexcode: '2618',
    shortcodes: ['shamrock'],
    emoji: '☘️',
    order: 2510,
    group: 3,
  },
  {
    hexcode: '1F340',
    shortcodes: ['four_leaf_clover'],
    emoji: '🍀',
    order: 2511,
    group: 3,
  },
  {
    hexcode: '1F341',
    shortcodes: ['maple_leaf'],
    emoji: '🍁',
    order: 2512,
    group: 3,
  },
  {
    hexcode: '1F342',
    shortcodes: ['fallen_leaf'],
    emoji: '🍂',
    order: 2513,
    group: 3,
  },
  {
    hexcode: '1F343',
    shortcodes: ['leaves'],
    emoji: '🍃',
    order: 2514,
    group: 3,
  },
  {
    hexcode: '1F347',
    shortcodes: ['grapes'],
    emoji: '🍇',
    order: 2515,
    group: 4,
  },
  {
    hexcode: '1F348',
    shortcodes: ['melon'],
    emoji: '🍈',
    order: 2516,
    group: 4,
  },
  {
    hexcode: '1F349',
    shortcodes: ['watermelon'],
    emoji: '🍉',
    order: 2517,
    group: 4,
  },
  {
    hexcode: '1F34A',
    shortcodes: ['tangerine'],
    emoji: '🍊',
    order: 2518,
    group: 4,
  },
  {
    hexcode: '1F34B',
    shortcodes: ['lemon'],
    emoji: '🍋',
    order: 2519,
    group: 4,
  },
  {
    hexcode: '1F34C',
    shortcodes: ['banana'],
    emoji: '🍌',
    order: 2520,
    group: 4,
  },
  {
    hexcode: '1F34D',
    shortcodes: ['pineapple'],
    emoji: '🍍',
    order: 2521,
    group: 4,
  },
  {
    hexcode: '1F96D',
    shortcodes: ['mango'],
    emoji: '🥭',
    order: 2522,
    group: 4,
  },
  {
    hexcode: '1F34E',
    shortcodes: ['apple'],
    emoji: '🍎',
    order: 2523,
    group: 4,
  },
  {
    hexcode: '1F34F',
    shortcodes: ['green_apple'],
    emoji: '🍏',
    order: 2524,
    group: 4,
  },
  {
    hexcode: '1F350',
    shortcodes: ['pear'],
    emoji: '🍐',
    order: 2525,
    group: 4,
  },
  {
    hexcode: '1F351',
    shortcodes: ['peach'],
    emoji: '🍑',
    order: 2526,
    group: 4,
  },
  {
    hexcode: '1F352',
    shortcodes: ['cherries'],
    emoji: '🍒',
    order: 2527,
    group: 4,
  },
  {
    hexcode: '1F353',
    shortcodes: ['strawberry'],
    emoji: '🍓',
    order: 2528,
    group: 4,
  },
  {
    hexcode: '1F95D',
    shortcodes: ['kiwi'],
    emoji: '🥝',
    order: 2529,
    group: 4,
  },
  {
    hexcode: '1F345',
    shortcodes: ['tomato'],
    emoji: '🍅',
    order: 2530,
    group: 4,
  },
  {
    hexcode: '1F965',
    shortcodes: ['coconut'],
    emoji: '🥥',
    order: 2531,
    group: 4,
  },
  {
    hexcode: '1F951',
    shortcodes: ['avocado'],
    emoji: '🥑',
    order: 2532,
    group: 4,
  },
  {
    hexcode: '1F346',
    shortcodes: ['eggplant'],
    emoji: '🍆',
    order: 2533,
    group: 4,
  },
  {
    hexcode: '1F954',
    shortcodes: ['potato'],
    emoji: '🥔',
    order: 2534,
    group: 4,
  },
  {
    hexcode: '1F955',
    shortcodes: ['carrot'],
    emoji: '🥕',
    order: 2535,
    group: 4,
  },
  {
    hexcode: '1F33D',
    shortcodes: ['corn'],
    emoji: '🌽',
    order: 2536,
    group: 4,
  },
  {
    hexcode: '1F336',
    shortcodes: ['hot_pepper'],
    emoji: '🌶️',
    order: 2538,
    group: 4,
  },
  {
    hexcode: '1F952',
    shortcodes: ['cucumber'],
    emoji: '🥒',
    order: 2539,
    group: 4,
  },
  {
    hexcode: '1F96C',
    shortcodes: ['leafy_green'],
    emoji: '🥬',
    order: 2540,
    group: 4,
  },
  {
    hexcode: '1F966',
    shortcodes: ['broccoli'],
    emoji: '🥦',
    order: 2541,
    group: 4,
  },
  {
    hexcode: '1F9C4',
    shortcodes: ['garlic'],
    emoji: '🧄',
    order: 2542,
    group: 4,
  },
  {
    hexcode: '1F9C5',
    shortcodes: ['onion'],
    emoji: '🧅',
    order: 2543,
    group: 4,
  },
  {
    hexcode: '1F344',
    shortcodes: ['mushroom'],
    emoji: '🍄',
    order: 2544,
    group: 4,
  },
  {
    hexcode: '1F95C',
    shortcodes: ['peanuts'],
    emoji: '🥜',
    order: 2545,
    group: 4,
  },
  {
    hexcode: '1F330',
    shortcodes: ['chestnut'],
    emoji: '🌰',
    order: 2546,
    group: 4,
  },
  {
    hexcode: '1F35E',
    shortcodes: ['bread'],
    emoji: '🍞',
    order: 2547,
    group: 4,
  },
  {
    hexcode: '1F950',
    shortcodes: ['croissant'],
    emoji: '🥐',
    order: 2548,
    group: 4,
  },
  {
    hexcode: '1F956',
    shortcodes: ['french_bread'],
    emoji: '🥖',
    order: 2549,
    group: 4,
  },
  {
    hexcode: '1F968',
    shortcodes: ['pretzel'],
    emoji: '🥨',
    order: 2550,
    group: 4,
  },
  {
    hexcode: '1F96F',
    shortcodes: ['bagel'],
    emoji: '🥯',
    order: 2551,
    group: 4,
  },
  {
    hexcode: '1F95E',
    shortcodes: ['pancakes'],
    emoji: '🥞',
    order: 2552,
    group: 4,
  },
  {
    hexcode: '1F9C7',
    shortcodes: ['waffle'],
    emoji: '🧇',
    order: 2553,
    group: 4,
  },
  {
    hexcode: '1F9C0',
    shortcodes: ['cheese'],
    emoji: '🧀',
    order: 2554,
    group: 4,
  },
  {
    hexcode: '1F356',
    shortcodes: ['meat_on_bone'],
    emoji: '🍖',
    order: 2555,
    group: 4,
  },
  {
    hexcode: '1F357',
    shortcodes: ['poultry_leg'],
    emoji: '🍗',
    order: 2556,
    group: 4,
  },
  {
    hexcode: '1F969',
    shortcodes: ['cut_of_meat'],
    emoji: '🥩',
    order: 2557,
    group: 4,
  },
  {
    hexcode: '1F953',
    shortcodes: ['bacon'],
    emoji: '🥓',
    order: 2558,
    group: 4,
  },
  {
    hexcode: '1F354',
    shortcodes: ['hamburger'],
    emoji: '🍔',
    order: 2559,
    group: 4,
  },
  {
    hexcode: '1F35F',
    shortcodes: ['fries'],
    emoji: '🍟',
    order: 2560,
    group: 4,
  },
  {
    hexcode: '1F355',
    shortcodes: ['pizza'],
    emoji: '🍕',
    order: 2561,
    group: 4,
  },
  {
    hexcode: '1F32D',
    shortcodes: ['hotdog'],
    emoji: '🌭',
    order: 2562,
    group: 4,
  },
  {
    hexcode: '1F96A',
    shortcodes: ['sandwich'],
    emoji: '🥪',
    order: 2563,
    group: 4,
  },
  {
    hexcode: '1F32E',
    shortcodes: ['taco'],
    emoji: '🌮',
    order: 2564,
    group: 4,
  },
  {
    hexcode: '1F32F',
    shortcodes: ['burrito'],
    emoji: '🌯',
    order: 2565,
    group: 4,
  },
  {
    hexcode: '1F959',
    shortcodes: ['stuffed_flatbread'],
    emoji: '🥙',
    order: 2566,
    group: 4,
  },
  {
    hexcode: '1F9C6',
    shortcodes: ['falafel'],
    emoji: '🧆',
    order: 2567,
    group: 4,
  },
  {
    hexcode: '1F95A',
    shortcodes: ['egg'],
    emoji: '🥚',
    order: 2568,
    group: 4,
  },
  {
    hexcode: '1F373',
    shortcodes: ['cooking'],
    emoji: '🍳',
    order: 2569,
    group: 4,
  },
  {
    hexcode: '1F958',
    shortcodes: ['shallow_pan_of_food'],
    emoji: '🥘',
    order: 2570,
    group: 4,
  },
  {
    hexcode: '1F372',
    shortcodes: ['stew'],
    emoji: '🍲',
    order: 2571,
    group: 4,
  },
  {
    hexcode: '1F963',
    shortcodes: ['bowl_spoon'],
    emoji: '🥣',
    order: 2572,
    group: 4,
  },
  {
    hexcode: '1F957',
    shortcodes: ['salad'],
    emoji: '🥗',
    order: 2573,
    group: 4,
  },
  {
    hexcode: '1F37F',
    shortcodes: ['popcorn'],
    emoji: '🍿',
    order: 2574,
    group: 4,
  },
  {
    hexcode: '1F9C8',
    shortcodes: ['butter'],
    emoji: '🧈',
    order: 2575,
    group: 4,
  },
  {
    hexcode: '1F9C2',
    shortcodes: ['salt'],
    emoji: '🧂',
    order: 2576,
    group: 4,
  },
  {
    hexcode: '1F96B',
    shortcodes: ['canned_food'],
    emoji: '🥫',
    order: 2577,
    group: 4,
  },
  {
    hexcode: '1F371',
    shortcodes: ['bento'],
    emoji: '🍱',
    order: 2578,
    group: 4,
  },
  {
    hexcode: '1F358',
    shortcodes: ['rice_cracker'],
    emoji: '🍘',
    order: 2579,
    group: 4,
  },
  {
    hexcode: '1F359',
    shortcodes: ['rice_ball'],
    emoji: '🍙',
    order: 2580,
    group: 4,
  },
  {
    hexcode: '1F35A',
    shortcodes: ['rice'],
    emoji: '🍚',
    order: 2581,
    group: 4,
  },
  {
    hexcode: '1F35B',
    shortcodes: ['curry'],
    emoji: '🍛',
    order: 2582,
    group: 4,
  },
  {
    hexcode: '1F35C',
    shortcodes: ['ramen'],
    emoji: '🍜',
    order: 2583,
    group: 4,
  },
  {
    hexcode: '1F35D',
    shortcodes: ['spaghetti'],
    emoji: '🍝',
    order: 2584,
    group: 4,
  },
  {
    hexcode: '1F360',
    shortcodes: ['sweet_potato'],
    emoji: '🍠',
    order: 2585,
    group: 4,
  },
  {
    hexcode: '1F362',
    shortcodes: ['oden'],
    emoji: '🍢',
    order: 2586,
    group: 4,
  },
  {
    hexcode: '1F363',
    shortcodes: ['sushi'],
    emoji: '🍣',
    order: 2587,
    group: 4,
  },
  {
    hexcode: '1F364',
    shortcodes: ['fried_shrimp'],
    emoji: '🍤',
    order: 2588,
    group: 4,
  },
  {
    hexcode: '1F365',
    shortcodes: ['fish_cake'],
    emoji: '🍥',
    order: 2589,
    group: 4,
  },
  {
    hexcode: '1F96E',
    shortcodes: ['moon_cake'],
    emoji: '🥮',
    order: 2590,
    group: 4,
  },
  {
    hexcode: '1F361',
    shortcodes: ['dango'],
    emoji: '🍡',
    order: 2591,
    group: 4,
  },
  {
    hexcode: '1F95F',
    shortcodes: ['dumpling'],
    emoji: '🥟',
    order: 2592,
    group: 4,
  },
  {
    hexcode: '1F960',
    shortcodes: ['fortune_cookie'],
    emoji: '🥠',
    order: 2593,
    group: 4,
  },
  {
    hexcode: '1F961',
    shortcodes: ['takeout_box'],
    emoji: '🥡',
    order: 2594,
    group: 4,
  },
  {
    hexcode: '1F980',
    shortcodes: ['crab'],
    emoji: '🦀',
    order: 2595,
    group: 4,
  },
  {
    hexcode: '1F99E',
    shortcodes: ['lobster'],
    emoji: '🦞',
    order: 2596,
    group: 4,
  },
  {
    hexcode: '1F990',
    shortcodes: ['shrimp'],
    emoji: '🦐',
    order: 2597,
    group: 4,
  },
  {
    hexcode: '1F991',
    shortcodes: ['squid'],
    emoji: '🦑',
    order: 2598,
    group: 4,
  },
  {
    hexcode: '1F9AA',
    shortcodes: ['oyster'],
    emoji: '🦪',
    order: 2599,
    group: 4,
  },
  {
    hexcode: '1F366',
    shortcodes: ['icecream'],
    emoji: '🍦',
    order: 2600,
    group: 4,
  },
  {
    hexcode: '1F367',
    shortcodes: ['shaved_ice'],
    emoji: '🍧',
    order: 2601,
    group: 4,
  },
  {
    hexcode: '1F368',
    shortcodes: ['ice_cream'],
    emoji: '🍨',
    order: 2602,
    group: 4,
  },
  {
    hexcode: '1F369',
    shortcodes: ['doughnut'],
    emoji: '🍩',
    order: 2603,
    group: 4,
  },
  {
    hexcode: '1F36A',
    shortcodes: ['cookie'],
    emoji: '🍪',
    order: 2604,
    group: 4,
  },
  {
    hexcode: '1F382',
    shortcodes: ['birthday'],
    emoji: '🎂',
    order: 2605,
    group: 4,
  },
  {
    hexcode: '1F370',
    shortcodes: ['cake'],
    emoji: '🍰',
    order: 2606,
    group: 4,
  },
  {
    hexcode: '1F9C1',
    shortcodes: ['cupcake'],
    emoji: '🧁',
    order: 2607,
    group: 4,
  },
  {
    hexcode: '1F967',
    shortcodes: ['pie'],
    emoji: '🥧',
    order: 2608,
    group: 4,
  },
  {
    hexcode: '1F36B',
    shortcodes: ['chocolate_bar'],
    emoji: '🍫',
    order: 2609,
    group: 4,
  },
  {
    hexcode: '1F36C',
    shortcodes: ['candy'],
    emoji: '🍬',
    order: 2610,
    group: 4,
  },
  {
    hexcode: '1F36D',
    shortcodes: ['lollipop'],
    emoji: '🍭',
    order: 2611,
    group: 4,
  },
  {
    hexcode: '1F36E',
    shortcodes: ['custard'],
    emoji: '🍮',
    order: 2612,
    group: 4,
  },
  {
    hexcode: '1F36F',
    shortcodes: ['honey_pot'],
    emoji: '🍯',
    order: 2613,
    group: 4,
  },
  {
    hexcode: '1F37C',
    shortcodes: ['baby_bottle'],
    emoji: '🍼',
    order: 2614,
    group: 4,
  },
  {
    hexcode: '1F95B',
    shortcodes: ['milk'],
    emoji: '🥛',
    order: 2615,
    group: 4,
  },
  {
    hexcode: '2615',
    shortcodes: ['coffee'],
    emoji: '☕️',
    order: 2616,
    group: 4,
  },
  {
    hexcode: '1F375',
    shortcodes: ['tea'],
    emoji: '🍵',
    order: 2617,
    group: 4,
  },
  {
    hexcode: '1F376',
    shortcodes: ['sake'],
    emoji: '🍶',
    order: 2618,
    group: 4,
  },
  {
    hexcode: '1F37E',
    shortcodes: ['champagne'],
    emoji: '🍾',
    order: 2619,
    group: 4,
  },
  {
    hexcode: '1F377',
    shortcodes: ['wine_glass'],
    emoji: '🍷',
    order: 2620,
    group: 4,
  },
  {
    hexcode: '1F378',
    shortcodes: ['cocktail'],
    emoji: '🍸️',
    order: 2621,
    group: 4,
  },
  {
    hexcode: '1F379',
    shortcodes: ['tropical_drink'],
    emoji: '🍹',
    order: 2622,
    group: 4,
  },
  {
    hexcode: '1F37A',
    shortcodes: ['beer'],
    emoji: '🍺',
    order: 2623,
    group: 4,
  },
  {
    hexcode: '1F37B',
    shortcodes: ['beers'],
    emoji: '🍻',
    order: 2624,
    group: 4,
  },
  {
    hexcode: '1F942',
    shortcodes: ['champagne_glass'],
    emoji: '🥂',
    order: 2625,
    group: 4,
  },
  {
    hexcode: '1F943',
    shortcodes: ['tumbler_glass'],
    emoji: '🥃',
    order: 2626,
    group: 4,
  },
  {
    hexcode: '1F964',
    shortcodes: ['cup_straw'],
    emoji: '🥤',
    order: 2627,
    group: 4,
  },
  {
    hexcode: '1F9C3',
    shortcodes: ['beverage_box', 'juice_box'],
    emoji: '🧃',
    order: 2628,
    group: 4,
  },
  {
    hexcode: '1F9C9',
    shortcodes: ['mate', 'yerba_mate'],
    emoji: '🧉',
    order: 2629,
    group: 4,
  },
  {
    hexcode: '1F9CA',
    shortcodes: ['ice'],
    emoji: '🧊',
    order: 2630,
    group: 4,
  },
  {
    hexcode: '1F962',
    shortcodes: ['chopsticks'],
    emoji: '🥢',
    order: 2631,
    group: 4,
  },
  {
    hexcode: '1F37D',
    shortcodes: ['fork_knife_plate'],
    emoji: '🍽️',
    order: 2633,
    group: 4,
  },
  {
    hexcode: '1F374',
    shortcodes: ['utensils'],
    emoji: '🍴',
    order: 2634,
    group: 4,
  },
  {
    hexcode: '1F944',
    shortcodes: ['spoon'],
    emoji: '🥄',
    order: 2635,
    group: 4,
  },
  {
    hexcode: '1F52A',
    shortcodes: ['knife'],
    emoji: '🔪',
    order: 2636,
    group: 4,
  },
  {
    hexcode: '1F3FA',
    shortcodes: ['amphora'],
    emoji: '🏺',
    order: 2637,
    group: 4,
  },
  {
    hexcode: '1F30D',
    shortcodes: ['earth_africa'],
    emoji: '🌍️',
    order: 2638,
    group: 5,
  },
  {
    hexcode: '1F30E',
    shortcodes: ['earth_americas'],
    emoji: '🌎️',
    order: 2639,
    group: 5,
  },
  {
    hexcode: '1F30F',
    shortcodes: ['earth_asia'],
    emoji: '🌏️',
    order: 2640,
    group: 5,
  },
  {
    hexcode: '1F310',
    shortcodes: ['globe'],
    emoji: '🌐',
    order: 2641,
    group: 5,
  },
  {
    hexcode: '1F5FA',
    shortcodes: ['map'],
    emoji: '🗺️',
    order: 2643,
    group: 5,
  },
  {
    hexcode: '1F5FE',
    shortcodes: ['japan'],
    emoji: '🗾',
    order: 2644,
    group: 5,
  },
  {
    hexcode: '1F9ED',
    shortcodes: ['compass'],
    emoji: '🧭',
    order: 2645,
    group: 5,
  },
  {
    hexcode: '1F3D4',
    shortcodes: ['snowy_mountain'],
    emoji: '🏔️',
    order: 2647,
    group: 5,
  },
  {
    hexcode: '26F0',
    shortcodes: ['mountain'],
    emoji: '⛰️',
    order: 2649,
    group: 5,
  },
  {
    hexcode: '1F30B',
    shortcodes: ['volcano'],
    emoji: '🌋',
    order: 2650,
    group: 5,
  },
  {
    hexcode: '1F5FB',
    shortcodes: ['mount_fuji'],
    emoji: '🗻',
    order: 2651,
    group: 5,
  },
  {
    hexcode: '1F3D5',
    shortcodes: ['camping'],
    emoji: '🏕️',
    order: 2653,
    group: 5,
  },
  {
    hexcode: '1F3D6',
    shortcodes: ['beach'],
    emoji: '🏖️',
    order: 2655,
    group: 5,
  },
  {
    hexcode: '1F3DC',
    shortcodes: ['desert'],
    emoji: '🏜️',
    order: 2657,
    group: 5,
  },
  {
    hexcode: '1F3DD',
    shortcodes: ['island'],
    emoji: '🏝️',
    order: 2659,
    group: 5,
  },
  {
    hexcode: '1F3DE',
    shortcodes: ['park'],
    emoji: '🏞️',
    order: 2661,
    group: 5,
  },
  {
    hexcode: '1F3DF',
    shortcodes: ['stadium'],
    emoji: '🏟️',
    order: 2663,
    group: 5,
  },
  {
    hexcode: '1F3DB',
    shortcodes: ['classical_building'],
    emoji: '🏛️',
    order: 2665,
    group: 5,
  },
  {
    hexcode: '1F3D7',
    shortcodes: ['construction_site'],
    emoji: '🏗️',
    order: 2667,
    group: 5,
  },
  {
    hexcode: '1F9F1',
    shortcodes: ['brick'],
    emoji: '🧱',
    order: 2668,
    group: 5,
  },
  {
    hexcode: '1F3D8',
    shortcodes: ['homes'],
    emoji: '🏘️',
    order: 2670,
    group: 5,
  },
  {
    hexcode: '1F3DA',
    shortcodes: ['house_abandoned'],
    emoji: '🏚️',
    order: 2672,
    group: 5,
  },
  {
    hexcode: '1F3E0',
    shortcodes: ['house'],
    emoji: '🏠️',
    order: 2673,
    group: 5,
  },
  {
    hexcode: '1F3E1',
    shortcodes: ['house_garden'],
    emoji: '🏡',
    order: 2674,
    group: 5,
  },
  {
    hexcode: '1F3E2',
    shortcodes: ['office'],
    emoji: '🏢',
    order: 2675,
    group: 5,
  },
  {
    hexcode: '1F3E3',
    shortcodes: ['ja_post_office'],
    emoji: '🏣',
    order: 2676,
    group: 5,
  },
  {
    hexcode: '1F3E4',
    shortcodes: ['post_office'],
    emoji: '🏤',
    order: 2677,
    group: 5,
  },
  {
    hexcode: '1F3E5',
    shortcodes: ['hospital'],
    emoji: '🏥',
    order: 2678,
    group: 5,
  },
  {
    hexcode: '1F3E6',
    shortcodes: ['bank'],
    emoji: '🏦',
    order: 2679,
    group: 5,
  },
  {
    hexcode: '1F3E8',
    shortcodes: ['hotel'],
    emoji: '🏨',
    order: 2680,
    group: 5,
  },
  {
    hexcode: '1F3E9',
    shortcodes: ['love_hotel'],
    emoji: '🏩',
    order: 2681,
    group: 5,
  },
  {
    hexcode: '1F3EA',
    shortcodes: ['convenience_store'],
    emoji: '🏪',
    order: 2682,
    group: 5,
  },
  {
    hexcode: '1F3EB',
    shortcodes: ['school'],
    emoji: '🏫',
    order: 2683,
    group: 5,
  },
  {
    hexcode: '1F3EC',
    shortcodes: ['department_store'],
    emoji: '🏬',
    order: 2684,
    group: 5,
  },
  {
    hexcode: '1F3ED',
    shortcodes: ['factory'],
    emoji: '🏭️',
    order: 2685,
    group: 5,
  },
  {
    hexcode: '1F3EF',
    shortcodes: ['japanese_castle'],
    emoji: '🏯',
    order: 2686,
    group: 5,
  },
  {
    hexcode: '1F3F0',
    shortcodes: ['castle', 'european_castle'],
    emoji: '🏰',
    order: 2687,
    group: 5,
  },
  {
    hexcode: '1F492',
    shortcodes: ['wedding'],
    emoji: '💒',
    order: 2688,
    group: 5,
  },
  {
    hexcode: '1F5FC',
    shortcodes: ['tokyo_tower'],
    emoji: '🗼',
    order: 2689,
    group: 5,
  },
  {
    hexcode: '1F5FD',
    shortcodes: ['statue_of_liberty'],
    emoji: '🗽',
    order: 2690,
    group: 5,
  },
  {
    hexcode: '26EA',
    shortcodes: ['church'],
    emoji: '⛪️',
    order: 2691,
    group: 5,
  },
  {
    hexcode: '1F54C',
    shortcodes: ['mosque'],
    emoji: '🕌',
    order: 2692,
    group: 5,
  },
  {
    hexcode: '1F6D5',
    shortcodes: ['hindu_temple'],
    emoji: '🛕',
    order: 2693,
    group: 5,
  },
  {
    hexcode: '1F54D',
    shortcodes: ['synagogue'],
    emoji: '🕍',
    order: 2694,
    group: 5,
  },
  {
    hexcode: '26E9',
    shortcodes: ['shinto_shrine'],
    emoji: '⛩️',
    order: 2696,
    group: 5,
  },
  {
    hexcode: '1F54B',
    shortcodes: ['kaaba'],
    emoji: '🕋',
    order: 2697,
    group: 5,
  },
  {
    hexcode: '26F2',
    shortcodes: ['fountain'],
    emoji: '⛲️',
    order: 2698,
    group: 5,
  },
  {
    hexcode: '26FA',
    shortcodes: ['tent'],
    emoji: '⛺️',
    order: 2699,
    group: 5,
  },
  {
    hexcode: '1F301',
    shortcodes: ['foggy'],
    emoji: '🌁',
    order: 2700,
    group: 5,
  },
  {
    hexcode: '1F303',
    shortcodes: ['night_stars'],
    emoji: '🌃',
    order: 2701,
    group: 5,
  },
  {
    hexcode: '1F3D9',
    shortcodes: ['cityscape'],
    emoji: '🏙️',
    order: 2703,
    group: 5,
  },
  {
    hexcode: '1F304',
    shortcodes: ['sunrise_over_mountains'],
    emoji: '🌄',
    order: 2704,
    group: 5,
  },
  {
    hexcode: '1F305',
    shortcodes: ['sunrise'],
    emoji: '🌅',
    order: 2705,
    group: 5,
  },
  {
    hexcode: '1F306',
    shortcodes: ['dusk'],
    emoji: '🌆',
    order: 2706,
    group: 5,
  },
  {
    hexcode: '1F307',
    shortcodes: ['sunset'],
    emoji: '🌇',
    order: 2707,
    group: 5,
  },
  {
    hexcode: '1F309',
    shortcodes: ['bridge_at_night'],
    emoji: '🌉',
    order: 2708,
    group: 5,
  },
  {
    hexcode: '2668',
    shortcodes: ['hotsprings'],
    emoji: '♨️',
    order: 2710,
    group: 5,
  },
  {
    hexcode: '1F3A0',
    shortcodes: ['carousel_horse'],
    emoji: '🎠',
    order: 2711,
    group: 5,
  },
  {
    hexcode: '1F3A1',
    shortcodes: ['ferris_wheel'],
    emoji: '🎡',
    order: 2712,
    group: 5,
  },
  {
    hexcode: '1F3A2',
    shortcodes: ['roller_coaster'],
    emoji: '🎢',
    order: 2713,
    group: 5,
  },
  {
    hexcode: '1F488',
    shortcodes: ['barber'],
    emoji: '💈',
    order: 2714,
    group: 5,
  },
  {
    hexcode: '1F3AA',
    shortcodes: ['circus_tent'],
    emoji: '🎪',
    order: 2715,
    group: 5,
  },
  {
    hexcode: '1F682',
    shortcodes: ['steam_locomotive'],
    emoji: '🚂',
    order: 2716,
    group: 5,
  },
  {
    hexcode: '1F683',
    shortcodes: ['railway_car'],
    emoji: '🚃',
    order: 2717,
    group: 5,
  },
  {
    hexcode: '1F684',
    shortcodes: ['bullettrain_side'],
    emoji: '🚄',
    order: 2718,
    group: 5,
  },
  {
    hexcode: '1F685',
    shortcodes: ['bullettrain'],
    emoji: '🚅',
    order: 2719,
    group: 5,
  },
  {
    hexcode: '1F686',
    shortcodes: ['train'],
    emoji: '🚆',
    order: 2720,
    group: 5,
  },
  {
    hexcode: '1F687',
    shortcodes: ['metro'],
    emoji: '🚇️',
    order: 2721,
    group: 5,
  },
  {
    hexcode: '1F688',
    shortcodes: ['light_rail'],
    emoji: '🚈',
    order: 2722,
    group: 5,
  },
  {
    hexcode: '1F689',
    shortcodes: ['station'],
    emoji: '🚉',
    order: 2723,
    group: 5,
  },
  {
    hexcode: '1F68A',
    shortcodes: ['tram'],
    emoji: '🚊',
    order: 2724,
    group: 5,
  },
  {
    hexcode: '1F69D',
    shortcodes: ['monorail'],
    emoji: '🚝',
    order: 2725,
    group: 5,
  },
  {
    hexcode: '1F69E',
    shortcodes: ['mountain_railway'],
    emoji: '🚞',
    order: 2726,
    group: 5,
  },
  {
    hexcode: '1F68B',
    shortcodes: ['tram_car'],
    emoji: '🚋',
    order: 2727,
    group: 5,
  },
  {
    hexcode: '1F68C',
    shortcodes: ['bus'],
    emoji: '🚌',
    order: 2728,
    group: 5,
  },
  {
    hexcode: '1F68D',
    shortcodes: ['oncoming_bus'],
    emoji: '🚍️',
    order: 2729,
    group: 5,
  },
  {
    hexcode: '1F68E',
    shortcodes: ['trolleybus'],
    emoji: '🚎',
    order: 2730,
    group: 5,
  },
  {
    hexcode: '1F690',
    shortcodes: ['minibus'],
    emoji: '🚐',
    order: 2731,
    group: 5,
  },
  {
    hexcode: '1F691',
    shortcodes: ['ambulance'],
    emoji: '🚑️',
    order: 2732,
    group: 5,
  },
  {
    hexcode: '1F692',
    shortcodes: ['fire_engine'],
    emoji: '🚒',
    order: 2733,
    group: 5,
  },
  {
    hexcode: '1F693',
    shortcodes: ['police_car'],
    emoji: '🚓',
    order: 2734,
    group: 5,
  },
  {
    hexcode: '1F694',
    shortcodes: ['oncoming_police_car'],
    emoji: '🚔️',
    order: 2735,
    group: 5,
  },
  {
    hexcode: '1F695',
    shortcodes: ['taxi'],
    emoji: '🚕',
    order: 2736,
    group: 5,
  },
  {
    hexcode: '1F696',
    shortcodes: ['oncoming_taxi'],
    emoji: '🚖',
    order: 2737,
    group: 5,
  },
  {
    hexcode: '1F697',
    shortcodes: ['red_car'],
    emoji: '🚗',
    order: 2738,
    group: 5,
  },
  {
    hexcode: '1F698',
    shortcodes: ['oncoming_automobile'],
    emoji: '🚘️',
    order: 2739,
    group: 5,
  },
  {
    hexcode: '1F699',
    shortcodes: ['blue_car'],
    emoji: '🚙',
    order: 2740,
    group: 5,
  },
  {
    hexcode: '1F69A',
    shortcodes: ['truck'],
    emoji: '🚚',
    order: 2741,
    group: 5,
  },
  {
    hexcode: '1F69B',
    shortcodes: ['lorry'],
    emoji: '🚛',
    order: 2742,
    group: 5,
  },
  {
    hexcode: '1F69C',
    shortcodes: ['tractor'],
    emoji: '🚜',
    order: 2743,
    group: 5,
  },
  {
    hexcode: '1F3CE',
    shortcodes: ['race_car'],
    emoji: '🏎️',
    order: 2745,
    group: 5,
  },
  {
    hexcode: '1F3CD',
    shortcodes: ['motorcycle'],
    emoji: '🏍️',
    order: 2747,
    group: 5,
  },
  {
    hexcode: '1F6F5',
    shortcodes: ['motor_scooter'],
    emoji: '🛵',
    order: 2748,
    group: 5,
  },
  {
    hexcode: '1F9BD',
    shortcodes: ['wheelchair'],
    emoji: '🦽',
    order: 2749,
    group: 5,
  },
  {
    hexcode: '1F9BC',
    shortcodes: ['motor_wheelchair'],
    emoji: '🦼',
    order: 2750,
    group: 5,
  },
  {
    hexcode: '1F6FA',
    shortcodes: ['auto_rickshaw'],
    emoji: '🛺',
    order: 2751,
    group: 5,
  },
  {
    hexcode: '1F6B2',
    shortcodes: ['bike'],
    emoji: '🚲️',
    order: 2752,
    group: 5,
  },
  {
    hexcode: '1F6F4',
    shortcodes: ['scooter'],
    emoji: '🛴',
    order: 2753,
    group: 5,
  },
  {
    hexcode: '1F6F9',
    shortcodes: ['skateboard'],
    emoji: '🛹',
    order: 2754,
    group: 5,
  },
  {
    hexcode: '1F68F',
    shortcodes: ['bus_stop'],
    emoji: '🚏',
    order: 2755,
    group: 5,
  },
  {
    hexcode: '1F6E3',
    shortcodes: ['motorway'],
    emoji: '🛣️',
    order: 2757,
    group: 5,
  },
  {
    hexcode: '1F6E4',
    shortcodes: ['railway_track'],
    emoji: '🛤️',
    order: 2759,
    group: 5,
  },
  {
    hexcode: '1F6E2',
    shortcodes: ['oil_drum'],
    emoji: '🛢️',
    order: 2761,
    group: 5,
  },
  {
    hexcode: '26FD',
    shortcodes: ['fuel_pump'],
    emoji: '⛽️',
    order: 2762,
    group: 5,
  },
  {
    hexcode: '1F6A8',
    shortcodes: ['rotating_light', 'police_light'],
    emoji: '🚨',
    order: 2763,
    group: 5,
  },
  {
    hexcode: '1F6A5',
    shortcodes: ['traffic_light'],
    emoji: '🚥',
    order: 2764,
    group: 5,
  },
  {
    hexcode: '1F6A6',
    shortcodes: ['vertical_traffic_light'],
    emoji: '🚦',
    order: 2765,
    group: 5,
  },
  {
    hexcode: '1F6D1',
    shortcodes: ['stop_sign', 'octagonal_sign'],
    emoji: '🛑',
    order: 2766,
    group: 5,
  },
  {
    hexcode: '1F6A7',
    shortcodes: ['construction'],
    emoji: '🚧',
    order: 2767,
    group: 5,
  },
  {
    hexcode: '2693',
    shortcodes: ['anchor'],
    emoji: '⚓️',
    order: 2768,
    group: 5,
  },
  {
    hexcode: '26F5',
    shortcodes: ['sailboat'],
    emoji: '⛵️',
    order: 2769,
    group: 5,
  },
  {
    hexcode: '1F6F6',
    shortcodes: ['canoe'],
    emoji: '🛶',
    order: 2770,
    group: 5,
  },
  {
    hexcode: '1F6A4',
    shortcodes: ['speedboat'],
    emoji: '🚤',
    order: 2771,
    group: 5,
  },
  {
    hexcode: '1F6F3',
    shortcodes: ['cruise_ship'],
    emoji: '🛳️',
    order: 2773,
    group: 5,
  },
  {
    hexcode: '26F4',
    shortcodes: ['ferry'],
    emoji: '⛴️',
    order: 2775,
    group: 5,
  },
  {
    hexcode: '1F6E5',
    shortcodes: ['motorboat'],
    emoji: '🛥️',
    order: 2777,
    group: 5,
  },
  {
    hexcode: '1F6A2',
    shortcodes: ['ship'],
    emoji: '🚢',
    order: 2778,
    group: 5,
  },
  {
    hexcode: '2708',
    shortcodes: ['airplane'],
    emoji: '✈️',
    order: 2780,
    group: 5,
  },
  {
    hexcode: '1F6E9',
    shortcodes: ['small_airplane'],
    emoji: '🛩️',
    order: 2782,
    group: 5,
  },
  {
    hexcode: '1F6EB',
    shortcodes: ['airplane_departure'],
    emoji: '🛫',
    order: 2783,
    group: 5,
  },
  {
    hexcode: '1F6EC',
    shortcodes: ['airplane_arriving'],
    emoji: '🛬',
    order: 2784,
    group: 5,
  },
  {
    hexcode: '1FA82',
    shortcodes: ['parachute'],
    emoji: '🪂',
    order: 2785,
    group: 5,
  },
  {
    hexcode: '1F4BA',
    shortcodes: ['seat'],
    emoji: '💺',
    order: 2786,
    group: 5,
  },
  {
    hexcode: '1F681',
    shortcodes: ['helicopter'],
    emoji: '🚁',
    order: 2787,
    group: 5,
  },
  {
    hexcode: '1F69F',
    shortcodes: ['suspension_railway'],
    emoji: '🚟',
    order: 2788,
    group: 5,
  },
  {
    hexcode: '1F6A0',
    shortcodes: ['mountain_cableway'],
    emoji: '🚠',
    order: 2789,
    group: 5,
  },
  {
    hexcode: '1F6A1',
    shortcodes: ['aerial_tramway'],
    emoji: '🚡',
    order: 2790,
    group: 5,
  },
  {
    hexcode: '1F6F0',
    shortcodes: ['satellite'],
    emoji: '🛰️',
    order: 2792,
    group: 5,
  },
  {
    hexcode: '1F680',
    shortcodes: ['rocket'],
    emoji: '🚀',
    order: 2793,
    group: 5,
  },
  {
    hexcode: '1F6F8',
    shortcodes: ['flying_saucer'],
    emoji: '🛸',
    order: 2794,
    group: 5,
  },
  {
    hexcode: '1F6CE',
    shortcodes: ['bellhop'],
    emoji: '🛎️',
    order: 2796,
    group: 5,
  },
  {
    hexcode: '1F9F3',
    shortcodes: ['luggage'],
    emoji: '🧳',
    order: 2797,
    group: 5,
  },
  {
    hexcode: '231B',
    shortcodes: ['hourglass'],
    emoji: '⌛️',
    order: 2798,
    group: 5,
  },
  {
    hexcode: '23F3',
    shortcodes: ['hourglass_flowing'],
    emoji: '⏳️',
    order: 2799,
    group: 5,
  },
  {
    hexcode: '231A',
    shortcodes: ['watch'],
    emoji: '⌚️',
    order: 2800,
    group: 5,
  },
  {
    hexcode: '23F0',
    shortcodes: ['alarm_clock'],
    emoji: '⏰',
    order: 2801,
    group: 5,
  },
  {
    hexcode: '23F1',
    shortcodes: ['stopwatch'],
    emoji: '⏱️',
    order: 2803,
    group: 5,
  },
  {
    hexcode: '23F2',
    shortcodes: ['timer'],
    emoji: '⏲️',
    order: 2805,
    group: 5,
  },
  {
    hexcode: '1F570',
    shortcodes: ['clock'],
    emoji: '🕰️',
    order: 2807,
    group: 5,
  },
  {
    hexcode: '1F55B',
    shortcodes: ['clock12'],
    emoji: '🕛️',
    order: 2808,
    group: 5,
  },
  {
    hexcode: '1F567',
    shortcodes: ['clock1230'],
    emoji: '🕧️',
    order: 2809,
    group: 5,
  },
  {
    hexcode: '1F550',
    shortcodes: ['clock1'],
    emoji: '🕐️',
    order: 2810,
    group: 5,
  },
  {
    hexcode: '1F55C',
    shortcodes: ['clock130'],
    emoji: '🕜️',
    order: 2811,
    group: 5,
  },
  {
    hexcode: '1F551',
    shortcodes: ['clock2'],
    emoji: '🕑️',
    order: 2812,
    group: 5,
  },
  {
    hexcode: '1F55D',
    shortcodes: ['clock230'],
    emoji: '🕝️',
    order: 2813,
    group: 5,
  },
  {
    hexcode: '1F552',
    shortcodes: ['clock3'],
    emoji: '🕒️',
    order: 2814,
    group: 5,
  },
  {
    hexcode: '1F55E',
    shortcodes: ['clock330'],
    emoji: '🕞️',
    order: 2815,
    group: 5,
  },
  {
    hexcode: '1F553',
    shortcodes: ['clock4'],
    emoji: '🕓️',
    order: 2816,
    group: 5,
  },
  {
    hexcode: '1F55F',
    shortcodes: ['clock430'],
    emoji: '🕟️',
    order: 2817,
    group: 5,
  },
  {
    hexcode: '1F554',
    shortcodes: ['clock5'],
    emoji: '🕔️',
    order: 2818,
    group: 5,
  },
  {
    hexcode: '1F560',
    shortcodes: ['clock530'],
    emoji: '🕠️',
    order: 2819,
    group: 5,
  },
  {
    hexcode: '1F555',
    shortcodes: ['clock6'],
    emoji: '🕕️',
    order: 2820,
    group: 5,
  },
  {
    hexcode: '1F561',
    shortcodes: ['clock630'],
    emoji: '🕡️',
    order: 2821,
    group: 5,
  },
  {
    hexcode: '1F556',
    shortcodes: ['clock7'],
    emoji: '🕖️',
    order: 2822,
    group: 5,
  },
  {
    hexcode: '1F562',
    shortcodes: ['clock730'],
    emoji: '🕢️',
    order: 2823,
    group: 5,
  },
  {
    hexcode: '1F557',
    shortcodes: ['clock8'],
    emoji: '🕗️',
    order: 2824,
    group: 5,
  },
  {
    hexcode: '1F563',
    shortcodes: ['clock830'],
    emoji: '🕣️',
    order: 2825,
    group: 5,
  },
  {
    hexcode: '1F558',
    shortcodes: ['clock9'],
    emoji: '🕘️',
    order: 2826,
    group: 5,
  },
  {
    hexcode: '1F564',
    shortcodes: ['clock930'],
    emoji: '🕤️',
    order: 2827,
    group: 5,
  },
  {
    hexcode: '1F559',
    shortcodes: ['clock10'],
    emoji: '🕙️',
    order: 2828,
    group: 5,
  },
  {
    hexcode: '1F565',
    shortcodes: ['clock1030'],
    emoji: '🕥️',
    order: 2829,
    group: 5,
  },
  {
    hexcode: '1F55A',
    shortcodes: ['clock11'],
    emoji: '🕚️',
    order: 2830,
    group: 5,
  },
  {
    hexcode: '1F566',
    shortcodes: ['clock1130'],
    emoji: '🕦️',
    order: 2831,
    group: 5,
  },
  {
    hexcode: '1F311',
    shortcodes: ['new_moon'],
    emoji: '🌑',
    order: 2832,
    group: 5,
  },
  {
    hexcode: '1F312',
    shortcodes: ['waxing_crescent_moon'],
    emoji: '🌒',
    order: 2833,
    group: 5,
  },
  {
    hexcode: '1F313',
    shortcodes: ['first_quarter_moon'],
    emoji: '🌓',
    order: 2834,
    group: 5,
  },
  {
    hexcode: '1F314',
    shortcodes: ['waxing_gibbous_moon'],
    emoji: '🌔',
    order: 2835,
    group: 5,
  },
  {
    hexcode: '1F315',
    shortcodes: ['full_moon'],
    emoji: '🌕️',
    order: 2836,
    group: 5,
  },
  {
    hexcode: '1F316',
    shortcodes: ['waning_gibbous_moon'],
    emoji: '🌖',
    order: 2837,
    group: 5,
  },
  {
    hexcode: '1F317',
    shortcodes: ['last_quarter_moon'],
    emoji: '🌗',
    order: 2838,
    group: 5,
  },
  {
    hexcode: '1F318',
    shortcodes: ['waning_crescent_moon'],
    emoji: '🌘',
    order: 2839,
    group: 5,
  },
  {
    hexcode: '1F319',
    shortcodes: ['crescent_moon'],
    emoji: '🌙',
    order: 2840,
    group: 5,
  },
  {
    hexcode: '1F31A',
    shortcodes: ['new_moon_face'],
    emoji: '🌚',
    order: 2841,
    group: 5,
  },
  {
    hexcode: '1F31B',
    shortcodes: ['first_quarter_moon_face'],
    emoji: '🌛',
    order: 2842,
    group: 5,
  },
  {
    hexcode: '1F31C',
    shortcodes: ['last_quarter_moon_face'],
    emoji: '🌜️',
    order: 2843,
    group: 5,
  },
  {
    hexcode: '1F321',
    shortcodes: ['thermometer'],
    emoji: '🌡️',
    order: 2845,
    group: 5,
  },
  {
    hexcode: '2600',
    shortcodes: ['sun'],
    emoji: '☀️',
    order: 2847,
    group: 5,
  },
  {
    hexcode: '1F31D',
    shortcodes: ['full_moon_face'],
    emoji: '🌝',
    order: 2848,
    group: 5,
  },
  {
    hexcode: '1F31E',
    shortcodes: ['sun_face'],
    emoji: '🌞',
    order: 2849,
    group: 5,
  },
  {
    hexcode: '1FA90',
    shortcodes: ['ringed_planet', 'saturn'],
    emoji: '🪐',
    order: 2850,
    group: 5,
  },
  {
    hexcode: '2B50',
    shortcodes: ['star'],
    emoji: '⭐️',
    order: 2851,
    group: 5,
  },
  {
    hexcode: '1F31F',
    shortcodes: ['star2', 'glowing_star'],
    emoji: '🌟',
    order: 2852,
    group: 5,
  },
  {
    hexcode: '1F320',
    shortcodes: ['star3', 'shooting_star'],
    emoji: '🌠',
    order: 2853,
    group: 5,
  },
  {
    hexcode: '1F30C',
    shortcodes: ['milky_way'],
    emoji: '🌌',
    order: 2854,
    group: 5,
  },
  {
    hexcode: '2601',
    shortcodes: ['cloud'],
    emoji: '☁️',
    order: 2856,
    group: 5,
  },
  {
    hexcode: '26C5',
    shortcodes: ['partly_sunny'],
    emoji: '⛅️',
    order: 2857,
    group: 5,
  },
  {
    hexcode: '26C8',
    shortcodes: ['storm'],
    emoji: '⛈️',
    order: 2859,
    group: 5,
  },
  {
    hexcode: '1F324',
    shortcodes: ['overcast'],
    emoji: '🌤️',
    order: 2861,
    group: 5,
  },
  {
    hexcode: '1F325',
    shortcodes: ['cloudy'],
    emoji: '🌥️',
    order: 2863,
    group: 5,
  },
  {
    hexcode: '1F326',
    shortcodes: ['sunshower'],
    emoji: '🌦️',
    order: 2865,
    group: 5,
  },
  {
    hexcode: '1F327',
    shortcodes: ['rain'],
    emoji: '🌧️',
    order: 2867,
    group: 5,
  },
  {
    hexcode: '1F328',
    shortcodes: ['snow'],
    emoji: '🌨️',
    order: 2869,
    group: 5,
  },
  {
    hexcode: '1F329',
    shortcodes: ['lightning'],
    emoji: '🌩️',
    order: 2871,
    group: 5,
  },
  {
    hexcode: '1F32A',
    shortcodes: ['tornado'],
    emoji: '🌪️',
    order: 2873,
    group: 5,
  },
  {
    hexcode: '1F32B',
    shortcodes: ['fog'],
    emoji: '🌫️',
    order: 2875,
    group: 5,
  },
  {
    hexcode: '1F32C',
    shortcodes: ['wind_face'],
    emoji: '🌬️',
    order: 2877,
    group: 5,
  },
  {
    hexcode: '1F300',
    shortcodes: ['cyclone'],
    emoji: '🌀',
    order: 2878,
    group: 5,
  },
  {
    hexcode: '1F308',
    shortcodes: ['rainbow'],
    emoji: '🌈',
    order: 2879,
    group: 5,
  },
  {
    hexcode: '1F302',
    shortcodes: ['closed_umbrella'],
    emoji: '🌂',
    order: 2880,
    group: 5,
  },
  {
    hexcode: '2602',
    shortcodes: ['umbrella'],
    emoji: '☂️',
    order: 2882,
    group: 5,
  },
  {
    hexcode: '2614',
    shortcodes: ['umbrella_rain'],
    emoji: '☔️',
    order: 2883,
    group: 5,
  },
  {
    hexcode: '26F1',
    shortcodes: ['beach_umbrella'],
    emoji: '⛱️',
    order: 2885,
    group: 5,
  },
  {
    hexcode: '26A1',
    shortcodes: ['zap', 'high_voltage'],
    emoji: '⚡️',
    order: 2886,
    group: 5,
  },
  {
    hexcode: '2744',
    shortcodes: ['snowflake'],
    emoji: '❄️',
    order: 2888,
    group: 5,
  },
  {
    hexcode: '2603',
    shortcodes: ['snowy_snowman'],
    emoji: '☃️',
    order: 2890,
    group: 5,
  },
  {
    hexcode: '26C4',
    shortcodes: ['snowman'],
    emoji: '⛄️',
    order: 2891,
    group: 5,
  },
  {
    hexcode: '2604',
    shortcodes: ['comet'],
    emoji: '☄️',
    order: 2893,
    group: 5,
  },
  {
    hexcode: '1F525',
    shortcodes: ['fire'],
    emoji: '🔥',
    order: 2894,
    group: 5,
  },
  {
    hexcode: '1F4A7',
    shortcodes: ['droplet'],
    emoji: '💧',
    order: 2895,
    group: 5,
  },
  {
    hexcode: '1F30A',
    shortcodes: ['ocean'],
    emoji: '🌊',
    order: 2896,
    group: 5,
  },
  {
    hexcode: '1F383',
    shortcodes: ['jack_o_lantern'],
    emoji: '🎃',
    order: 2897,
    group: 6,
  },
  {
    hexcode: '1F384',
    shortcodes: ['christmas_tree', 'xmas_tree'],
    emoji: '🎄',
    order: 2898,
    group: 6,
  },
  {
    hexcode: '1F386',
    shortcodes: ['fireworks'],
    emoji: '🎆',
    order: 2899,
    group: 6,
  },
  {
    hexcode: '1F387',
    shortcodes: ['sparkler'],
    emoji: '🎇',
    order: 2900,
    group: 6,
  },
  {
    hexcode: '1F9E8',
    shortcodes: ['firecracker'],
    emoji: '🧨',
    order: 2901,
    group: 6,
  },
  {
    hexcode: '2728',
    shortcodes: ['sparkles'],
    emoji: '✨',
    order: 2902,
    group: 6,
  },
  {
    hexcode: '1F388',
    shortcodes: ['balloon'],
    emoji: '🎈',
    order: 2903,
    group: 6,
  },
  {
    hexcode: '1F389',
    shortcodes: ['tada', 'party'],
    emoji: '🎉',
    order: 2904,
    group: 6,
  },
  {
    hexcode: '1F38A',
    shortcodes: ['confetti_ball'],
    emoji: '🎊',
    order: 2905,
    group: 6,
  },
  {
    hexcode: '1F38B',
    shortcodes: ['tanabata_tree'],
    emoji: '🎋',
    order: 2906,
    group: 6,
  },
  {
    hexcode: '1F38D',
    shortcodes: ['bamboo', 'pine_decor'],
    emoji: '🎍',
    order: 2907,
    group: 6,
  },
  {
    hexcode: '1F38E',
    shortcodes: ['dolls'],
    emoji: '🎎',
    order: 2908,
    group: 6,
  },
  {
    hexcode: '1F38F',
    shortcodes: ['carp_streamer'],
    emoji: '🎏',
    order: 2909,
    group: 6,
  },
  {
    hexcode: '1F390',
    shortcodes: ['wind_chime'],
    emoji: '🎐',
    order: 2910,
    group: 6,
  },
  {
    hexcode: '1F391',
    shortcodes: ['moon_ceremony', 'rice_scene'],
    emoji: '🎑',
    order: 2911,
    group: 6,
  },
  {
    hexcode: '1F9E7',
    shortcodes: ['red_envelope'],
    emoji: '🧧',
    order: 2912,
    group: 6,
  },
  {
    hexcode: '1F380',
    shortcodes: ['ribbon'],
    emoji: '🎀',
    order: 2913,
    group: 6,
  },
  {
    hexcode: '1F381',
    shortcodes: ['gift'],
    emoji: '🎁',
    order: 2914,
    group: 6,
  },
  {
    hexcode: '1F397',
    shortcodes: ['reminder_ribbon'],
    emoji: '🎗️',
    order: 2916,
    group: 6,
  },
  {
    hexcode: '1F39F',
    shortcodes: ['tickets', 'admission'],
    emoji: '🎟️',
    order: 2918,
    group: 6,
  },
  {
    hexcode: '1F3AB',
    shortcodes: ['ticket'],
    emoji: '🎫',
    order: 2919,
    group: 6,
  },
  {
    hexcode: '1F396',
    shortcodes: ['military_medal'],
    emoji: '🎖️',
    order: 2921,
    group: 6,
  },
  {
    hexcode: '1F3C6',
    shortcodes: ['trophy'],
    emoji: '🏆️',
    order: 2922,
    group: 6,
  },
  {
    hexcode: '1F3C5',
    shortcodes: ['medal'],
    emoji: '🏅',
    order: 2923,
    group: 6,
  },
  {
    hexcode: '1F947',
    shortcodes: ['first_place'],
    emoji: '🥇',
    order: 2924,
    group: 6,
  },
  {
    hexcode: '1F948',
    shortcodes: ['second_place'],
    emoji: '🥈',
    order: 2925,
    group: 6,
  },
  {
    hexcode: '1F949',
    shortcodes: ['third_place'],
    emoji: '🥉',
    order: 2926,
    group: 6,
  },
  {
    hexcode: '26BD',
    shortcodes: ['soccer'],
    emoji: '⚽️',
    order: 2927,
    group: 6,
  },
  {
    hexcode: '26BE',
    shortcodes: ['baseball'],
    emoji: '⚾️',
    order: 2928,
    group: 6,
  },
  {
    hexcode: '1F94E',
    shortcodes: ['softball'],
    emoji: '🥎',
    order: 2929,
    group: 6,
  },
  {
    hexcode: '1F3C0',
    shortcodes: ['basketball'],
    emoji: '🏀',
    order: 2930,
    group: 6,
  },
  {
    hexcode: '1F3D0',
    shortcodes: ['volleyball'],
    emoji: '🏐',
    order: 2931,
    group: 6,
  },
  {
    hexcode: '1F3C8',
    shortcodes: ['football'],
    emoji: '🏈',
    order: 2932,
    group: 6,
  },
  {
    hexcode: '1F3C9',
    shortcodes: ['rugby'],
    emoji: '🏉',
    order: 2933,
    group: 6,
  },
  {
    hexcode: '1F3BE',
    shortcodes: ['tennis'],
    emoji: '🎾',
    order: 2934,
    group: 6,
  },
  {
    hexcode: '1F94F',
    shortcodes: ['flying_disc'],
    emoji: '🥏',
    order: 2935,
    group: 6,
  },
  {
    hexcode: '1F3B3',
    shortcodes: ['bowling'],
    emoji: '🎳',
    order: 2936,
    group: 6,
  },
  {
    hexcode: '1F3CF',
    shortcodes: ['cricket_game'],
    emoji: '🏏',
    order: 2937,
    group: 6,
  },
  {
    hexcode: '1F3D1',
    shortcodes: ['field_hockey'],
    emoji: '🏑',
    order: 2938,
    group: 6,
  },
  {
    hexcode: '1F3D2',
    shortcodes: ['hockey'],
    emoji: '🏒',
    order: 2939,
    group: 6,
  },
  {
    hexcode: '1F94D',
    shortcodes: ['lacrosse'],
    emoji: '🥍',
    order: 2940,
    group: 6,
  },
  {
    hexcode: '1F3D3',
    shortcodes: ['ping_pong'],
    emoji: '🏓',
    order: 2941,
    group: 6,
  },
  {
    hexcode: '1F3F8',
    shortcodes: ['badminton'],
    emoji: '🏸',
    order: 2942,
    group: 6,
  },
  {
    hexcode: '1F94A',
    shortcodes: ['boxing_glove'],
    emoji: '🥊',
    order: 2943,
    group: 6,
  },
  {
    hexcode: '1F94B',
    shortcodes: ['gi', 'martial_arts_uniform'],
    emoji: '🥋',
    order: 2944,
    group: 6,
  },
  {
    hexcode: '1F945',
    shortcodes: ['goal'],
    emoji: '🥅',
    order: 2945,
    group: 6,
  },
  {
    hexcode: '26F3',
    shortcodes: ['golf'],
    emoji: '⛳️',
    order: 2946,
    group: 6,
  },
  {
    hexcode: '26F8',
    shortcodes: ['ice_skate'],
    emoji: '⛸️',
    order: 2948,
    group: 6,
  },
  {
    hexcode: '1F3A3',
    shortcodes: ['fishing_pole'],
    emoji: '🎣',
    order: 2949,
    group: 6,
  },
  {
    hexcode: '1F93F',
    shortcodes: ['diving_mask', 'scuba_mask'],
    emoji: '🤿',
    order: 2950,
    group: 6,
  },
  {
    hexcode: '1F3BD',
    shortcodes: ['running_shirt'],
    emoji: '🎽',
    order: 2951,
    group: 6,
  },
  {
    hexcode: '1F3BF',
    shortcodes: ['ski'],
    emoji: '🎿',
    order: 2952,
    group: 6,
  },
  {
    hexcode: '1F6F7',
    shortcodes: ['sled'],
    emoji: '🛷',
    order: 2953,
    group: 6,
  },
  {
    hexcode: '1F94C',
    shortcodes: ['curling_stone'],
    emoji: '🥌',
    order: 2954,
    group: 6,
  },
  {
    hexcode: '1F3AF',
    shortcodes: ['dart'],
    emoji: '🎯',
    order: 2955,
    group: 6,
  },
  {
    hexcode: '1FA80',
    shortcodes: ['yoyo'],
    emoji: '🪀',
    order: 2956,
    group: 6,
  },
  {
    hexcode: '1FA81',
    shortcodes: ['kite'],
    emoji: '🪁',
    order: 2957,
    group: 6,
  },
  {
    hexcode: '1F3B1',
    shortcodes: ['8ball'],
    emoji: '🎱',
    order: 2958,
    group: 6,
  },
  {
    hexcode: '1F52E',
    shortcodes: ['crystal_ball'],
    emoji: '🔮',
    order: 2959,
    group: 6,
  },
  {
    hexcode: '1F9FF',
    shortcodes: ['nazar_amulet'],
    emoji: '🧿',
    order: 2960,
    group: 6,
  },
  {
    hexcode: '1F3AE',
    shortcodes: ['video_game'],
    emoji: '🎮️',
    order: 2961,
    group: 6,
  },
  {
    hexcode: '1F579',
    shortcodes: ['joystick'],
    emoji: '🕹️',
    order: 2963,
    group: 6,
  },
  {
    hexcode: '1F3B0',
    shortcodes: ['slot_machine'],
    emoji: '🎰',
    order: 2964,
    group: 6,
  },
  {
    hexcode: '1F3B2',
    shortcodes: ['game_die'],
    emoji: '🎲',
    order: 2965,
    group: 6,
  },
  {
    hexcode: '1F9E9',
    shortcodes: ['jigsaw', 'puzzle_piece'],
    emoji: '🧩',
    order: 2966,
    group: 6,
  },
  {
    hexcode: '1F9F8',
    shortcodes: ['teddy_bear'],
    emoji: '🧸',
    order: 2967,
    group: 6,
  },
  {
    hexcode: '2660',
    shortcodes: ['spades'],
    emoji: '♠️',
    order: 2969,
    group: 6,
  },
  {
    hexcode: '2665',
    shortcodes: ['hearts'],
    emoji: '♥️',
    order: 2971,
    group: 6,
  },
  {
    hexcode: '2666',
    shortcodes: ['diamonds'],
    emoji: '♦️',
    order: 2973,
    group: 6,
  },
  {
    hexcode: '2663',
    shortcodes: ['clubs'],
    emoji: '♣️',
    order: 2975,
    group: 6,
  },
  {
    hexcode: '265F',
    shortcodes: ['chess_pawn'],
    emoji: '♟️',
    order: 2977,
    group: 6,
  },
  {
    hexcode: '1F0CF',
    shortcodes: ['black_joker'],
    emoji: '🃏',
    order: 2978,
    group: 6,
  },
  {
    hexcode: '1F004',
    shortcodes: ['mahjong'],
    emoji: '🀄️',
    order: 2979,
    group: 6,
  },
  {
    hexcode: '1F3B4',
    shortcodes: ['flower_cards'],
    emoji: '🎴',
    order: 2980,
    group: 6,
  },
  {
    hexcode: '1F3AD',
    shortcodes: ['performing_arts'],
    emoji: '🎭️',
    order: 2981,
    group: 6,
  },
  {
    hexcode: '1F5BC',
    shortcodes: ['frame_photo'],
    emoji: '🖼️',
    order: 2983,
    group: 6,
  },
  {
    hexcode: '1F3A8',
    shortcodes: ['art', 'palette'],
    emoji: '🎨',
    order: 2984,
    group: 6,
  },
  {
    hexcode: '1F9F5',
    shortcodes: ['spool'],
    emoji: '🧵',
    order: 2985,
    group: 6,
  },
  {
    hexcode: '1F9F6',
    shortcodes: ['yarn'],
    emoji: '🧶',
    order: 2986,
    group: 6,
  },
  {
    hexcode: '1F453',
    shortcodes: ['glasses'],
    emoji: '👓️',
    order: 2987,
    group: 7,
  },
  {
    hexcode: '1F576',
    shortcodes: ['sunglasses'],
    emoji: '🕶️',
    order: 2989,
    group: 7,
  },
  {
    hexcode: '1F97D',
    shortcodes: ['goggles'],
    emoji: '🥽',
    order: 2990,
    group: 7,
  },
  {
    hexcode: '1F97C',
    shortcodes: ['lab_coat'],
    emoji: '🥼',
    order: 2991,
    group: 7,
  },
  {
    hexcode: '1F9BA',
    shortcodes: ['safety_vest'],
    emoji: '🦺',
    order: 2992,
    group: 7,
  },
  {
    hexcode: '1F454',
    shortcodes: ['necktie', 'tie'],
    emoji: '👔',
    order: 2993,
    group: 7,
  },
  {
    hexcode: '1F455',
    shortcodes: ['shirt'],
    emoji: '👕',
    order: 2994,
    group: 7,
  },
  {
    hexcode: '1F456',
    shortcodes: ['jeans'],
    emoji: '👖',
    order: 2995,
    group: 7,
  },
  {
    hexcode: '1F9E3',
    shortcodes: ['scarf'],
    emoji: '🧣',
    order: 2996,
    group: 7,
  },
  {
    hexcode: '1F9E4',
    shortcodes: ['gloves'],
    emoji: '🧤',
    order: 2997,
    group: 7,
  },
  {
    hexcode: '1F9E5',
    shortcodes: ['coat'],
    emoji: '🧥',
    order: 2998,
    group: 7,
  },
  {
    hexcode: '1F9E6',
    shortcodes: ['socks'],
    emoji: '🧦',
    order: 2999,
    group: 7,
  },
  {
    hexcode: '1F457',
    shortcodes: ['dress'],
    emoji: '👗',
    order: 3000,
    group: 7,
  },
  {
    hexcode: '1F458',
    shortcodes: ['kimono'],
    emoji: '👘',
    order: 3001,
    group: 7,
  },
  {
    hexcode: '1F97B',
    shortcodes: ['sari'],
    emoji: '🥻',
    order: 3002,
    group: 7,
  },
  {
    hexcode: '1FA71',
    shortcodes: ['one_piece_swimsuit'],
    emoji: '🩱',
    order: 3003,
    group: 7,
  },
  {
    hexcode: '1FA72',
    shortcodes: ['briefs'],
    emoji: '🩲',
    order: 3004,
    group: 7,
  },
  {
    hexcode: '1FA73',
    shortcodes: ['shorts'],
    emoji: '🩳',
    order: 3005,
    group: 7,
  },
  {
    hexcode: '1F459',
    shortcodes: ['bikini'],
    emoji: '👙',
    order: 3006,
    group: 7,
  },
  {
    hexcode: '1F45A',
    shortcodes: ['blouse', 'womans_clothes'],
    emoji: '👚',
    order: 3007,
    group: 7,
  },
  {
    hexcode: '1F45B',
    shortcodes: ['purse'],
    emoji: '👛',
    order: 3008,
    group: 7,
  },
  {
    hexcode: '1F45C',
    shortcodes: ['handbag'],
    emoji: '👜',
    order: 3009,
    group: 7,
  },
  {
    hexcode: '1F45D',
    shortcodes: ['pouch', 'clutch_bag'],
    emoji: '👝',
    order: 3010,
    group: 7,
  },
  {
    hexcode: '1F6CD',
    shortcodes: ['shopping_bags'],
    emoji: '🛍️',
    order: 3012,
    group: 7,
  },
  {
    hexcode: '1F392',
    shortcodes: ['backpack'],
    emoji: '🎒',
    order: 3013,
    group: 7,
  },
  {
    hexcode: '1F45E',
    shortcodes: ['dress_shoe', 'mans_shoe'],
    emoji: '👞',
    order: 3014,
    group: 7,
  },
  {
    hexcode: '1F45F',
    shortcodes: ['sneaker', 'athletic_shoe'],
    emoji: '👟',
    order: 3015,
    group: 7,
  },
  {
    hexcode: '1F97E',
    shortcodes: ['hiking_boot'],
    emoji: '🥾',
    order: 3016,
    group: 7,
  },
  {
    hexcode: '1F97F',
    shortcodes: ['flat_shoe'],
    emoji: '🥿',
    order: 3017,
    group: 7,
  },
  {
    hexcode: '1F460',
    shortcodes: ['high_heel'],
    emoji: '👠',
    order: 3018,
    group: 7,
  },
  {
    hexcode: '1F461',
    shortcodes: ['womans_sandal'],
    emoji: '👡',
    order: 3019,
    group: 7,
  },
  {
    hexcode: '1FA70',
    shortcodes: ['ballet_shoes'],
    emoji: '🩰',
    order: 3020,
    group: 7,
  },
  {
    hexcode: '1F462',
    shortcodes: ['womans_boot'],
    emoji: '👢',
    order: 3021,
    group: 7,
  },
  {
    hexcode: '1F451',
    shortcodes: ['crown'],
    emoji: '👑',
    order: 3022,
    group: 7,
  },
  {
    hexcode: '1F452',
    shortcodes: ['womans_hat'],
    emoji: '👒',
    order: 3023,
    group: 7,
  },
  {
    hexcode: '1F3A9',
    shortcodes: ['top_hat'],
    emoji: '🎩',
    order: 3024,
    group: 7,
  },
  {
    hexcode: '1F393',
    shortcodes: ['graduation_cap'],
    emoji: '🎓️',
    order: 3025,
    group: 7,
  },
  {
    hexcode: '1F9E2',
    shortcodes: ['billed_cap'],
    emoji: '🧢',
    order: 3026,
    group: 7,
  },
  {
    hexcode: '26D1',
    shortcodes: ['helmet_cross'],
    emoji: '⛑️',
    order: 3028,
    group: 7,
  },
  {
    hexcode: '1F4FF',
    shortcodes: ['prayer_beads'],
    emoji: '📿',
    order: 3029,
    group: 7,
  },
  {
    hexcode: '1F484',
    shortcodes: ['lipstick'],
    emoji: '💄',
    order: 3030,
    group: 7,
  },
  {
    hexcode: '1F48D',
    shortcodes: ['ring'],
    emoji: '💍',
    order: 3031,
    group: 7,
  },
  {
    hexcode: '1F48E',
    shortcodes: ['gem'],
    emoji: '💎',
    order: 3032,
    group: 7,
  },
  {
    hexcode: '1F507',
    shortcodes: ['mute', 'no_sound'],
    emoji: '🔇',
    order: 3033,
    group: 7,
  },
  {
    hexcode: '1F508',
    shortcodes: ['speaker', 'low_sound'],
    emoji: '🔈️',
    order: 3034,
    group: 7,
  },
  {
    hexcode: '1F509',
    shortcodes: ['sound'],
    emoji: '🔉',
    order: 3035,
    group: 7,
  },
  {
    hexcode: '1F50A',
    shortcodes: ['loud_sound'],
    emoji: '🔊',
    order: 3036,
    group: 7,
  },
  {
    hexcode: '1F4E2',
    shortcodes: ['loudspeaker'],
    emoji: '📢',
    order: 3037,
    group: 7,
  },
  {
    hexcode: '1F4E3',
    shortcodes: ['megaphone'],
    emoji: '📣',
    order: 3038,
    group: 7,
  },
  {
    hexcode: '1F4EF',
    shortcodes: ['postal_horn'],
    emoji: '📯',
    order: 3039,
    group: 7,
  },
  {
    hexcode: '1F514',
    shortcodes: ['bell'],
    emoji: '🔔',
    order: 3040,
    group: 7,
  },
  {
    hexcode: '1F515',
    shortcodes: ['no_bell'],
    emoji: '🔕',
    order: 3041,
    group: 7,
  },
  {
    hexcode: '1F3BC',
    shortcodes: ['musical_score'],
    emoji: '🎼',
    order: 3042,
    group: 7,
  },
  {
    hexcode: '1F3B5',
    shortcodes: ['musical_note'],
    emoji: '🎵',
    order: 3043,
    group: 7,
  },
  {
    hexcode: '1F3B6',
    shortcodes: ['musical_notes'],
    emoji: '🎶',
    order: 3044,
    group: 7,
  },
  {
    hexcode: '1F399',
    shortcodes: ['studio_microphone'],
    emoji: '🎙️',
    order: 3046,
    group: 7,
  },
  {
    hexcode: '1F39A',
    shortcodes: ['level_slider'],
    emoji: '🎚️',
    order: 3048,
    group: 7,
  },
  {
    hexcode: '1F39B',
    shortcodes: ['control_knobs'],
    emoji: '🎛️',
    order: 3050,
    group: 7,
  },
  {
    hexcode: '1F3A4',
    shortcodes: ['microphone'],
    emoji: '🎤',
    order: 3051,
    group: 7,
  },
  {
    hexcode: '1F3A7',
    shortcodes: ['headphones'],
    emoji: '🎧️',
    order: 3052,
    group: 7,
  },
  {
    hexcode: '1F4FB',
    shortcodes: ['radio'],
    emoji: '📻️',
    order: 3053,
    group: 7,
  },
  {
    hexcode: '1F3B7',
    shortcodes: ['saxophone'],
    emoji: '🎷',
    order: 3054,
    group: 7,
  },
  {
    hexcode: '1F3B8',
    shortcodes: ['guitar'],
    emoji: '🎸',
    order: 3055,
    group: 7,
  },
  {
    hexcode: '1F3B9',
    shortcodes: ['musical_keyboard'],
    emoji: '🎹',
    order: 3056,
    group: 7,
  },
  {
    hexcode: '1F3BA',
    shortcodes: ['trumpet'],
    emoji: '🎺',
    order: 3057,
    group: 7,
  },
  {
    hexcode: '1F3BB',
    shortcodes: ['violin'],
    emoji: '🎻',
    order: 3058,
    group: 7,
  },
  {
    hexcode: '1FA95',
    shortcodes: ['banjo'],
    emoji: '🪕',
    order: 3059,
    group: 7,
  },
  {
    hexcode: '1F941',
    shortcodes: ['drum'],
    emoji: '🥁',
    order: 3060,
    group: 7,
  },
  {
    hexcode: '1F4F1',
    shortcodes: ['mobile', 'iphone', 'android'],
    emoji: '📱',
    order: 3061,
    group: 7,
  },
  {
    hexcode: '1F4F2',
    shortcodes: ['mobile_calling'],
    emoji: '📲',
    order: 3062,
    group: 7,
  },
  {
    hexcode: '260E',
    shortcodes: ['telephone'],
    emoji: '☎️',
    order: 3064,
    group: 7,
  },
  {
    hexcode: '1F4DE',
    shortcodes: ['telephone_receiver'],
    emoji: '📞',
    order: 3065,
    group: 7,
  },
  {
    hexcode: '1F4DF',
    shortcodes: ['pager'],
    emoji: '📟️',
    order: 3066,
    group: 7,
  },
  {
    hexcode: '1F4E0',
    shortcodes: ['fax'],
    emoji: '📠',
    order: 3067,
    group: 7,
  },
  {
    hexcode: '1F50B',
    shortcodes: ['battery'],
    emoji: '🔋',
    order: 3068,
    group: 7,
  },
  {
    hexcode: '1F50C',
    shortcodes: ['electric_plug'],
    emoji: '🔌',
    order: 3069,
    group: 7,
  },
  {
    hexcode: '1F4BB',
    shortcodes: ['laptop'],
    emoji: '💻️',
    order: 3070,
    group: 7,
  },
  {
    hexcode: '1F5A5',
    shortcodes: ['desktop', 'computer'],
    emoji: '🖥️',
    order: 3072,
    group: 7,
  },
  {
    hexcode: '1F5A8',
    shortcodes: ['printer'],
    emoji: '🖨️',
    order: 3074,
    group: 7,
  },
  {
    hexcode: '2328',
    shortcodes: ['keyboard'],
    emoji: '⌨️',
    order: 3076,
    group: 7,
  },
  {
    hexcode: '1F5B1',
    shortcodes: ['computer_mouse'],
    emoji: '🖱️',
    order: 3078,
    group: 7,
  },
  {
    hexcode: '1F5B2',
    shortcodes: ['trackball'],
    emoji: '🖲️',
    order: 3080,
    group: 7,
  },
  {
    hexcode: '1F4BD',
    shortcodes: ['minidisc'],
    emoji: '💽',
    order: 3081,
    group: 7,
  },
  {
    hexcode: '1F4BE',
    shortcodes: ['floppy_disk'],
    emoji: '💾',
    order: 3082,
    group: 7,
  },
  {
    hexcode: '1F4BF',
    shortcodes: ['cd', 'disk'],
    emoji: '💿️',
    order: 3083,
    group: 7,
  },
  {
    hexcode: '1F4C0',
    shortcodes: ['dvd'],
    emoji: '📀',
    order: 3084,
    group: 7,
  },
  {
    hexcode: '1F9EE',
    shortcodes: ['abacus'],
    emoji: '🧮',
    order: 3085,
    group: 7,
  },
  {
    hexcode: '1F3A5',
    shortcodes: ['movie_camera'],
    emoji: '🎥',
    order: 3086,
    group: 7,
  },
  {
    hexcode: '1F39E',
    shortcodes: ['film_frames'],
    emoji: '🎞️',
    order: 3088,
    group: 7,
  },
  {
    hexcode: '1F4FD',
    shortcodes: ['projector'],
    emoji: '📽️',
    order: 3090,
    group: 7,
  },
  {
    hexcode: '1F3AC',
    shortcodes: ['clapper'],
    emoji: '🎬️',
    order: 3091,
    group: 7,
  },
  {
    hexcode: '1F4FA',
    shortcodes: ['tv'],
    emoji: '📺️',
    order: 3092,
    group: 7,
  },
  {
    hexcode: '1F4F7',
    shortcodes: ['camera'],
    emoji: '📷️',
    order: 3093,
    group: 7,
  },
  {
    hexcode: '1F4F8',
    shortcodes: ['camera_flash'],
    emoji: '📸',
    order: 3094,
    group: 7,
  },
  {
    hexcode: '1F4F9',
    shortcodes: ['video_camera'],
    emoji: '📹️',
    order: 3095,
    group: 7,
  },
  {
    hexcode: '1F4FC',
    shortcodes: ['vhs'],
    emoji: '📼',
    order: 3096,
    group: 7,
  },
  {
    hexcode: '1F50D',
    shortcodes: ['mag'],
    emoji: '🔍️',
    order: 3097,
    group: 7,
  },
  {
    hexcode: '1F50E',
    shortcodes: ['mag_right'],
    emoji: '🔎',
    order: 3098,
    group: 7,
  },
  {
    hexcode: '1F56F',
    shortcodes: ['candle'],
    emoji: '🕯️',
    order: 3100,
    group: 7,
  },
  {
    hexcode: '1F4A1',
    shortcodes: ['bulb', 'light_bulb'],
    emoji: '💡',
    order: 3101,
    group: 7,
  },
  {
    hexcode: '1F526',
    shortcodes: ['flashlight'],
    emoji: '🔦',
    order: 3102,
    group: 7,
  },
  {
    hexcode: '1F3EE',
    shortcodes: ['red_lantern'],
    emoji: '🏮',
    order: 3103,
    group: 7,
  },
  {
    hexcode: '1FA94',
    shortcodes: ['diya_lamp'],
    emoji: '🪔',
    order: 3104,
    group: 7,
  },
  {
    hexcode: '1F4D4',
    shortcodes: ['decorative_notebook'],
    emoji: '📔',
    order: 3105,
    group: 7,
  },
  {
    hexcode: '1F4D5',
    shortcodes: ['closed_book'],
    emoji: '📕',
    order: 3106,
    group: 7,
  },
  {
    hexcode: '1F4D6',
    shortcodes: ['book'],
    emoji: '📖',
    order: 3107,
    group: 7,
  },
  {
    hexcode: '1F4D7',
    shortcodes: ['green_book'],
    emoji: '📗',
    order: 3108,
    group: 7,
  },
  {
    hexcode: '1F4D8',
    shortcodes: ['blue_book'],
    emoji: '📘',
    order: 3109,
    group: 7,
  },
  {
    hexcode: '1F4D9',
    shortcodes: ['orange_book'],
    emoji: '📙',
    order: 3110,
    group: 7,
  },
  {
    hexcode: '1F4DA',
    shortcodes: ['books'],
    emoji: '📚️',
    order: 3111,
    group: 7,
  },
  {
    hexcode: '1F4D3',
    shortcodes: ['notebook'],
    emoji: '📓',
    order: 3112,
    group: 7,
  },
  {
    hexcode: '1F4D2',
    shortcodes: ['ledger'],
    emoji: '📒',
    order: 3113,
    group: 7,
  },
  {
    hexcode: '1F4C3',
    shortcodes: ['page_curl'],
    emoji: '📃',
    order: 3114,
    group: 7,
  },
  {
    hexcode: '1F4DC',
    shortcodes: ['scroll'],
    emoji: '📜',
    order: 3115,
    group: 7,
  },
  {
    hexcode: '1F4C4',
    shortcodes: ['page_facing_up'],
    emoji: '📄',
    order: 3116,
    group: 7,
  },
  {
    hexcode: '1F4F0',
    shortcodes: ['newspaper'],
    emoji: '📰',
    order: 3117,
    group: 7,
  },
  {
    hexcode: '1F5DE',
    shortcodes: ['rolled_newspaper'],
    emoji: '🗞️',
    order: 3119,
    group: 7,
  },
  {
    hexcode: '1F4D1',
    shortcodes: ['bookmark_tabs'],
    emoji: '📑',
    order: 3120,
    group: 7,
  },
  {
    hexcode: '1F516',
    shortcodes: ['bookmark'],
    emoji: '🔖',
    order: 3121,
    group: 7,
  },
  {
    hexcode: '1F3F7',
    shortcodes: ['label'],
    emoji: '🏷️',
    order: 3123,
    group: 7,
  },
  {
    hexcode: '1F4B0',
    shortcodes: ['moneybag'],
    emoji: '💰️',
    order: 3124,
    group: 7,
  },
  {
    hexcode: '1F4B4',
    shortcodes: ['yen'],
    emoji: '💴',
    order: 3125,
    group: 7,
  },
  {
    hexcode: '1F4B5',
    shortcodes: ['dollar'],
    emoji: '💵',
    order: 3126,
    group: 7,
  },
  {
    hexcode: '1F4B6',
    shortcodes: ['euro'],
    emoji: '💶',
    order: 3127,
    group: 7,
  },
  {
    hexcode: '1F4B7',
    shortcodes: ['pound'],
    emoji: '💷',
    order: 3128,
    group: 7,
  },
  {
    hexcode: '1F4B8',
    shortcodes: ['money_wings'],
    emoji: '💸',
    order: 3129,
    group: 7,
  },
  {
    hexcode: '1F4B3',
    shortcodes: ['credit_card'],
    emoji: '💳️',
    order: 3130,
    group: 7,
  },
  {
    hexcode: '1F9FE',
    shortcodes: ['receipt'],
    emoji: '🧾',
    order: 3131,
    group: 7,
  },
  {
    hexcode: '1F4B9',
    shortcodes: ['ja_chart'],
    emoji: '💹',
    order: 3132,
    group: 7,
  },
  {
    hexcode: '1F4B1',
    shortcodes: ['currency_exchange'],
    emoji: '💱',
    order: 3133,
    group: 7,
  },
  {
    hexcode: '1F4B2',
    shortcodes: ['dollar_sign'],
    emoji: '💲',
    order: 3134,
    group: 7,
  },
  {
    hexcode: '2709',
    shortcodes: ['envelope'],
    emoji: '✉️',
    order: 3136,
    group: 7,
  },
  {
    hexcode: '1F4E7',
    shortcodes: ['email'],
    emoji: '📧',
    order: 3137,
    group: 7,
  },
  {
    hexcode: '1F4E8',
    shortcodes: ['incoming_envelope'],
    emoji: '📨',
    order: 3138,
    group: 7,
  },
  {
    hexcode: '1F4E9',
    shortcodes: ['envelope_arrow'],
    emoji: '📩',
    order: 3139,
    group: 7,
  },
  {
    hexcode: '1F4E4',
    shortcodes: ['outbox_tray'],
    emoji: '📤️',
    order: 3140,
    group: 7,
  },
  {
    hexcode: '1F4E5',
    shortcodes: ['inbox_tray'],
    emoji: '📥️',
    order: 3141,
    group: 7,
  },
  {
    hexcode: '1F4E6',
    shortcodes: ['package'],
    emoji: '📦️',
    order: 3142,
    group: 7,
  },
  {
    hexcode: '1F4EB',
    shortcodes: ['mailbox'],
    emoji: '📫️',
    order: 3143,
    group: 7,
  },
  {
    hexcode: '1F4EA',
    shortcodes: ['mailbox_closed'],
    emoji: '📪️',
    order: 3144,
    group: 7,
  },
  {
    hexcode: '1F4EC',
    shortcodes: ['mailbox_mail'],
    emoji: '📬️',
    order: 3145,
    group: 7,
  },
  {
    hexcode: '1F4ED',
    shortcodes: ['mailbox_no_mail'],
    emoji: '📭️',
    order: 3146,
    group: 7,
  },
  {
    hexcode: '1F4EE',
    shortcodes: ['postbox'],
    emoji: '📮',
    order: 3147,
    group: 7,
  },
  {
    hexcode: '1F5F3',
    shortcodes: ['ballot_box'],
    emoji: '🗳️',
    order: 3149,
    group: 7,
  },
  {
    hexcode: '270F',
    shortcodes: ['pencil'],
    emoji: '✏️',
    order: 3151,
    group: 7,
  },
  {
    hexcode: '2712',
    shortcodes: ['black_nib'],
    emoji: '✒️',
    order: 3153,
    group: 7,
  },
  {
    hexcode: '1F58B',
    shortcodes: ['fountain_pen'],
    emoji: '🖋️',
    order: 3155,
    group: 7,
  },
  {
    hexcode: '1F58A',
    shortcodes: ['pen'],
    emoji: '🖊️',
    order: 3157,
    group: 7,
  },
  {
    hexcode: '1F58C',
    shortcodes: ['paintbrush'],
    emoji: '🖌️',
    order: 3159,
    group: 7,
  },
  {
    hexcode: '1F58D',
    shortcodes: ['crayon'],
    emoji: '🖍️',
    order: 3161,
    group: 7,
  },
  {
    hexcode: '1F4DD',
    shortcodes: ['memo'],
    emoji: '📝',
    order: 3162,
    group: 7,
  },
  {
    hexcode: '1F4BC',
    shortcodes: ['briefcase'],
    emoji: '💼',
    order: 3163,
    group: 7,
  },
  {
    hexcode: '1F4C1',
    shortcodes: ['file_folder'],
    emoji: '📁',
    order: 3164,
    group: 7,
  },
  {
    hexcode: '1F4C2',
    shortcodes: ['open_file_folder'],
    emoji: '📂',
    order: 3165,
    group: 7,
  },
  {
    hexcode: '1F5C2',
    shortcodes: ['dividers'],
    emoji: '🗂️',
    order: 3167,
    group: 7,
  },
  {
    hexcode: '1F4C5',
    shortcodes: ['date', 'calendar'],
    emoji: '📅',
    order: 3168,
    group: 7,
  },
  {
    hexcode: '1F4C6',
    shortcodes: ['torn_calendar'],
    emoji: '📆',
    order: 3169,
    group: 7,
  },
  {
    hexcode: '1F5D2',
    shortcodes: ['notepad_spiral'],
    emoji: '🗒️',
    order: 3171,
    group: 7,
  },
  {
    hexcode: '1F5D3',
    shortcodes: ['calendar_spiral'],
    emoji: '🗓️',
    order: 3173,
    group: 7,
  },
  {
    hexcode: '1F4C7',
    shortcodes: ['card_index'],
    emoji: '📇',
    order: 3174,
    group: 7,
  },
  {
    hexcode: '1F4C8',
    shortcodes: ['chart_up'],
    emoji: '📈',
    order: 3175,
    group: 7,
  },
  {
    hexcode: '1F4C9',
    shortcodes: ['chart_down'],
    emoji: '📉',
    order: 3176,
    group: 7,
  },
  {
    hexcode: '1F4CA',
    shortcodes: ['bar_chart'],
    emoji: '📊',
    order: 3177,
    group: 7,
  },
  {
    hexcode: '1F4CB',
    shortcodes: ['clipboard'],
    emoji: '📋️',
    order: 3178,
    group: 7,
  },
  {
    hexcode: '1F4CC',
    shortcodes: ['pushpin'],
    emoji: '📌',
    order: 3179,
    group: 7,
  },
  {
    hexcode: '1F4CD',
    shortcodes: ['round_pushpin'],
    emoji: '📍',
    order: 3180,
    group: 7,
  },
  {
    hexcode: '1F4CE',
    shortcodes: ['paperclip'],
    emoji: '📎',
    order: 3181,
    group: 7,
  },
  {
    hexcode: '1F587',
    shortcodes: ['paperclips'],
    emoji: '🖇️',
    order: 3183,
    group: 7,
  },
  {
    hexcode: '1F4CF',
    shortcodes: ['straight_ruler'],
    emoji: '📏',
    order: 3184,
    group: 7,
  },
  {
    hexcode: '1F4D0',
    shortcodes: ['triangular_ruler'],
    emoji: '📐',
    order: 3185,
    group: 7,
  },
  {
    hexcode: '2702',
    shortcodes: ['scissors'],
    emoji: '✂️',
    order: 3187,
    group: 7,
  },
  {
    hexcode: '1F5C3',
    shortcodes: ['card_box'],
    emoji: '🗃️',
    order: 3189,
    group: 7,
  },
  {
    hexcode: '1F5C4',
    shortcodes: ['file_cabinet'],
    emoji: '🗄️',
    order: 3191,
    group: 7,
  },
  {
    hexcode: '1F5D1',
    shortcodes: ['trashcan', 'wastebasket'],
    emoji: '🗑️',
    order: 3193,
    group: 7,
  },
  {
    hexcode: '1F512',
    shortcodes: ['lock'],
    emoji: '🔒️',
    order: 3194,
    group: 7,
  },
  {
    hexcode: '1F513',
    shortcodes: ['unlock'],
    emoji: '🔓️',
    order: 3195,
    group: 7,
  },
  {
    hexcode: '1F50F',
    shortcodes: ['locked_pen'],
    emoji: '🔏',
    order: 3196,
    group: 7,
  },
  {
    hexcode: '1F510',
    shortcodes: ['locked_key'],
    emoji: '🔐',
    order: 3197,
    group: 7,
  },
  {
    hexcode: '1F511',
    shortcodes: ['key'],
    emoji: '🔑',
    order: 3198,
    group: 7,
  },
  {
    hexcode: '1F5DD',
    shortcodes: ['old_key'],
    emoji: '🗝️',
    order: 3200,
    group: 7,
  },
  {
    hexcode: '1F528',
    shortcodes: ['hammer'],
    emoji: '🔨',
    order: 3201,
    group: 7,
  },
  {
    hexcode: '1FA93',
    shortcodes: ['axe'],
    emoji: '🪓',
    order: 3202,
    group: 7,
  },
  {
    hexcode: '26CF',
    shortcodes: ['pick'],
    emoji: '⛏️',
    order: 3204,
    group: 7,
  },
  {
    hexcode: '2692',
    shortcodes: ['hammer_pick'],
    emoji: '⚒️',
    order: 3206,
    group: 7,
  },
  {
    hexcode: '1F6E0',
    shortcodes: ['tools', 'hammer_wrench'],
    emoji: '🛠️',
    order: 3208,
    group: 7,
  },
  {
    hexcode: '1F5E1',
    shortcodes: ['dagger'],
    emoji: '🗡️',
    order: 3210,
    group: 7,
  },
  {
    hexcode: '2694',
    shortcodes: ['crossed_swords'],
    emoji: '⚔️',
    order: 3212,
    group: 7,
  },
  {
    hexcode: '1F52B',
    shortcodes: ['gun', 'pistol'],
    emoji: '🔫',
    order: 3213,
    group: 7,
  },
  {
    hexcode: '1F3F9',
    shortcodes: ['bow'],
    emoji: '🏹',
    order: 3214,
    group: 7,
  },
  {
    hexcode: '1F6E1',
    shortcodes: ['shield'],
    emoji: '🛡️',
    order: 3216,
    group: 7,
  },
  {
    hexcode: '1F527',
    shortcodes: ['wrench'],
    emoji: '🔧',
    order: 3217,
    group: 7,
  },
  {
    hexcode: '1F529',
    shortcodes: ['nut_and_bolt'],
    emoji: '🔩',
    order: 3218,
    group: 7,
  },
  {
    hexcode: '2699',
    shortcodes: ['gear'],
    emoji: '⚙️',
    order: 3220,
    group: 7,
  },
  {
    hexcode: '1F5DC',
    shortcodes: ['clamp', 'compression'],
    emoji: '🗜️',
    order: 3222,
    group: 7,
  },
  {
    hexcode: '2696',
    shortcodes: ['scales'],
    emoji: '⚖️',
    order: 3224,
    group: 7,
  },
  {
    hexcode: '1F9AF',
    shortcodes: ['probing_cane'],
    emoji: '🦯',
    order: 3225,
    group: 7,
  },
  {
    hexcode: '1F517',
    shortcodes: ['link'],
    emoji: '🔗',
    order: 3226,
    group: 7,
  },
  {
    hexcode: '26D3',
    shortcodes: ['chains'],
    emoji: '⛓️',
    order: 3228,
    group: 7,
  },
  {
    hexcode: '1F9F0',
    shortcodes: ['toolbox'],
    emoji: '🧰',
    order: 3229,
    group: 7,
  },
  {
    hexcode: '1F9F2',
    shortcodes: ['magnet'],
    emoji: '🧲',
    order: 3230,
    group: 7,
  },
  {
    hexcode: '2697',
    shortcodes: ['alembic'],
    emoji: '⚗️',
    order: 3232,
    group: 7,
  },
  {
    hexcode: '1F9EA',
    shortcodes: ['test_tube'],
    emoji: '🧪',
    order: 3233,
    group: 7,
  },
  {
    hexcode: '1F9EB',
    shortcodes: ['petri_dish'],
    emoji: '🧫',
    order: 3234,
    group: 7,
  },
  {
    hexcode: '1F9EC',
    shortcodes: ['dna', 'double_helix'],
    emoji: '🧬',
    order: 3235,
    group: 7,
  },
  {
    hexcode: '1F52C',
    shortcodes: ['microscope'],
    emoji: '🔬',
    order: 3236,
    group: 7,
  },
  {
    hexcode: '1F52D',
    shortcodes: ['telescope'],
    emoji: '🔭',
    order: 3237,
    group: 7,
  },
  {
    hexcode: '1F4E1',
    shortcodes: ['satellite_antenna'],
    emoji: '📡',
    order: 3238,
    group: 7,
  },
  {
    hexcode: '1F489',
    shortcodes: ['syringe'],
    emoji: '💉',
    order: 3239,
    group: 7,
  },
  {
    hexcode: '1FA78',
    shortcodes: ['blood_drop'],
    emoji: '🩸',
    order: 3240,
    group: 7,
  },
  {
    hexcode: '1F48A',
    shortcodes: ['pill'],
    emoji: '💊',
    order: 3241,
    group: 7,
  },
  {
    hexcode: '1FA79',
    shortcodes: ['bandaid', 'adhesive_bandage'],
    emoji: '🩹',
    order: 3242,
    group: 7,
  },
  {
    hexcode: '1FA7A',
    shortcodes: ['stethoscope'],
    emoji: '🩺',
    order: 3243,
    group: 7,
  },
  {
    hexcode: '1F6AA',
    shortcodes: ['door'],
    emoji: '🚪',
    order: 3244,
    group: 7,
  },
  {
    hexcode: '1F6CF',
    shortcodes: ['bed'],
    emoji: '🛏️',
    order: 3246,
    group: 7,
  },
  {
    hexcode: '1F6CB',
    shortcodes: ['couch'],
    emoji: '🛋️',
    order: 3248,
    group: 7,
  },
  {
    hexcode: '1FA91',
    shortcodes: ['chair'],
    emoji: '🪑',
    order: 3249,
    group: 7,
  },
  {
    hexcode: '1F6BD',
    shortcodes: ['toilet'],
    emoji: '🚽',
    order: 3250,
    group: 7,
  },
  {
    hexcode: '1F6BF',
    shortcodes: ['shower'],
    emoji: '🚿',
    order: 3251,
    group: 7,
  },
  {
    hexcode: '1F6C1',
    shortcodes: ['bathtub'],
    emoji: '🛁',
    order: 3252,
    group: 7,
  },
  {
    hexcode: '1FA92',
    shortcodes: ['razor'],
    emoji: '🪒',
    order: 3253,
    group: 7,
  },
  {
    hexcode: '1F9F4',
    shortcodes: ['lotion'],
    emoji: '🧴',
    order: 3254,
    group: 7,
  },
  {
    hexcode: '1F9F7',
    shortcodes: ['safety_pin'],
    emoji: '🧷',
    order: 3255,
    group: 7,
  },
  {
    hexcode: '1F9F9',
    shortcodes: ['broom'],
    emoji: '🧹',
    order: 3256,
    group: 7,
  },
  {
    hexcode: '1F9FA',
    shortcodes: ['basket'],
    emoji: '🧺',
    order: 3257,
    group: 7,
  },
  {
    hexcode: '1F9FB',
    shortcodes: ['toilet_paper'],
    emoji: '🧻',
    order: 3258,
    group: 7,
  },
  {
    hexcode: '1F9FC',
    shortcodes: ['soap'],
    emoji: '🧼',
    order: 3259,
    group: 7,
  },
  {
    hexcode: '1F9FD',
    shortcodes: ['sponge'],
    emoji: '🧽',
    order: 3260,
    group: 7,
  },
  {
    hexcode: '1F9EF',
    shortcodes: ['fire_extinguisher'],
    emoji: '🧯',
    order: 3261,
    group: 7,
  },
  {
    hexcode: '1F6D2',
    shortcodes: ['shopping_cart'],
    emoji: '🛒',
    order: 3262,
    group: 7,
  },
  {
    hexcode: '1F6AC',
    shortcodes: ['cigarette', 'smoking'],
    emoji: '🚬',
    order: 3263,
    group: 7,
  },
  {
    hexcode: '26B0',
    shortcodes: ['coffin'],
    emoji: '⚰️',
    order: 3265,
    group: 7,
  },
  {
    hexcode: '26B1',
    shortcodes: ['urn'],
    emoji: '⚱️',
    order: 3267,
    group: 7,
  },
  {
    hexcode: '1F5FF',
    shortcodes: ['moai'],
    emoji: '🗿',
    order: 3268,
    group: 7,
  },
  {
    hexcode: '1F3E7',
    shortcodes: ['atm'],
    emoji: '🏧',
    order: 3269,
    group: 8,
  },
  {
    hexcode: '1F6AE',
    shortcodes: ['litter_bin'],
    emoji: '🚮',
    order: 3270,
    group: 8,
  },
  {
    hexcode: '1F6B0',
    shortcodes: ['potable_water'],
    emoji: '🚰',
    order: 3271,
    group: 8,
  },
  {
    hexcode: '267F',
    shortcodes: ['handicapped'],
    emoji: '♿️',
    order: 3272,
    group: 8,
  },
  {
    hexcode: '1F6B9',
    shortcodes: ['mens'],
    emoji: '🚹️',
    order: 3273,
    group: 8,
  },
  {
    hexcode: '1F6BA',
    shortcodes: ['womens'],
    emoji: '🚺️',
    order: 3274,
    group: 8,
  },
  {
    hexcode: '1F6BB',
    shortcodes: ['restroom', 'bathroom'],
    emoji: '🚻',
    order: 3275,
    group: 8,
  },
  {
    hexcode: '1F6BC',
    shortcodes: ['baby_symbol'],
    emoji: '🚼️',
    order: 3276,
    group: 8,
  },
  {
    hexcode: '1F6BE',
    shortcodes: ['wc'],
    emoji: '🚾',
    order: 3277,
    group: 8,
  },
  {
    hexcode: '1F6C2',
    shortcodes: ['passport_control'],
    emoji: '🛂',
    order: 3278,
    group: 8,
  },
  {
    hexcode: '1F6C3',
    shortcodes: ['customs'],
    emoji: '🛃',
    order: 3279,
    group: 8,
  },
  {
    hexcode: '1F6C4',
    shortcodes: ['baggage_claim'],
    emoji: '🛄',
    order: 3280,
    group: 8,
  },
  {
    hexcode: '1F6C5',
    shortcodes: ['left_luggage'],
    emoji: '🛅',
    order: 3281,
    group: 8,
  },
  {
    hexcode: '26A0',
    shortcodes: ['warning'],
    emoji: '⚠️',
    order: 3283,
    group: 8,
  },
  {
    hexcode: '1F6B8',
    shortcodes: ['children_crossing'],
    emoji: '🚸',
    order: 3284,
    group: 8,
  },
  {
    hexcode: '26D4',
    shortcodes: ['no_entry'],
    emoji: '⛔️',
    order: 3285,
    group: 8,
  },
  {
    hexcode: '1F6AB',
    shortcodes: ['no_entry_sign'],
    emoji: '🚫',
    order: 3286,
    group: 8,
  },
  {
    hexcode: '1F6B3',
    shortcodes: ['no_bicycles'],
    emoji: '🚳',
    order: 3287,
    group: 8,
  },
  {
    hexcode: '1F6AD',
    shortcodes: ['no_smoking'],
    emoji: '🚭️',
    order: 3288,
    group: 8,
  },
  {
    hexcode: '1F6AF',
    shortcodes: ['do_not_litter'],
    emoji: '🚯',
    order: 3289,
    group: 8,
  },
  {
    hexcode: '1F6B1',
    shortcodes: ['non_potable_water'],
    emoji: '🚱',
    order: 3290,
    group: 8,
  },
  {
    hexcode: '1F6B7',
    shortcodes: ['no_pedestrians'],
    emoji: '🚷',
    order: 3291,
    group: 8,
  },
  {
    hexcode: '1F4F5',
    shortcodes: ['no_mobile_phones'],
    emoji: '📵',
    order: 3292,
    group: 8,
  },
  {
    hexcode: '1F51E',
    shortcodes: ['underage'],
    emoji: '🔞',
    order: 3293,
    group: 8,
  },
  {
    hexcode: '2622',
    shortcodes: ['radioactive'],
    emoji: '☢️',
    order: 3295,
    group: 8,
  },
  {
    hexcode: '2623',
    shortcodes: ['biohazard'],
    emoji: '☣️',
    order: 3297,
    group: 8,
  },
  {
    hexcode: '2B06',
    shortcodes: ['arrow_up'],
    emoji: '⬆️',
    order: 3299,
    group: 8,
  },
  {
    hexcode: '2197',
    shortcodes: ['arrow_upper_right'],
    emoji: '↗️',
    order: 3301,
    group: 8,
  },
  {
    hexcode: '27A1',
    shortcodes: ['arrow_right'],
    emoji: '➡️',
    order: 3303,
    group: 8,
  },
  {
    hexcode: '2198',
    shortcodes: ['arrow_lower_right'],
    emoji: '↘️',
    order: 3305,
    group: 8,
  },
  {
    hexcode: '2B07',
    shortcodes: ['arrow_down'],
    emoji: '⬇️',
    order: 3307,
    group: 8,
  },
  {
    hexcode: '2199',
    shortcodes: ['arrow_lower_left'],
    emoji: '↙️',
    order: 3309,
    group: 8,
  },
  {
    hexcode: '2B05',
    shortcodes: ['arrow_left'],
    emoji: '⬅️',
    order: 3311,
    group: 8,
  },
  {
    hexcode: '2196',
    shortcodes: ['arrow_upper_left'],
    emoji: '↖️',
    order: 3313,
    group: 8,
  },
  {
    hexcode: '2195',
    shortcodes: ['arrow_up_down'],
    emoji: '↕️',
    order: 3315,
    group: 8,
  },
  {
    hexcode: '2194',
    shortcodes: ['arrow_left_right'],
    emoji: '↔️',
    order: 3317,
    group: 8,
  },
  {
    hexcode: '21A9',
    shortcodes: ['arrow_left_hook'],
    emoji: '↩️',
    order: 3319,
    group: 8,
  },
  {
    hexcode: '21AA',
    shortcodes: ['arrow_right_hook'],
    emoji: '↪️',
    order: 3321,
    group: 8,
  },
  {
    hexcode: '2934',
    shortcodes: ['arrow_heading_up'],
    emoji: '⤴️',
    order: 3323,
    group: 8,
  },
  {
    hexcode: '2935',
    shortcodes: ['arrow_heading_down'],
    emoji: '⤵️',
    order: 3325,
    group: 8,
  },
  {
    hexcode: '1F503',
    shortcodes: ['clockwise'],
    emoji: '🔃',
    order: 3326,
    group: 8,
  },
  {
    hexcode: '1F504',
    shortcodes: ['counter_clockwise'],
    emoji: '🔄',
    order: 3327,
    group: 8,
  },
  {
    hexcode: '1F519',
    shortcodes: ['back'],
    emoji: '🔙',
    order: 3328,
    group: 8,
  },
  {
    hexcode: '1F51A',
    shortcodes: ['end'],
    emoji: '🔚',
    order: 3329,
    group: 8,
  },
  {
    hexcode: '1F51B',
    shortcodes: ['on'],
    emoji: '🔛',
    order: 3330,
    group: 8,
  },
  {
    hexcode: '1F51C',
    shortcodes: ['soon'],
    emoji: '🔜',
    order: 3331,
    group: 8,
  },
  {
    hexcode: '1F51D',
    shortcodes: ['top'],
    emoji: '🔝',
    order: 3332,
    group: 8,
  },
  {
    hexcode: '1F6D0',
    shortcodes: ['place_of_worship'],
    emoji: '🛐',
    order: 3333,
    group: 8,
  },
  {
    hexcode: '269B',
    shortcodes: ['atom'],
    emoji: '⚛️',
    order: 3335,
    group: 8,
  },
  {
    hexcode: '1F549',
    shortcodes: ['om_symbol'],
    emoji: '🕉️',
    order: 3337,
    group: 8,
  },
  {
    hexcode: '2721',
    shortcodes: ['star_of_david'],
    emoji: '✡️',
    order: 3339,
    group: 8,
  },
  {
    hexcode: '2638',
    shortcodes: ['wheel_of_dharma'],
    emoji: '☸️',
    order: 3341,
    group: 8,
  },
  {
    hexcode: '262F',
    shortcodes: ['yin_yang'],
    emoji: '☯️',
    order: 3343,
    group: 8,
  },
  {
    hexcode: '271D',
    shortcodes: ['cross'],
    emoji: '✝️',
    order: 3345,
    group: 8,
  },
  {
    hexcode: '2626',
    shortcodes: ['orthodox_cross'],
    emoji: '☦️',
    order: 3347,
    group: 8,
  },
  {
    hexcode: '262A',
    shortcodes: ['star_and_crescent'],
    emoji: '☪️',
    order: 3349,
    group: 8,
  },
  {
    hexcode: '262E',
    shortcodes: ['peace'],
    emoji: '☮️',
    order: 3351,
    group: 8,
  },
  {
    hexcode: '1F54E',
    shortcodes: ['menorah'],
    emoji: '🕎',
    order: 3352,
    group: 8,
  },
  {
    hexcode: '1F52F',
    shortcodes: ['six_pointed_star'],
    emoji: '🔯',
    order: 3353,
    group: 8,
  },
  {
    hexcode: '2648',
    shortcodes: ['aries'],
    emoji: '♈️',
    order: 3354,
    group: 8,
  },
  {
    hexcode: '2649',
    shortcodes: ['taurus'],
    emoji: '♉️',
    order: 3355,
    group: 8,
  },
  {
    hexcode: '264A',
    shortcodes: ['gemini'],
    emoji: '♊️',
    order: 3356,
    group: 8,
  },
  {
    hexcode: '264B',
    shortcodes: ['cancer'],
    emoji: '♋️',
    order: 3357,
    group: 8,
  },
  {
    hexcode: '264C',
    shortcodes: ['leo'],
    emoji: '♌️',
    order: 3358,
    group: 8,
  },
  {
    hexcode: '264D',
    shortcodes: ['virgo'],
    emoji: '♍️',
    order: 3359,
    group: 8,
  },
  {
    hexcode: '264E',
    shortcodes: ['libra'],
    emoji: '♎️',
    order: 3360,
    group: 8,
  },
  {
    hexcode: '264F',
    shortcodes: ['scorpius'],
    emoji: '♏️',
    order: 3361,
    group: 8,
  },
  {
    hexcode: '2650',
    shortcodes: ['sagittarius'],
    emoji: '♐️',
    order: 3362,
    group: 8,
  },
  {
    hexcode: '2651',
    shortcodes: ['capricorn'],
    emoji: '♑️',
    order: 3363,
    group: 8,
  },
  {
    hexcode: '2652',
    shortcodes: ['aquarius'],
    emoji: '♒️',
    order: 3364,
    group: 8,
  },
  {
    hexcode: '2653',
    shortcodes: ['pisces'],
    emoji: '♓️',
    order: 3365,
    group: 8,
  },
  {
    hexcode: '26CE',
    shortcodes: ['ophiuchus'],
    emoji: '⛎',
    order: 3366,
    group: 8,
  },
  {
    hexcode: '1F500',
    shortcodes: ['shuffle'],
    emoji: '🔀',
    order: 3367,
    group: 8,
  },
  {
    hexcode: '1F501',
    shortcodes: ['repeat'],
    emoji: '🔁',
    order: 3368,
    group: 8,
  },
  {
    hexcode: '1F502',
    shortcodes: ['repeat_single'],
    emoji: '🔂',
    order: 3369,
    group: 8,
  },
  {
    hexcode: '25B6',
    shortcodes: ['play'],
    emoji: '▶️',
    order: 3371,
    group: 8,
  },
  {
    hexcode: '23E9',
    shortcodes: ['fast_forward'],
    emoji: '⏩️',
    order: 3372,
    group: 8,
  },
  {
    hexcode: '23ED',
    shortcodes: ['next_track'],
    emoji: '⏭️',
    order: 3374,
    group: 8,
  },
  {
    hexcode: '23EF',
    shortcodes: ['play_pause'],
    emoji: '⏯️',
    order: 3376,
    group: 8,
  },
  {
    hexcode: '25C0',
    shortcodes: ['reverse'],
    emoji: '◀️',
    order: 3378,
    group: 8,
  },
  {
    hexcode: '23EA',
    shortcodes: ['rewind'],
    emoji: '⏪️',
    order: 3379,
    group: 8,
  },
  {
    hexcode: '23EE',
    shortcodes: ['previous_track'],
    emoji: '⏮️',
    order: 3381,
    group: 8,
  },
  {
    hexcode: '1F53C',
    shortcodes: ['up_button'],
    emoji: '🔼',
    order: 3382,
    group: 8,
  },
  {
    hexcode: '23EB',
    shortcodes: ['fast_up_button'],
    emoji: '⏫',
    order: 3383,
    group: 8,
  },
  {
    hexcode: '1F53D',
    shortcodes: ['down_button'],
    emoji: '🔽',
    order: 3384,
    group: 8,
  },
  {
    hexcode: '23EC',
    shortcodes: ['fast_down_button'],
    emoji: '⏬',
    order: 3385,
    group: 8,
  },
  {
    hexcode: '23F8',
    shortcodes: ['pause'],
    emoji: '⏸️',
    order: 3387,
    group: 8,
  },
  {
    hexcode: '23F9',
    shortcodes: ['stop'],
    emoji: '⏹️',
    order: 3389,
    group: 8,
  },
  {
    hexcode: '23FA',
    shortcodes: ['record'],
    emoji: '⏺️',
    order: 3391,
    group: 8,
  },
  {
    hexcode: '23CF',
    shortcodes: ['eject'],
    emoji: '⏏️',
    order: 3393,
    group: 8,
  },
  {
    hexcode: '1F3A6',
    shortcodes: ['cinema'],
    emoji: '🎦',
    order: 3394,
    group: 8,
  },
  {
    hexcode: '1F505',
    shortcodes: ['dim', 'low_brightness'],
    emoji: '🔅',
    order: 3395,
    group: 8,
  },
  {
    hexcode: '1F506',
    shortcodes: ['bright', 'high_brightness'],
    emoji: '🔆',
    order: 3396,
    group: 8,
  },
  {
    hexcode: '1F4F6',
    shortcodes: ['signal_strength', 'antenna_bars'],
    emoji: '📶',
    order: 3397,
    group: 8,
  },
  {
    hexcode: '1F4F3',
    shortcodes: ['vibration_mode'],
    emoji: '📳',
    order: 3398,
    group: 8,
  },
  {
    hexcode: '1F4F4',
    shortcodes: ['mobile_phone_off'],
    emoji: '📴',
    order: 3399,
    group: 8,
  },
  {
    hexcode: '2640',
    shortcodes: ['female', 'female_sign'],
    emoji: '♀️',
    order: 3401,
    group: 8,
  },
  {
    hexcode: '2642',
    shortcodes: ['male', 'male_sign'],
    emoji: '♂️',
    order: 3403,
    group: 8,
  },
  {
    hexcode: '2695',
    shortcodes: ['medical'],
    emoji: '⚕️',
    order: 3405,
    group: 8,
  },
  {
    hexcode: '267E',
    shortcodes: ['infinity'],
    emoji: '♾️',
    order: 3407,
    group: 8,
  },
  {
    hexcode: '267B',
    shortcodes: ['recycle'],
    emoji: '♻️',
    order: 3409,
    group: 8,
  },
  {
    hexcode: '269C',
    shortcodes: ['fleur-de-lis'],
    emoji: '⚜️',
    order: 3411,
    group: 8,
  },
  {
    hexcode: '1F531',
    shortcodes: ['trident'],
    emoji: '🔱',
    order: 3412,
    group: 8,
  },
  {
    hexcode: '1F4DB',
    shortcodes: ['name_badge'],
    emoji: '📛',
    order: 3413,
    group: 8,
  },
  {
    hexcode: '1F530',
    shortcodes: ['ja_beginner'],
    emoji: '🔰',
    order: 3414,
    group: 8,
  },
  {
    hexcode: '2B55',
    shortcodes: ['o'],
    emoji: '⭕️',
    order: 3415,
    group: 8,
  },
  {
    hexcode: '2705',
    shortcodes: ['white_check_mark'],
    emoji: '✅',
    order: 3416,
    group: 8,
  },
  {
    hexcode: '2611',
    shortcodes: ['checked_ballot'],
    emoji: '☑️',
    order: 3418,
    group: 8,
  },
  {
    hexcode: '2714',
    shortcodes: ['check_mark'],
    emoji: '✔️',
    order: 3420,
    group: 8,
  },
  {
    hexcode: '2716',
    shortcodes: ['multiplication'],
    emoji: '✖️',
    order: 3422,
    group: 8,
  },
  {
    hexcode: '274C',
    shortcodes: ['x', 'cross_mark'],
    emoji: '❌',
    order: 3423,
    group: 8,
  },
  {
    hexcode: '274E',
    shortcodes: ['cross_mark_button'],
    emoji: '❎',
    order: 3424,
    group: 8,
  },
  {
    hexcode: '2795',
    shortcodes: ['plus'],
    emoji: '➕',
    order: 3425,
    group: 8,
  },
  {
    hexcode: '2796',
    shortcodes: ['minus'],
    emoji: '➖',
    order: 3426,
    group: 8,
  },
  {
    hexcode: '2797',
    shortcodes: ['division'],
    emoji: '➗',
    order: 3427,
    group: 8,
  },
  {
    hexcode: '27B0',
    shortcodes: ['curly_loop'],
    emoji: '➰',
    order: 3428,
    group: 8,
  },
  {
    hexcode: '27BF',
    shortcodes: ['double_curly_loop'],
    emoji: '➿',
    order: 3429,
    group: 8,
  },
  {
    hexcode: '303D',
    shortcodes: ['part_alternation_mark'],
    emoji: '〽️',
    order: 3431,
    group: 8,
  },
  {
    hexcode: '2733',
    shortcodes: ['eight_spoked_asterisk'],
    emoji: '✳️',
    order: 3433,
    group: 8,
  },
  {
    hexcode: '2734',
    shortcodes: ['eight_pointed_star'],
    emoji: '✴️',
    order: 3435,
    group: 8,
  },
  {
    hexcode: '2747',
    shortcodes: ['sparkle'],
    emoji: '❇️',
    order: 3437,
    group: 8,
  },
  {
    hexcode: '203C',
    shortcodes: ['bangbang', 'double_exclamation'],
    emoji: '‼️',
    order: 3439,
    group: 8,
  },
  {
    hexcode: '2049',
    shortcodes: ['interrobang', 'exclamation_question'],
    emoji: '⁉️',
    order: 3441,
    group: 8,
  },
  {
    hexcode: '2753',
    shortcodes: ['question'],
    emoji: '❓️',
    order: 3442,
    group: 8,
  },
  {
    hexcode: '2754',
    shortcodes: ['white_question'],
    emoji: '❔',
    order: 3443,
    group: 8,
  },
  {
    hexcode: '2755',
    shortcodes: ['white_exclamation'],
    emoji: '❕',
    order: 3444,
    group: 8,
  },
  {
    hexcode: '2757',
    shortcodes: ['exclamation'],
    emoji: '❗️',
    order: 3445,
    group: 8,
  },
  {
    hexcode: '3030',
    shortcodes: ['wavy_dash'],
    emoji: '〰️',
    order: 3447,
    group: 8,
  },
  {
    hexcode: '00A9',
    shortcodes: ['copyright'],
    emoji: '©️',
    order: 3449,
    group: 8,
  },
  {
    hexcode: '00AE',
    shortcodes: ['registered'],
    emoji: '®️',
    order: 3451,
    group: 8,
  },
  {
    hexcode: '2122',
    shortcodes: ['tm'],
    emoji: '™️',
    order: 3453,
    group: 8,
  },
  {
    hexcode: '0023-FE0F-20E3',
    shortcodes: ['hash'],
    emoji: '#️⃣',
    order: 3454,
    group: 8,
  },
  {
    hexcode: '002A-FE0F-20E3',
    shortcodes: ['asterisk'],
    emoji: '*️⃣',
    order: 3456,
    group: 8,
  },
  {
    hexcode: '0030-FE0F-20E3',
    shortcodes: ['zero'],
    emoji: '0️⃣',
    order: 3458,
    group: 8,
  },
  {
    hexcode: '0031-FE0F-20E3',
    shortcodes: ['one'],
    emoji: '1️⃣',
    order: 3460,
    group: 8,
  },
  {
    hexcode: '0032-FE0F-20E3',
    shortcodes: ['two'],
    emoji: '2️⃣',
    order: 3462,
    group: 8,
  },
  {
    hexcode: '0033-FE0F-20E3',
    shortcodes: ['three'],
    emoji: '3️⃣',
    order: 3464,
    group: 8,
  },
  {
    hexcode: '0034-FE0F-20E3',
    shortcodes: ['four'],
    emoji: '4️⃣',
    order: 3466,
    group: 8,
  },
  {
    hexcode: '0035-FE0F-20E3',
    shortcodes: ['five'],
    emoji: '5️⃣',
    order: 3468,
    group: 8,
  },
  {
    hexcode: '0036-FE0F-20E3',
    shortcodes: ['six'],
    emoji: '6️⃣',
    order: 3470,
    group: 8,
  },
  {
    hexcode: '0037-FE0F-20E3',
    shortcodes: ['seven'],
    emoji: '7️⃣',
    order: 3472,
    group: 8,
  },
  {
    hexcode: '0038-FE0F-20E3',
    shortcodes: ['eight'],
    emoji: '8️⃣',
    order: 3474,
    group: 8,
  },
  {
    hexcode: '0039-FE0F-20E3',
    shortcodes: ['nine'],
    emoji: '9️⃣',
    order: 3476,
    group: 8,
  },
  {
    hexcode: '1F51F',
    shortcodes: ['ten'],
    emoji: '🔟',
    order: 3478,
    group: 8,
  },
  {
    hexcode: '1F520',
    shortcodes: ['upper_abcd'],
    emoji: '🔠',
    order: 3479,
    group: 8,
  },
  {
    hexcode: '1F521',
    shortcodes: ['abcd'],
    emoji: '🔡',
    order: 3480,
    group: 8,
  },
  {
    hexcode: '1F522',
    shortcodes: ['1234'],
    emoji: '🔢',
    order: 3481,
    group: 8,
  },
  {
    hexcode: '1F523',
    shortcodes: ['symbols'],
    emoji: '🔣',
    order: 3482,
    group: 8,
  },
  {
    hexcode: '1F524',
    shortcodes: ['abc'],
    emoji: '🔤',
    order: 3483,
    group: 8,
  },
  {
    hexcode: '1F170',
    shortcodes: ['a_blood'],
    emoji: '🅰️',
    order: 3485,
    group: 8,
  },
  {
    hexcode: '1F18E',
    shortcodes: ['ab_blood'],
    emoji: '🆎',
    order: 3486,
    group: 8,
  },
  {
    hexcode: '1F171',
    shortcodes: ['b_blood'],
    emoji: '🅱️',
    order: 3488,
    group: 8,
  },
  {
    hexcode: '1F191',
    shortcodes: ['cl'],
    emoji: '🆑',
    order: 3489,
    group: 8,
  },
  {
    hexcode: '1F192',
    shortcodes: ['cool'],
    emoji: '🆒',
    order: 3490,
    group: 8,
  },
  {
    hexcode: '1F193',
    shortcodes: ['free'],
    emoji: '🆓',
    order: 3491,
    group: 8,
  },
  {
    hexcode: '2139',
    shortcodes: ['info'],
    emoji: 'ℹ️',
    order: 3493,
    group: 8,
  },
  {
    hexcode: '1F194',
    shortcodes: ['id'],
    emoji: '🆔',
    order: 3494,
    group: 8,
  },
  {
    hexcode: '24C2',
    shortcodes: ['m'],
    emoji: 'Ⓜ️',
    order: 3496,
    group: 8,
  },
  {
    hexcode: '1F195',
    shortcodes: ['new'],
    emoji: '🆕',
    order: 3497,
    group: 8,
  },
  {
    hexcode: '1F196',
    shortcodes: ['ng'],
    emoji: '🆖',
    order: 3498,
    group: 8,
  },
  {
    hexcode: '1F17E',
    shortcodes: ['o_blood'],
    emoji: '🅾️',
    order: 3500,
    group: 8,
  },
  {
    hexcode: '1F197',
    shortcodes: ['ok'],
    emoji: '🆗',
    order: 3501,
    group: 8,
  },
  {
    hexcode: '1F17F',
    shortcodes: ['p'],
    emoji: '🅿️',
    order: 3503,
    group: 8,
  },
  {
    hexcode: '1F198',
    shortcodes: ['sos'],
    emoji: '🆘',
    order: 3504,
    group: 8,
  },
  {
    hexcode: '1F199',
    shortcodes: ['up'],
    emoji: '🆙',
    order: 3505,
    group: 8,
  },
  {
    hexcode: '1F19A',
    shortcodes: ['vs'],
    emoji: '🆚',
    order: 3506,
    group: 8,
  },
  {
    hexcode: '1F201',
    shortcodes: ['ja_here', 'koko'],
    emoji: '🈁',
    order: 3507,
    group: 8,
  },
  {
    hexcode: '1F202',
    shortcodes: ['ja_service_charge'],
    emoji: '🈂️',
    order: 3509,
    group: 8,
  },
  {
    hexcode: '1F237',
    shortcodes: ['ja_monthly_amount'],
    emoji: '🈷️',
    order: 3511,
    group: 8,
  },
  {
    hexcode: '1F236',
    shortcodes: ['ja_not_free_of_carge'],
    emoji: '🈶',
    order: 3512,
    group: 8,
  },
  {
    hexcode: '1F22F',
    shortcodes: ['ja_reserved'],
    emoji: '🈯️',
    order: 3513,
    group: 8,
  },
  {
    hexcode: '1F250',
    shortcodes: ['ja_bargain'],
    emoji: '🉐',
    order: 3514,
    group: 8,
  },
  {
    hexcode: '1F239',
    shortcodes: ['ja_discount'],
    emoji: '🈹',
    order: 3515,
    group: 8,
  },
  {
    hexcode: '1F21A',
    shortcodes: ['ja_free_of_charge'],
    emoji: '🈚️',
    order: 3516,
    group: 8,
  },
  {
    hexcode: '1F232',
    shortcodes: ['ja_prohibited'],
    emoji: '🈲',
    order: 3517,
    group: 8,
  },
  {
    hexcode: '1F251',
    shortcodes: ['ja_acceptable'],
    emoji: '🉑',
    order: 3518,
    group: 8,
  },
  {
    hexcode: '1F238',
    shortcodes: ['ja_application'],
    emoji: '🈸',
    order: 3519,
    group: 8,
  },
  {
    hexcode: '1F234',
    shortcodes: ['ja_passing_grade'],
    emoji: '🈴',
    order: 3520,
    group: 8,
  },
  {
    hexcode: '1F233',
    shortcodes: ['ja_vacancy'],
    emoji: '🈳',
    order: 3521,
    group: 8,
  },
  {
    hexcode: '3297',
    shortcodes: ['ja_congratulations'],
    emoji: '㊗️',
    order: 3523,
    group: 8,
  },
  {
    hexcode: '3299',
    shortcodes: ['ja_secret'],
    emoji: '㊙️',
    order: 3525,
    group: 8,
  },
  {
    hexcode: '1F23A',
    shortcodes: ['ja_open_for_business'],
    emoji: '🈺',
    order: 3526,
    group: 8,
  },
  {
    hexcode: '1F235',
    shortcodes: ['ja_no_vacancy'],
    emoji: '🈵',
    order: 3527,
    group: 8,
  },
  {
    hexcode: '1F534',
    shortcodes: ['red_circle'],
    emoji: '🔴',
    order: 3528,
    group: 8,
  },
  {
    hexcode: '1F7E0',
    shortcodes: ['orange_circle'],
    emoji: '🟠',
    order: 3529,
    group: 8,
  },
  {
    hexcode: '1F7E1',
    shortcodes: ['yellow_circle'],
    emoji: '🟡',
    order: 3530,
    group: 8,
  },
  {
    hexcode: '1F7E2',
    shortcodes: ['green_circle'],
    emoji: '🟢',
    order: 3531,
    group: 8,
  },
  {
    hexcode: '1F535',
    shortcodes: ['blue_circle'],
    emoji: '🔵',
    order: 3532,
    group: 8,
  },
  {
    hexcode: '1F7E3',
    shortcodes: ['purple_circle'],
    emoji: '🟣',
    order: 3533,
    group: 8,
  },
  {
    hexcode: '1F7E4',
    shortcodes: ['brown_circle'],
    emoji: '🟤',
    order: 3534,
    group: 8,
  },
  {
    hexcode: '26AB',
    shortcodes: ['black_circle'],
    emoji: '⚫️',
    order: 3535,
    group: 8,
  },
  {
    hexcode: '26AA',
    shortcodes: ['white_circle'],
    emoji: '⚪️',
    order: 3536,
    group: 8,
  },
  {
    hexcode: '1F7E5',
    shortcodes: ['red_square'],
    emoji: '🟥',
    order: 3537,
    group: 8,
  },
  {
    hexcode: '1F7E7',
    shortcodes: ['orange_square'],
    emoji: '🟧',
    order: 3538,
    group: 8,
  },
  {
    hexcode: '1F7E8',
    shortcodes: ['yellow_square'],
    emoji: '🟨',
    order: 3539,
    group: 8,
  },
  {
    hexcode: '1F7E9',
    shortcodes: ['green_square'],
    emoji: '🟩',
    order: 3540,
    group: 8,
  },
  {
    hexcode: '1F7E6',
    shortcodes: ['blue_square'],
    emoji: '🟦',
    order: 3541,
    group: 8,
  },
  {
    hexcode: '1F7EA',
    shortcodes: ['purple_square'],
    emoji: '🟪',
    order: 3542,
    group: 8,
  },
  {
    hexcode: '1F7EB',
    shortcodes: ['brown_square'],
    emoji: '🟫',
    order: 3543,
    group: 8,
  },
  {
    hexcode: '2B1B',
    shortcodes: ['large_black_square'],
    emoji: '⬛️',
    order: 3544,
    group: 8,
  },
  {
    hexcode: '2B1C',
    shortcodes: ['large_white_square'],
    emoji: '⬜️',
    order: 3545,
    group: 8,
  },
  {
    hexcode: '25FC',
    shortcodes: ['medium_black_square'],
    emoji: '◼️',
    order: 3547,
    group: 8,
  },
  {
    hexcode: '25FB',
    shortcodes: ['medium_white_square'],
    emoji: '◻️',
    order: 3549,
    group: 8,
  },
  {
    hexcode: '25FE',
    shortcodes: ['medium_small_black_square'],
    emoji: '◾️',
    order: 3550,
    group: 8,
  },
  {
    hexcode: '25FD',
    shortcodes: ['medium_small_white_square'],
    emoji: '◽️',
    order: 3551,
    group: 8,
  },
  {
    hexcode: '25AA',
    shortcodes: ['small_black_square'],
    emoji: '▪️',
    order: 3553,
    group: 8,
  },
  {
    hexcode: '25AB',
    shortcodes: ['small_white_square'],
    emoji: '▫️',
    order: 3555,
    group: 8,
  },
  {
    hexcode: '1F536',
    shortcodes: ['large_orange_diamond'],
    emoji: '🔶',
    order: 3556,
    group: 8,
  },
  {
    hexcode: '1F537',
    shortcodes: ['large_blue_diamond'],
    emoji: '🔷',
    order: 3557,
    group: 8,
  },
  {
    hexcode: '1F538',
    shortcodes: ['small_orange_diamond'],
    emoji: '🔸',
    order: 3558,
    group: 8,
  },
  {
    hexcode: '1F539',
    shortcodes: ['small_blue_diamond'],
    emoji: '🔹',
    order: 3559,
    group: 8,
  },
  {
    hexcode: '1F53A',
    shortcodes: ['up_red_triangle'],
    emoji: '🔺',
    order: 3560,
    group: 8,
  },
  {
    hexcode: '1F53B',
    shortcodes: ['down_red_triangle'],
    emoji: '🔻',
    order: 3561,
    group: 8,
  },
  {
    hexcode: '1F4A0',
    shortcodes: ['diamond_dot'],
    emoji: '💠',
    order: 3562,
    group: 8,
  },
  {
    hexcode: '1F518',
    shortcodes: ['radio_button'],
    emoji: '🔘',
    order: 3563,
    group: 8,
  },
  {
    hexcode: '1F533',
    shortcodes: ['white_square_button'],
    emoji: '🔳',
    order: 3564,
    group: 8,
  },
  {
    hexcode: '1F532',
    shortcodes: ['black_square_button'],
    emoji: '🔲',
    order: 3565,
    group: 8,
  },
  {
    hexcode: '1F3C1',
    shortcodes: ['checkered_flag'],
    emoji: '🏁',
    order: 3566,
    group: 9,
  },
  {
    hexcode: '1F6A9',
    shortcodes: ['triangle_flag'],
    emoji: '🚩',
    order: 3567,
    group: 9,
  },
  {
    hexcode: '1F38C',
    shortcodes: ['crossed_flags'],
    emoji: '🎌',
    order: 3568,
    group: 9,
  },
  {
    hexcode: '1F3F4',
    shortcodes: ['black_flag'],
    emoji: '🏴',
    order: 3569,
    group: 9,
  },
  {
    hexcode: '1F3F3',
    shortcodes: ['white_flag'],
    emoji: '🏳️',
    order: 3571,
    group: 9,
  },
  {
    hexcode: '1F3F3-FE0F-200D-1F308',
    shortcodes: ['rainbow_flag'],
    emoji: '🏳️‍🌈',
    order: 3572,
    group: 9,
  },
  {
    hexcode: '1F3F4-200D-2620-FE0F',
    shortcodes: ['pirate_flag', 'jolly_roger'],
    emoji: '🏴‍☠️',
    order: 3574,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1E8',
    shortcodes: ['flag_ac'],
    emoji: '🇦🇨',
    order: 3576,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1E9',
    shortcodes: ['flag_ad'],
    emoji: '🇦🇩',
    order: 3577,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1EA',
    shortcodes: ['flag_ae'],
    emoji: '🇦🇪',
    order: 3578,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1EB',
    shortcodes: ['flag_af'],
    emoji: '🇦🇫',
    order: 3579,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1EC',
    shortcodes: ['flag_ag'],
    emoji: '🇦🇬',
    order: 3580,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1EE',
    shortcodes: ['flag_ai'],
    emoji: '🇦🇮',
    order: 3581,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F1',
    shortcodes: ['flag_al'],
    emoji: '🇦🇱',
    order: 3582,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F2',
    shortcodes: ['flag_am'],
    emoji: '🇦🇲',
    order: 3583,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F4',
    shortcodes: ['flag_ao'],
    emoji: '🇦🇴',
    order: 3584,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F6',
    shortcodes: ['flag_aq'],
    emoji: '🇦🇶',
    order: 3585,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F7',
    shortcodes: ['flag_ar'],
    emoji: '🇦🇷',
    order: 3586,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F8',
    shortcodes: ['flag_as'],
    emoji: '🇦🇸',
    order: 3587,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1F9',
    shortcodes: ['flag_at'],
    emoji: '🇦🇹',
    order: 3588,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1FA',
    shortcodes: ['flag_au'],
    emoji: '🇦🇺',
    order: 3589,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1FC',
    shortcodes: ['flag_aw'],
    emoji: '🇦🇼',
    order: 3590,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1FD',
    shortcodes: ['flag_ax'],
    emoji: '🇦🇽',
    order: 3591,
    group: 9,
  },
  {
    hexcode: '1F1E6-1F1FF',
    shortcodes: ['flag_az'],
    emoji: '🇦🇿',
    order: 3592,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1E6',
    shortcodes: ['flag_ba'],
    emoji: '🇧🇦',
    order: 3593,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1E7',
    shortcodes: ['flag_bb'],
    emoji: '🇧🇧',
    order: 3594,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1E9',
    shortcodes: ['flag_bd'],
    emoji: '🇧🇩',
    order: 3595,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1EA',
    shortcodes: ['flag_be'],
    emoji: '🇧🇪',
    order: 3596,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1EB',
    shortcodes: ['flag_bf'],
    emoji: '🇧🇫',
    order: 3597,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1EC',
    shortcodes: ['flag_bg'],
    emoji: '🇧🇬',
    order: 3598,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1ED',
    shortcodes: ['flag_bh'],
    emoji: '🇧🇭',
    order: 3599,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1EE',
    shortcodes: ['flag_bi'],
    emoji: '🇧🇮',
    order: 3600,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1EF',
    shortcodes: ['flag_bj'],
    emoji: '🇧🇯',
    order: 3601,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F1',
    shortcodes: ['flag_bl'],
    emoji: '🇧🇱',
    order: 3602,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F2',
    shortcodes: ['flag_bm'],
    emoji: '🇧🇲',
    order: 3603,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F3',
    shortcodes: ['flag_bn'],
    emoji: '🇧🇳',
    order: 3604,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F4',
    shortcodes: ['flag_bo'],
    emoji: '🇧🇴',
    order: 3605,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F6',
    shortcodes: ['flag_bq'],
    emoji: '🇧🇶',
    order: 3606,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F7',
    shortcodes: ['flag_br'],
    emoji: '🇧🇷',
    order: 3607,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F8',
    shortcodes: ['flag_bs'],
    emoji: '🇧🇸',
    order: 3608,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1F9',
    shortcodes: ['flag_bt'],
    emoji: '🇧🇹',
    order: 3609,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1FB',
    shortcodes: ['flag_bv'],
    emoji: '🇧🇻',
    order: 3610,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1FC',
    shortcodes: ['flag_bw'],
    emoji: '🇧🇼',
    order: 3611,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1FE',
    shortcodes: ['flag_by'],
    emoji: '🇧🇾',
    order: 3612,
    group: 9,
  },
  {
    hexcode: '1F1E7-1F1FF',
    shortcodes: ['flag_bz'],
    emoji: '🇧🇿',
    order: 3613,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1E6',
    shortcodes: ['flag_ca'],
    emoji: '🇨🇦',
    order: 3614,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1E8',
    shortcodes: ['flag_cc'],
    emoji: '🇨🇨',
    order: 3615,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1E9',
    shortcodes: ['flag_cd'],
    emoji: '🇨🇩',
    order: 3616,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1EB',
    shortcodes: ['flag_cf'],
    emoji: '🇨🇫',
    order: 3617,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1EC',
    shortcodes: ['flag_cg'],
    emoji: '🇨🇬',
    order: 3618,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1ED',
    shortcodes: ['flag_ch'],
    emoji: '🇨🇭',
    order: 3619,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1EE',
    shortcodes: ['flag_ci'],
    emoji: '🇨🇮',
    order: 3620,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F0',
    shortcodes: ['flag_ck'],
    emoji: '🇨🇰',
    order: 3621,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F1',
    shortcodes: ['flag_cl'],
    emoji: '🇨🇱',
    order: 3622,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F2',
    shortcodes: ['flag_cm'],
    emoji: '🇨🇲',
    order: 3623,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F3',
    shortcodes: ['flag_cn'],
    emoji: '🇨🇳',
    order: 3624,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F4',
    shortcodes: ['flag_co'],
    emoji: '🇨🇴',
    order: 3625,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F5',
    shortcodes: ['flag_cp'],
    emoji: '🇨🇵',
    order: 3626,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1F7',
    shortcodes: ['flag_cr'],
    emoji: '🇨🇷',
    order: 3627,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FA',
    shortcodes: ['flag_cu'],
    emoji: '🇨🇺',
    order: 3628,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FB',
    shortcodes: ['flag_cv'],
    emoji: '🇨🇻',
    order: 3629,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FC',
    shortcodes: ['flag_cw'],
    emoji: '🇨🇼',
    order: 3630,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FD',
    shortcodes: ['flag_cx'],
    emoji: '🇨🇽',
    order: 3631,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FE',
    shortcodes: ['flag_cy'],
    emoji: '🇨🇾',
    order: 3632,
    group: 9,
  },
  {
    hexcode: '1F1E8-1F1FF',
    shortcodes: ['flag_cz'],
    emoji: '🇨🇿',
    order: 3633,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1EA',
    shortcodes: ['flag_de'],
    emoji: '🇩🇪',
    order: 3634,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1EC',
    shortcodes: ['flag_dg'],
    emoji: '🇩🇬',
    order: 3635,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1EF',
    shortcodes: ['flag_dj'],
    emoji: '🇩🇯',
    order: 3636,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1F0',
    shortcodes: ['flag_dk'],
    emoji: '🇩🇰',
    order: 3637,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1F2',
    shortcodes: ['flag_dm'],
    emoji: '🇩🇲',
    order: 3638,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1F4',
    shortcodes: ['flag_do'],
    emoji: '🇩🇴',
    order: 3639,
    group: 9,
  },
  {
    hexcode: '1F1E9-1F1FF',
    shortcodes: ['flag_dz'],
    emoji: '🇩🇿',
    order: 3640,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1E6',
    shortcodes: ['flag_ea'],
    emoji: '🇪🇦',
    order: 3641,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1E8',
    shortcodes: ['flag_ec'],
    emoji: '🇪🇨',
    order: 3642,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1EA',
    shortcodes: ['flag_ee'],
    emoji: '🇪🇪',
    order: 3643,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1EC',
    shortcodes: ['flag_eg'],
    emoji: '🇪🇬',
    order: 3644,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1ED',
    shortcodes: ['flag_eh'],
    emoji: '🇪🇭',
    order: 3645,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1F7',
    shortcodes: ['flag_er'],
    emoji: '🇪🇷',
    order: 3646,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1F8',
    shortcodes: ['flag_es'],
    emoji: '🇪🇸',
    order: 3647,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1F9',
    shortcodes: ['flag_et'],
    emoji: '🇪🇹',
    order: 3648,
    group: 9,
  },
  {
    hexcode: '1F1EA-1F1FA',
    shortcodes: ['flag_eu'],
    emoji: '🇪🇺',
    order: 3649,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1EE',
    shortcodes: ['flag_fi'],
    emoji: '🇫🇮',
    order: 3650,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1EF',
    shortcodes: ['flag_fj'],
    emoji: '🇫🇯',
    order: 3651,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1F0',
    shortcodes: ['flag_fk'],
    emoji: '🇫🇰',
    order: 3652,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1F2',
    shortcodes: ['flag_fm'],
    emoji: '🇫🇲',
    order: 3653,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1F4',
    shortcodes: ['flag_fo'],
    emoji: '🇫🇴',
    order: 3654,
    group: 9,
  },
  {
    hexcode: '1F1EB-1F1F7',
    shortcodes: ['flag_fr'],
    emoji: '🇫🇷',
    order: 3655,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1E6',
    shortcodes: ['flag_ga'],
    emoji: '🇬🇦',
    order: 3656,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1E7',
    shortcodes: ['flag_gb'],
    emoji: '🇬🇧',
    order: 3657,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1E9',
    shortcodes: ['flag_gd'],
    emoji: '🇬🇩',
    order: 3658,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1EA',
    shortcodes: ['flag_ge'],
    emoji: '🇬🇪',
    order: 3659,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1EB',
    shortcodes: ['flag_gf'],
    emoji: '🇬🇫',
    order: 3660,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1EC',
    shortcodes: ['flag_gg'],
    emoji: '🇬🇬',
    order: 3661,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1ED',
    shortcodes: ['flag_gh'],
    emoji: '🇬🇭',
    order: 3662,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1EE',
    shortcodes: ['flag_gi'],
    emoji: '🇬🇮',
    order: 3663,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F1',
    shortcodes: ['flag_gl'],
    emoji: '🇬🇱',
    order: 3664,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F2',
    shortcodes: ['flag_gm'],
    emoji: '🇬🇲',
    order: 3665,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F3',
    shortcodes: ['flag_gn'],
    emoji: '🇬🇳',
    order: 3666,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F5',
    shortcodes: ['flag_gp'],
    emoji: '🇬🇵',
    order: 3667,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F6',
    shortcodes: ['flag_gq'],
    emoji: '🇬🇶',
    order: 3668,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F7',
    shortcodes: ['flag_gr'],
    emoji: '🇬🇷',
    order: 3669,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F8',
    shortcodes: ['flag_gs'],
    emoji: '🇬🇸',
    order: 3670,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1F9',
    shortcodes: ['flag_gt'],
    emoji: '🇬🇹',
    order: 3671,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1FA',
    shortcodes: ['flag_gu'],
    emoji: '🇬🇺',
    order: 3672,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1FC',
    shortcodes: ['flag_gw'],
    emoji: '🇬🇼',
    order: 3673,
    group: 9,
  },
  {
    hexcode: '1F1EC-1F1FE',
    shortcodes: ['flag_gy'],
    emoji: '🇬🇾',
    order: 3674,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1F0',
    shortcodes: ['flag_hk'],
    emoji: '🇭🇰',
    order: 3675,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1F2',
    shortcodes: ['flag_hm'],
    emoji: '🇭🇲',
    order: 3676,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1F3',
    shortcodes: ['flag_hn'],
    emoji: '🇭🇳',
    order: 3677,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1F7',
    shortcodes: ['flag_hr'],
    emoji: '🇭🇷',
    order: 3678,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1F9',
    shortcodes: ['flag_ht'],
    emoji: '🇭🇹',
    order: 3679,
    group: 9,
  },
  {
    hexcode: '1F1ED-1F1FA',
    shortcodes: ['flag_hu'],
    emoji: '🇭🇺',
    order: 3680,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1E8',
    shortcodes: ['flag_ic'],
    emoji: '🇮🇨',
    order: 3681,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1E9',
    shortcodes: ['flag_id'],
    emoji: '🇮🇩',
    order: 3682,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1EA',
    shortcodes: ['flag_ie'],
    emoji: '🇮🇪',
    order: 3683,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F1',
    shortcodes: ['flag_il'],
    emoji: '🇮🇱',
    order: 3684,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F2',
    shortcodes: ['flag_im'],
    emoji: '🇮🇲',
    order: 3685,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F3',
    shortcodes: ['flag_in'],
    emoji: '🇮🇳',
    order: 3686,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F4',
    shortcodes: ['flag_io'],
    emoji: '🇮🇴',
    order: 3687,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F6',
    shortcodes: ['flag_iq'],
    emoji: '🇮🇶',
    order: 3688,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F7',
    shortcodes: ['flag_ir'],
    emoji: '🇮🇷',
    order: 3689,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F8',
    shortcodes: ['flag_is'],
    emoji: '🇮🇸',
    order: 3690,
    group: 9,
  },
  {
    hexcode: '1F1EE-1F1F9',
    shortcodes: ['flag_it'],
    emoji: '🇮🇹',
    order: 3691,
    group: 9,
  },
  {
    hexcode: '1F1EF-1F1EA',
    shortcodes: ['flag_je'],
    emoji: '🇯🇪',
    order: 3692,
    group: 9,
  },
  {
    hexcode: '1F1EF-1F1F2',
    shortcodes: ['flag_jm'],
    emoji: '🇯🇲',
    order: 3693,
    group: 9,
  },
  {
    hexcode: '1F1EF-1F1F4',
    shortcodes: ['flag_jo'],
    emoji: '🇯🇴',
    order: 3694,
    group: 9,
  },
  {
    hexcode: '1F1EF-1F1F5',
    shortcodes: ['flag_jp'],
    emoji: '🇯🇵',
    order: 3695,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1EA',
    shortcodes: ['flag_ke'],
    emoji: '🇰🇪',
    order: 3696,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1EC',
    shortcodes: ['flag_kg'],
    emoji: '🇰🇬',
    order: 3697,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1ED',
    shortcodes: ['flag_kh'],
    emoji: '🇰🇭',
    order: 3698,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1EE',
    shortcodes: ['flag_ki'],
    emoji: '🇰🇮',
    order: 3699,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1F2',
    shortcodes: ['flag_km'],
    emoji: '🇰🇲',
    order: 3700,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1F3',
    shortcodes: ['flag_kn'],
    emoji: '🇰🇳',
    order: 3701,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1F5',
    shortcodes: ['flag_kp'],
    emoji: '🇰🇵',
    order: 3702,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1F7',
    shortcodes: ['flag_kr'],
    emoji: '🇰🇷',
    order: 3703,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1FC',
    shortcodes: ['flag_kw'],
    emoji: '🇰🇼',
    order: 3704,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1FE',
    shortcodes: ['flag_ky'],
    emoji: '🇰🇾',
    order: 3705,
    group: 9,
  },
  {
    hexcode: '1F1F0-1F1FF',
    shortcodes: ['flag_kz'],
    emoji: '🇰🇿',
    order: 3706,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1E6',
    shortcodes: ['flag_la'],
    emoji: '🇱🇦',
    order: 3707,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1E7',
    shortcodes: ['flag_lb'],
    emoji: '🇱🇧',
    order: 3708,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1E8',
    shortcodes: ['flag_lc'],
    emoji: '🇱🇨',
    order: 3709,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1EE',
    shortcodes: ['flag_li'],
    emoji: '🇱🇮',
    order: 3710,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1F0',
    shortcodes: ['flag_lk'],
    emoji: '🇱🇰',
    order: 3711,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1F7',
    shortcodes: ['flag_lr'],
    emoji: '🇱🇷',
    order: 3712,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1F8',
    shortcodes: ['flag_ls'],
    emoji: '🇱🇸',
    order: 3713,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1F9',
    shortcodes: ['flag_lt'],
    emoji: '🇱🇹',
    order: 3714,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1FA',
    shortcodes: ['flag_lu'],
    emoji: '🇱🇺',
    order: 3715,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1FB',
    shortcodes: ['flag_lv'],
    emoji: '🇱🇻',
    order: 3716,
    group: 9,
  },
  {
    hexcode: '1F1F1-1F1FE',
    shortcodes: ['flag_ly'],
    emoji: '🇱🇾',
    order: 3717,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1E6',
    shortcodes: ['flag_ma'],
    emoji: '🇲🇦',
    order: 3718,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1E8',
    shortcodes: ['flag_mc'],
    emoji: '🇲🇨',
    order: 3719,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1E9',
    shortcodes: ['flag_md'],
    emoji: '🇲🇩',
    order: 3720,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1EA',
    shortcodes: ['flag_me'],
    emoji: '🇲🇪',
    order: 3721,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1EB',
    shortcodes: ['flag_mf'],
    emoji: '🇲🇫',
    order: 3722,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1EC',
    shortcodes: ['flag_mg'],
    emoji: '🇲🇬',
    order: 3723,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1ED',
    shortcodes: ['flag_mh'],
    emoji: '🇲🇭',
    order: 3724,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F0',
    shortcodes: ['flag_mk'],
    emoji: '🇲🇰',
    order: 3725,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F1',
    shortcodes: ['flag_ml'],
    emoji: '🇲🇱',
    order: 3726,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F2',
    shortcodes: ['flag_mm'],
    emoji: '🇲🇲',
    order: 3727,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F3',
    shortcodes: ['flag_mn'],
    emoji: '🇲🇳',
    order: 3728,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F4',
    shortcodes: ['flag_mo'],
    emoji: '🇲🇴',
    order: 3729,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F5',
    shortcodes: ['flag_mp'],
    emoji: '🇲🇵',
    order: 3730,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F6',
    shortcodes: ['flag_mq'],
    emoji: '🇲🇶',
    order: 3731,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F7',
    shortcodes: ['flag_mr'],
    emoji: '🇲🇷',
    order: 3732,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F8',
    shortcodes: ['flag_ms'],
    emoji: '🇲🇸',
    order: 3733,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1F9',
    shortcodes: ['flag_mt'],
    emoji: '🇲🇹',
    order: 3734,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FA',
    shortcodes: ['flag_mu'],
    emoji: '🇲🇺',
    order: 3735,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FB',
    shortcodes: ['flag_mv'],
    emoji: '🇲🇻',
    order: 3736,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FC',
    shortcodes: ['flag_mw'],
    emoji: '🇲🇼',
    order: 3737,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FD',
    shortcodes: ['flag_mx'],
    emoji: '🇲🇽',
    order: 3738,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FE',
    shortcodes: ['flag_my'],
    emoji: '🇲🇾',
    order: 3739,
    group: 9,
  },
  {
    hexcode: '1F1F2-1F1FF',
    shortcodes: ['flag_mz'],
    emoji: '🇲🇿',
    order: 3740,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1E6',
    shortcodes: ['flag_na'],
    emoji: '🇳🇦',
    order: 3741,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1E8',
    shortcodes: ['flag_nc'],
    emoji: '🇳🇨',
    order: 3742,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1EA',
    shortcodes: ['flag_ne'],
    emoji: '🇳🇪',
    order: 3743,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1EB',
    shortcodes: ['flag_nf'],
    emoji: '🇳🇫',
    order: 3744,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1EC',
    shortcodes: ['flag_ng'],
    emoji: '🇳🇬',
    order: 3745,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1EE',
    shortcodes: ['flag_ni'],
    emoji: '🇳🇮',
    order: 3746,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1F1',
    shortcodes: ['flag_nl'],
    emoji: '🇳🇱',
    order: 3747,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1F4',
    shortcodes: ['flag_no'],
    emoji: '🇳🇴',
    order: 3748,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1F5',
    shortcodes: ['flag_np'],
    emoji: '🇳🇵',
    order: 3749,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1F7',
    shortcodes: ['flag_nr'],
    emoji: '🇳🇷',
    order: 3750,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1FA',
    shortcodes: ['flag_nu'],
    emoji: '🇳🇺',
    order: 3751,
    group: 9,
  },
  {
    hexcode: '1F1F3-1F1FF',
    shortcodes: ['flag_nz'],
    emoji: '🇳🇿',
    order: 3752,
    group: 9,
  },
  {
    hexcode: '1F1F4-1F1F2',
    shortcodes: ['flag_om'],
    emoji: '🇴🇲',
    order: 3753,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1E6',
    shortcodes: ['flag_pa'],
    emoji: '🇵🇦',
    order: 3754,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1EA',
    shortcodes: ['flag_pe'],
    emoji: '🇵🇪',
    order: 3755,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1EB',
    shortcodes: ['flag_pf'],
    emoji: '🇵🇫',
    order: 3756,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1EC',
    shortcodes: ['flag_pg'],
    emoji: '🇵🇬',
    order: 3757,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1ED',
    shortcodes: ['flag_ph'],
    emoji: '🇵🇭',
    order: 3758,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F0',
    shortcodes: ['flag_pk'],
    emoji: '🇵🇰',
    order: 3759,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F1',
    shortcodes: ['flag_pl'],
    emoji: '🇵🇱',
    order: 3760,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F2',
    shortcodes: ['flag_pm'],
    emoji: '🇵🇲',
    order: 3761,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F3',
    shortcodes: ['flag_pn'],
    emoji: '🇵🇳',
    order: 3762,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F7',
    shortcodes: ['flag_pr'],
    emoji: '🇵🇷',
    order: 3763,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F8',
    shortcodes: ['flag_ps'],
    emoji: '🇵🇸',
    order: 3764,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1F9',
    shortcodes: ['flag_pt'],
    emoji: '🇵🇹',
    order: 3765,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1FC',
    shortcodes: ['flag_pw'],
    emoji: '🇵🇼',
    order: 3766,
    group: 9,
  },
  {
    hexcode: '1F1F5-1F1FE',
    shortcodes: ['flag_py'],
    emoji: '🇵🇾',
    order: 3767,
    group: 9,
  },
  {
    hexcode: '1F1F6-1F1E6',
    shortcodes: ['flag_qa'],
    emoji: '🇶🇦',
    order: 3768,
    group: 9,
  },
  {
    hexcode: '1F1F7-1F1EA',
    shortcodes: ['flag_re'],
    emoji: '🇷🇪',
    order: 3769,
    group: 9,
  },
  {
    hexcode: '1F1F7-1F1F4',
    shortcodes: ['flag_ro'],
    emoji: '🇷🇴',
    order: 3770,
    group: 9,
  },
  {
    hexcode: '1F1F7-1F1F8',
    shortcodes: ['flag_rs'],
    emoji: '🇷🇸',
    order: 3771,
    group: 9,
  },
  {
    hexcode: '1F1F7-1F1FA',
    shortcodes: ['flag_ru'],
    emoji: '🇷🇺',
    order: 3772,
    group: 9,
  },
  {
    hexcode: '1F1F7-1F1FC',
    shortcodes: ['flag_rw'],
    emoji: '🇷🇼',
    order: 3773,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1E6',
    shortcodes: ['flag_sa'],
    emoji: '🇸🇦',
    order: 3774,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1E7',
    shortcodes: ['flag_sb'],
    emoji: '🇸🇧',
    order: 3775,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1E8',
    shortcodes: ['flag_sc'],
    emoji: '🇸🇨',
    order: 3776,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1E9',
    shortcodes: ['flag_sd'],
    emoji: '🇸🇩',
    order: 3777,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1EA',
    shortcodes: ['flag_se'],
    emoji: '🇸🇪',
    order: 3778,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1EC',
    shortcodes: ['flag_sg'],
    emoji: '🇸🇬',
    order: 3779,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1ED',
    shortcodes: ['flag_sh'],
    emoji: '🇸🇭',
    order: 3780,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1EE',
    shortcodes: ['flag_si'],
    emoji: '🇸🇮',
    order: 3781,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1EF',
    shortcodes: ['flag_sj'],
    emoji: '🇸🇯',
    order: 3782,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F0',
    shortcodes: ['flag_sk'],
    emoji: '🇸🇰',
    order: 3783,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F1',
    shortcodes: ['flag_sl'],
    emoji: '🇸🇱',
    order: 3784,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F2',
    shortcodes: ['flag_sm'],
    emoji: '🇸🇲',
    order: 3785,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F3',
    shortcodes: ['flag_sn'],
    emoji: '🇸🇳',
    order: 3786,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F4',
    shortcodes: ['flag_so'],
    emoji: '🇸🇴',
    order: 3787,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F7',
    shortcodes: ['flag_sr'],
    emoji: '🇸🇷',
    order: 3788,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F8',
    shortcodes: ['flag_ss'],
    emoji: '🇸🇸',
    order: 3789,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1F9',
    shortcodes: ['flag_st'],
    emoji: '🇸🇹',
    order: 3790,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1FB',
    shortcodes: ['flag_sv'],
    emoji: '🇸🇻',
    order: 3791,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1FD',
    shortcodes: ['flag_sx'],
    emoji: '🇸🇽',
    order: 3792,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1FE',
    shortcodes: ['flag_sy'],
    emoji: '🇸🇾',
    order: 3793,
    group: 9,
  },
  {
    hexcode: '1F1F8-1F1FF',
    shortcodes: ['flag_sz'],
    emoji: '🇸🇿',
    order: 3794,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1E6',
    shortcodes: ['flag_ta'],
    emoji: '🇹🇦',
    order: 3795,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1E8',
    shortcodes: ['flag_tc'],
    emoji: '🇹🇨',
    order: 3796,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1E9',
    shortcodes: ['flag_td'],
    emoji: '🇹🇩',
    order: 3797,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1EB',
    shortcodes: ['flag_tf'],
    emoji: '🇹🇫',
    order: 3798,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1EC',
    shortcodes: ['flag_tg'],
    emoji: '🇹🇬',
    order: 3799,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1ED',
    shortcodes: ['flag_th'],
    emoji: '🇹🇭',
    order: 3800,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1EF',
    shortcodes: ['flag_tj'],
    emoji: '🇹🇯',
    order: 3801,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F0',
    shortcodes: ['flag_tk'],
    emoji: '🇹🇰',
    order: 3802,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F1',
    shortcodes: ['flag_tl'],
    emoji: '🇹🇱',
    order: 3803,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F2',
    shortcodes: ['flag_tm'],
    emoji: '🇹🇲',
    order: 3804,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F3',
    shortcodes: ['flag_tn'],
    emoji: '🇹🇳',
    order: 3805,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F4',
    shortcodes: ['flag_to'],
    emoji: '🇹🇴',
    order: 3806,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F7',
    shortcodes: ['flag_tr'],
    emoji: '🇹🇷',
    order: 3807,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1F9',
    shortcodes: ['flag_tt'],
    emoji: '🇹🇹',
    order: 3808,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1FB',
    shortcodes: ['flag_tv'],
    emoji: '🇹🇻',
    order: 3809,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1FC',
    shortcodes: ['flag_tw'],
    emoji: '🇹🇼',
    order: 3810,
    group: 9,
  },
  {
    hexcode: '1F1F9-1F1FF',
    shortcodes: ['flag_tz'],
    emoji: '🇹🇿',
    order: 3811,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1E6',
    shortcodes: ['flag_ua'],
    emoji: '🇺🇦',
    order: 3812,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1EC',
    shortcodes: ['flag_ug'],
    emoji: '🇺🇬',
    order: 3813,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1F2',
    shortcodes: ['flag_um'],
    emoji: '🇺🇲',
    order: 3814,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1F3',
    shortcodes: ['flag_un'],
    emoji: '🇺🇳',
    order: 3815,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1F8',
    shortcodes: ['flag_us', 'usa'],
    emoji: '🇺🇸',
    order: 3816,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1FE',
    shortcodes: ['flag_uy'],
    emoji: '🇺🇾',
    order: 3817,
    group: 9,
  },
  {
    hexcode: '1F1FA-1F1FF',
    shortcodes: ['flag_uz'],
    emoji: '🇺🇿',
    order: 3818,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1E6',
    shortcodes: ['flag_va'],
    emoji: '🇻🇦',
    order: 3819,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1E8',
    shortcodes: ['flag_vc'],
    emoji: '🇻🇨',
    order: 3820,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1EA',
    shortcodes: ['flag_ve'],
    emoji: '🇻🇪',
    order: 3821,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1EC',
    shortcodes: ['flag_vg'],
    emoji: '🇻🇬',
    order: 3822,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1EE',
    shortcodes: ['flag_vi'],
    emoji: '🇻🇮',
    order: 3823,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1F3',
    shortcodes: ['flag_vn'],
    emoji: '🇻🇳',
    order: 3824,
    group: 9,
  },
  {
    hexcode: '1F1FB-1F1FA',
    shortcodes: ['flag_vu'],
    emoji: '🇻🇺',
    order: 3825,
    group: 9,
  },
  {
    hexcode: '1F1FC-1F1EB',
    shortcodes: ['flag_wf'],
    emoji: '🇼🇫',
    order: 3826,
    group: 9,
  },
  {
    hexcode: '1F1FC-1F1F8',
    shortcodes: ['flag_ws'],
    emoji: '🇼🇸',
    order: 3827,
    group: 9,
  },
  {
    hexcode: '1F1FD-1F1F0',
    shortcodes: ['flag_xk'],
    emoji: '🇽🇰',
    order: 3828,
    group: 9,
  },
  {
    hexcode: '1F1FE-1F1EA',
    shortcodes: ['flag_ye'],
    emoji: '🇾🇪',
    order: 3829,
    group: 9,
  },
  {
    hexcode: '1F1FE-1F1F9',
    shortcodes: ['flag_yt'],
    emoji: '🇾🇹',
    order: 3830,
    group: 9,
  },
  {
    hexcode: '1F1FF-1F1E6',
    shortcodes: ['flag_za'],
    emoji: '🇿🇦',
    order: 3831,
    group: 9,
  },
  {
    hexcode: '1F1FF-1F1F2',
    shortcodes: ['flag_zm'],
    emoji: '🇿🇲',
    order: 3832,
    group: 9,
  },
  {
    hexcode: '1F1FF-1F1FC',
    shortcodes: ['flag_zw'],
    emoji: '🇿🇼',
    order: 3833,
    group: 9,
  },
  {
    hexcode: '1F3F4-E0067-E0062-E0065-E006E-E0067-E007F',
    shortcodes: ['flag_gbeng', 'england'],
    emoji: '🏴󠁧󠁢󠁥󠁮󠁧󠁿',
    order: 3834,
    group: 9,
  },
  {
    hexcode: '1F3F4-E0067-E0062-E0073-E0063-E0074-E007F',
    shortcodes: ['flag_gbsct', 'scotland'],
    emoji: '🏴󠁧󠁢󠁳󠁣󠁴󠁿',
    order: 3835,
    group: 9,
  },
  {
    hexcode: '1F3F4-E0067-E0062-E0077-E006C-E0073-E007F',
    shortcodes: ['flag_gbwls', 'wales'],
    emoji: '🏴󠁧󠁢󠁷󠁬󠁳󠁿',
    order: 3836,
    group: 9,
  },
];

export default emojis;
